...
require(servr); require(readxl); require(openxlsx); require(janitor)
...
replace_repunit_rows <- function(df, reporting_unit_list, reporting_unit_name){
  df_sub <- df[which(df$reporting_unit %in% reporting_unit_list),]
  
  df_sub_s <- df_sub[1,]
  
  for (i in c(7:9,13)) {
    df_sub_s[1,i] <- sum(df_sub[,i], na.rm = T)
    
  }
  
  for (i in c(10:12)) {
    df_sub_s[1,i] <- mean(df_sub[,i], na.rm = T)
    
  }
  df_sub_s$reporting_unit <- reporting_unit_name
  
  df <- df[-which(df$reporting_unit %in% reporting_unit_list),]
  
  df <- rbind(df, df_sub_s)
...
{   # 2008 ward data
  ward.08 = read.csv(
    "C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/2008 results/2008_FallElection_President_WardbyWard.csv",
    stringsAsFactors = F,
    header = T,
    strip.white = T
  )
...
wisc.spat <-
  readOGR(
    dsn = "C:/Users/s_cas/Dropbox (Personal)/2016 voting election county results/Wisconsin/Geodata Wisconsin/WI_MunicipalWards_2016/WI_MunicipalWards_2016.shx",
...
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files/Prepped files")
...
# Loading Verified voting and 2016 areas to get the MCD_FIPS values - NEED ---------------------------
ward.2016 = read.csv(
  "wardwisconsin2016andrecount.csv",
  stringsAsFactors = F,
  header = T,
...
vot.equip.veri = read.csv(
  "C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/Old unused files/verifier-search-wiMOD.csv",
  header = TRUE,
  colClasses = NA,
  stringsAsFactors = F
)
...
# TIGER demographics - join to hindi and muni_county - NEED ------------------------------------------------------
# hindi_fips_2016_lookup.csv

tig_dem = data.frame(c(1:1926))

setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/Wisconsin municipal stats/Tiger data/")
...
hindi_fips_2016_lookup <- read.csv("C:/Users/s_cas/Dropbox (Personal)/2016 voting election county results/Wisconsin/All prep together/All processed files/hindi_fips_2016_lookup.csv")
...
wisc.spat <-
  readOGR(
    dsn = "C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/Geodata Wisconsin/WI_MunicipalWards_2016/WI_MunicipalWards_2016.shx",
    layer = "WI_MunicipalWards_2016",
    stringsAsFactors = F
  )
...
# 2016 election municipal GAB data NEED TO RUN -----------------------------------------------------------
ward.maydata.2016 = read.csv(
  "2016 Presidential and General Election EL-190 2017-10-05.csv",
  stringsAsFactors = F,
  header = T,
...
ward.edldata.2012 = xlsx::read.xlsx2(
...
ward.gabdata.2008 = xlsx::read.xlsx2(
  "C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/2008 results/2008nov_gab190_statistics_xls_13418MOD.xls",
  stringsAsFactors = F,
  header = T,
  sheetIndex = 1,
  colClasses = NA
...
gov.2018 = xlsx::read.xlsx2(
  "2018/Governor_WardByWard_withDistricts 2018 General_0.xlsx",
  stringsAsFactors = F, sheetIndex = 1)

# Replace blanks with NA
gov.2018 = data.frame(apply(gov.2018, 2, function(x) gsub("^$|^ $", NA, x)))
gov.2018 = clean_names(gov.2018)

{ # Preparing basic data
  names(gov.2018)[3] <- c("reporting_unit")
  
  gov.2018[,7:length(gov.2018)] <- lapply(gov.2018[,7:length(gov.2018)], function(x) as.numeric(as.character(x)))
  gov.2018$votes.tot <- rowSums(gov.2018[,7:length(gov.2018)], na.rm = T)
  
  gov.2018$county[gov.2018$county == ""] <- NA
  
  require(zoo)
  gov.2018$county <- na.locf(gov.2018$county)
  
  gov.2018$reporting_unit = as.character(gov.2018$reporting_unit)
  gov.2018$reporting_unit_orig <- gov.2018$reporting_unit
  gov.2018$reporting_unit = ifelse(gov.2018$reporting_unit == "","Wards 1",gov.2018$reporting_unit)
  gov.2018$reporting_unit = toupper(gov.2018$reporting_unit)
  gov.2018$county = toupper(gov.2018$county)
  
  gov.2018 <- subset(gov.2018, reporting_unit != "COUNTY TOTALS:")
  gov.2018 <- subset(gov.2018, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  # gov.2018 <- as.data.frame(lapply(gov.2018, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(gov.2018$reporting_unit)*2,2)
  
  require(stringr)
  gov.2018$reporting_unit[which(grepl("[0-9]", gov.2018$reporting_unit) == F)] <- paste(gov.2018$reporting_unit[which(grepl("[0-9]", gov.2018$reporting_unit) == F)], "1", sep = " ")
  
  gov.2018$reporting_unit <- gsub("WARD WD", "WARD", gov.2018$reporting_unit)
  gov.2018$reporting_unit <- gsub("\\bWD\\b", "WARD", gov.2018$reporting_unit)
  gov.2018$reporting_unit <- gsub("\\bWDS\\b", "WARD", gov.2018$reporting_unit)
  gov.2018$reporting_unit <- gsub("\\bWARD\\b", "WARD", gov.2018$reporting_unit)
  
  gov.2018$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", gov.2018$reporting_unit)
  gov.2018$reporting_unit <- gsub("WARDS([0-9])", "WARD \\1", gov.2018$reporting_unit)
  
  
  gov.2018$municipality_name <- unlist(str_split(str = gov.2018$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  gov.2018$municipality_name <- gsub(" WARDS","", gov.2018$municipality_name)
  gov.2018$municipality_name <- gsub(" WARD","", gov.2018$municipality_name)
  gov.2018$municipality_name <- na.locf(gov.2018$municipality_name)
  gov.2018$municipality_orig = gov.2018$municipality_name
  
  
  # gov.2018$municipality_name = with(gov.2018, paste(municipality_type, "of", municipality,sep=" "))
  # gov.2018$reporting_unit = with(gov.2018, paste(municipality_name, reporting_unit))
  # colnames(gov.2018)[1:3] = c("county","municipality_name","reporting_unit")
  # gov.2018$municipality_name = toupper(gov.2018$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gov.2018$municipality_name[grep("TOWN OF WINDSOR", gov.2018$municipality_name)] = "VILLAGE OF WINDSOR"
  gov.2018$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gov.2018$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gov.2018$county[grep("TOWN OF MAINE", gov.2018$municipality_name)] = "MARATHON"
  gov.2018$municipality_name[grepl("TOWN OF MAINE", gov.2018$municipality_name) &
                               grepl("MARATHON", gov.2018$county)
                             ] = "VILLAGE OF MAINE"
  # gov.2018$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gov.2018$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gov.2018$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gov.2018$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  gov.2018$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gov.2018$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  gov.2018$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gov.2018$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gov.2018$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gov.2018$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # gov.2018.look = with(gov.2018, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(gov.2018.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # gov.2018$congress_district = na.locf(gov.2018$congress_district)
  gov.2018$county = na.locf(gov.2018$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  gov.2018$county = trim.trailing(gov.2018$county)
  
  gov.2018$municipality_name = toupper(gov.2018$municipality_name)
  gov.2018$county = toupper(gov.2018$county)
  
  gov.2018$muni_county = paste(gov.2018$municipality_name, gov.2018$county)
  gov.2018$muni_county_orig = paste(gov.2018$municipality_orig, gov.2018$county)
  ...
  gov.2018$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gov.2018$reporting_unit)
  gov.2018$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("WARDS","WARD",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("  "," ",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("&","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("\\bAND\\b","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub(",","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub(" - ","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub(" -","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("- ","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub(" ","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("WD","",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("COMBINED","",gov.2018$reporting_unit)
  
  gov.2018$reporting_unit = toupper(gov.2018$reporting_unit)
  gov.2018$reporting_unit = paste(gov.2018$reporting_unit, gov.2018$county, sep = "-")
  
  gov.2018$reporting_unit = gsub(" ","-",gov.2018$reporting_unit)
  gov.2018$reporting_unit = gsub("--","-",gov.2018$reporting_unit)
  
  gov.2018.look = with(gov.2018, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  names(gov.2018.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(gov.20182,gov.20183,gov.20184)
  
  gov.2018.v = gov.2018
  # gov.2018.v$county = NULL
  gov.2018.v$county = NULL
  gov.2018.v$municipality_name = NULL
  gov.2018.v$municipality_type = NULL
  gov.2018.v$municipalityno = NULL
  gov.2018.v$municipality = NULL
  gov.2018.v$municipality_orig = NULL
  # gov.2018.v$reporting_unit = NULL
  gov.2018.v$order = NULL
  gov.2018.v$hindi = NULL
  # gov.2018.v$assemblydistrict = NULL
  
  gov.2018.l <- gov.2018.v
  gov.2018.l$muni_county = NULL
  gov.2018.l$reporting_unit <- NULL
  gov.2018.l$muni_county_orig = NULL
  gov.2018.l$reporting_unit_orig <- NULL
  gov.2018.l$votes.tot <- NULL
  gov.2018.l$x_3 <- NULL
  require(tidyr)
  gov.2018.l = gather(gov.2018.l, cand, votes_rec)
  gov.2018.l$reporting_unit <- rep(gov.2018.v$reporting_unit)
  # colnames(gov.2018.l) = c("reporting_unit", "cand", "votes.rec")
  
  # names(gov.2018.v)
  
  unique(gov.2018.l$cand)
  
  gov.2018.l$cand.group = NA
  gov.2018.l$cand.group[grep("tony_evers",gov.2018.l$cand)] = "democrat"
  gov.2018.l$cand.group[grep("scott_walker",gov.2018.l$cand)] = "republican"
  gov.2018.l$cand.group[which(!grepl("county",gov.2018.l$cand) &
                                !grepl("muni",gov.2018.l$cand) & 
                                !grepl("district",gov.2018.l$cand) &
                                !grepl("tony_evers",gov.2018.l$cand) & !grepl("scott_walker",gov.2018.l$cand) &
                                !grepl("reporting",gov.2018.l$cand) 
                              )] = "other"

  # Joining back county and municipality names
  gov.2018.l = plyr::join(gov.2018.l, gov.2018.look, by = "reporting_unit", match = "first")
  
  gov.2018.l$votes_rec = as.numeric(as.integer(gov.2018.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(gov.2018.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  gov.2018.l = plyr::join(gov.2018.l,tot.votes.look,by="reporting_unit",match="first")
  gov.2018.l$votes_perc = (gov.2018.l$votes_rec / gov.2018.l$votes_tot) * 100
  
  gov.2018.l$muni_county = paste(gov.2018.l$municipality, gov.2018.l$county)
  
  # gov.2018.l$votes_perc = (gov.2018.l$votes_rec / gov.2018.l$votes_tot) * 100
  
  gov.2018.l$votes_tot <- NULL
  gov.2018.melt = melt(gov.2018.l)
  gov.2018.cast = dcast(gov.2018.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  gov.2018.l = gov.2018.cast
  gov.2018.l[,grep("_NA", names(gov.2018.l))] <- NULL
  
  gov.2018.l = plyr::join(gov.2018.l,tot.votes.look,by="reporting_unit",match="first")
  
  # gov.2018.l$trump_prim_won = with(gov.2018.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # gov.2018.l$votes_tot_cruz = NULL
  # gov.2018.l$votes_tot_trump = NULL
  # gov.2018.l$votes_tot_kasich = NULL
  
  colnames(gov.2018.l)[grep("votes_tot_other",colnames(gov.2018.l))] = "votes_tot"
  
  # rm(gov.2018.cast,gov.2018.melt,
  # gov.2018.mun.group ,
  # gov.2018, gov.2018.look, gov.2018.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2018 gab stats
  gabdata.2018.gov = read.xlsx2(
    "2018/EL-190 2018 General Election report 2019-05-07.xlsx",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
    colClasses = NA # Thanks: http://www.briandalessandro.com/blog/how-to-read-numeric-columns-with-read-xlsx2-in-r/
  ...
  # Preparing basic data
  gabdata.2018.gov = clean_names(gabdata.2018.gov)
  
  # Removing the totals row
  gabdata.2018.gov <- gabdata.2018.gov[gabdata.2018.gov$polling_place_name != "TOTALS",]
  
  gabdata.2018.gov$reporting_unit_orig = gabdata.2018.gov$reporting_unit
  # gabdata.2018.gov$reporting_unit = toupper(gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$municipality = toupper(gabdata.2018.gov$municipality)
  gabdata.2018.gov$county = toupper(gabdata.2018.gov$county)
  
  gabdata.2018.gov$county = gsub(" COUNTY","", gabdata.2018.gov$county)
  gabdata.2018.gov$muni_county = paste(gabdata.2018.gov$municipality, gabdata.2018.gov$county)
  
  gabdata.2018.gov$reporting_unit = paste(gabdata.2018.gov$municipality,gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = toupper(gabdata.2018.gov$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2018.gov$municipality[grep("TOWN OF WINDSOR", gabdata.2018.gov$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2018.gov$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2018.gov$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2018.gov$county[grep("TOWN OF MAINE", gabdata.2018.gov$municipality)] = "MARATHON"
  gabdata.2018.gov$municipality[grepl("TOWN OF MAINE", gabdata.2018.gov$municipality) &
                                  grepl("MARATHON", gabdata.2018.gov$county)
                                ] = "VILLAGE OF MAINE"
  # gabdata.2018.gov$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2018.gov$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2018.gov$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2018.gov$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2018.gov$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2018.gov$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2018.gov$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2018.gov$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2018.gov$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2018.gov$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2018.gov$muni_county = paste(gabdata.2018.gov$municipality, gabdata.2018.gov$county)
  
  gabdata.2018.gov$reporting_unit = paste(gabdata.2018.gov$reporting_unit, gabdata.2018.gov$county)
  
  gabdata.2018.gov$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit <- gsub("\\bWDS\\b", "WARD", gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2018.gov$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2018.gov$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("WARDS","WARD",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("  "," ",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("&","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("\\bAND\\b","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub(",","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub(" - ","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub(" -","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("- ","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub(" ","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("WD","",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("COMBINED","",gabdata.2018.gov$reporting_unit)
  
  gabdata.2018.gov$reporting_unit = gsub(" ","-",gabdata.2018.gov$reporting_unit)
  gabdata.2018.gov$reporting_unit = gsub("--","-",gabdata.2018.gov$reporting_unit)
  
  gabdata.2018.gov$registrants = with(gabdata.2018.gov, open_registrants + late_registrants + election_day_registrants)
  
  gabdata.repunit <- gabdata.2018.gov
  
  
  # ward.gabdata.group = dplyr::group_by(gabdata.2018.gov,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  ...
  # 
  # write.csv(gabdata.repunit,"jun.2016.2018.gov.repunit.data.csv")
...
{ # Matching voting statistics with the election statistics
  ...
  # setdiff(unique(gabdata.repunit$muni_county), unique(gov.2018.l$muni_county))
  # setdiff(unique(gov.2018.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # gov.2018.l$muni_county[grep("VILLAGE OF MAINE MARATHON",gov.2018.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # gov.2018.l$municipality[grep("TOWN OF MAINE MARATHON",gov.2018.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(gov.2018.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(gov.2018.l, muni_county %in% differences)
  # 
  # Missing:
  ...
  gabdata.join$tch_votes = with(gabdata.repunit, dre_touch_screen);
  gabdata.join$tch_prop = with(gabdata.repunit, dre_touch_screen / total_ballots)
  ...
  gabdata.join$counted_absent_prop = with(gabdata.repunit,
                                          (in_person_absentees_counted + non_uocava_absentees_transmitted_counted) / total_ballots)
  ...
  gabdata.join$rejected_absent_prop = with(gabdata.repunit,
                                           (in_person_absentees_rejected + non_uocava_absentees_transmitted_rejected) / total_ballots)

  gabdata.join$absentee_undeliverable = with(gabdata.repunit,
                                             non_uocava_absentees_transmitted_undeliverable / total_ballots)
  ...
  table(gov.2018.l$reporting_unit %in% gabdata.join$reporting_unit) # all there
  
  # Not in function from here: https://stackoverflow.com/questions/5831794/opposite-of-in
  '%!in%' <- function(x,y)!('%in%'(x,y))
  
  
  table(gabdata.join$reporting_unit %in% gov.2018.l$reporting_unit )
  
  
  gov.2018.new = plyr::join(gov.2018.l, gabdata.join, by = "reporting_unit")
  gov.2018.l = gov.2018.new
  
  gov.2018.l[is.na(gov.2018.l)] = NA
  rm(gov.2018.new)
  
  setdiff(gov.2018.l$reporting_unit, gabdata.join$reporting_unit) # nothing
...
} # Matching voting statistics with election results
{
  ...
  gov.2018.dem = plyr::join(gov.2018.l, demographics.caps.join, by = "county", match = "first")
  ...
  gov.2018.dem = plyr::join(gov.2018.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  #
  # unique(grepl("550101201",gov.2018.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, gov.2018.dem$muni_county)
  ...
  # setdiff(gov.2018.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(gov.2018.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  gov.2018.dem = plyr::join(gov.2018.dem, off_vote_machines, by = "muni_county")
  
  gov.2018.dem$tch_model_off_full = NA
  gov.2018.dem$tch_model_off_full = with(gov.2018.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", gov.2018.dem$tch_model_off_full))
  gov.2018.dem$tch_model_off_full = with(gov.2018.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",gov.2018.dem$tch_model_off_full))
  gov.2018.dem$tch_model_off_full = with(gov.2018.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",gov.2018.dem$tch_model_off_full))
  gov.2018.dem$tch_model_off_full = with(gov.2018.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",gov.2018.dem$tch_model_off_full))
  gov.2018.dem$tch_model_off_full[is.na(gov.2018.dem$tch_model_off_full)] = "aNone"
  gov.2018.dem$tch_model_off_full = with(gov.2018.dem, ifelse(is.na(tch_use_off),
                                                              NA,gov.2018.dem$tch_model_off_full))
  
  colnames(gov.2018.dem)[grep("tch.use",colnames(gov.2018.dem),fixed = T)] = "tch_use_verivote"
  colnames(gov.2018.dem)[grep("os.use",colnames(gov.2018.dem),fixed = T)] = "os_use_verivote"
  colnames(gov.2018.dem)[grep("bmd.use",colnames(gov.2018.dem),fixed = T)] = "bmd_use_verivote"
  colnames(gov.2018.dem)[grep("X._collegedegreepersons25._2013",colnames(gov.2018.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2018.gov.repunit.df = gov.2018.dem
  # model.2018.gov.df = subset(model.2018.gov.df, votes_tot > 19)
  ...
  model.2018.gov.repunit.df$log.pop.sq.mile.2010 = log(model.2018.gov.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2018.gov.repunit.df))
  
  model.2018.gov.repunit.df2 = with(model.2018.gov.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
  ...
  rownames(model.2018.gov.repunit.df2) = model.2018.gov.repunit.df2$reporting_unit
  
  model.2018.gov.repunit.df = model.2018.gov.repunit.df2
  
  # model.2018.gov.repunit.df$MCD_FIPS = as.character(model.2018.gov.repunit.df$MCD_FIPS)
  ...
  model.2018.gov.repunit.df = plyr::join(model.2018.gov.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2018.gov.repunit.df, by = "muni_county", match = "first")
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2018.gov.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2018.gov.df,"model_2018_gov.csv")
write.csv(model.2018.gov.repunit.df,"Prepped files/2018/model_2018_gov_repunit.csv")

# Senator 2018 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
getwd()

sen.2018 = xlsx::read.xlsx2(
  "2018/US Senator_WardByWard_withDistricts 2018 General_0.xlsx",
  stringsAsFactors = F, sheetIndex = 1)

# Replace blanks with NA
sen.2018 = data.frame(apply(sen.2018, 2, function(x) gsub("^$|^ $", NA, x)))
sen.2018 = clean_names(sen.2018)

{ # Preparing basic data
  names(sen.2018)[3] <- c("reporting_unit")
  
  sen.2018[,7:length(sen.2018)] <- lapply(sen.2018[,7:length(sen.2018)], function(x) as.numeric(as.character(x)))
  sen.2018$votes.tot <- rowSums(sen.2018[,7:length(sen.2018)], na.rm = T)
  
  sen.2018$county[sen.2018$county == ""] <- NA
  
  require(zoo)
  sen.2018$county <- na.locf(sen.2018$county)
  
  sen.2018$reporting_unit = as.character(sen.2018$reporting_unit)
  sen.2018$reporting_unit_orig <- sen.2018$reporting_unit
  sen.2018$reporting_unit = ifelse(sen.2018$reporting_unit == "","Wards 1",sen.2018$reporting_unit)
  sen.2018$reporting_unit = toupper(sen.2018$reporting_unit)
  sen.2018$county = toupper(sen.2018$county)
  
  sen.2018 <- subset(sen.2018, reporting_unit != "COUNTY TOTALS:")
  sen.2018 <- subset(sen.2018, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  # sen.2018 <- as.data.frame(lapply(sen.2018, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(sen.2018$reporting_unit)*2,2)
  
  require(stringr)
  sen.2018$reporting_unit[which(grepl("[0-9]", sen.2018$reporting_unit) == F)] <- paste(sen.2018$reporting_unit[which(grepl("[0-9]", sen.2018$reporting_unit) == F)], "1", sep = " ")
  
  sen.2018$reporting_unit <- gsub("WARD WD", "WARD", sen.2018$reporting_unit)
  sen.2018$reporting_unit <- gsub("\\bWD\\b", "WARD", sen.2018$reporting_unit)
  sen.2018$reporting_unit <- gsub("\\bWDS\\b", "WARD", sen.2018$reporting_unit)
  sen.2018$reporting_unit <- gsub("\\bWARD\\b", "WARD", sen.2018$reporting_unit)
  
  sen.2018$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", sen.2018$reporting_unit)
  sen.2018$reporting_unit <- gsub("WARDS([0-9])", "WARD \\1", sen.2018$reporting_unit)
  
  
  sen.2018$municipality_name <- unlist(str_split(str = sen.2018$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  sen.2018$municipality_name <- gsub(" WARDS","", sen.2018$municipality_name)
  sen.2018$municipality_name <- gsub(" WARD","", sen.2018$municipality_name)
  sen.2018$municipality_name <- na.locf(sen.2018$municipality_name)
  sen.2018$municipality_orig = sen.2018$municipality_name
  
  
  # sen.2018$municipality_name = with(sen.2018, paste(municipality_type, "of", municipality,sep=" "))
  # sen.2018$reporting_unit = with(sen.2018, paste(municipality_name, reporting_unit))
  # colnames(sen.2018)[1:3] = c("county","municipality_name","reporting_unit")
  # sen.2018$municipality_name = toupper(sen.2018$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  sen.2018$municipality_name[grep("TOWN OF WINDSOR", sen.2018$municipality_name)] = "VILLAGE OF WINDSOR"
  sen.2018$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", sen.2018$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # sen.2018$county[grep("TOWN OF MAINE", sen.2018$municipality_name)] = "MARATHON"
  sen.2018$municipality_name[grepl("TOWN OF MAINE", sen.2018$municipality_name) &
                               grepl("MARATHON", sen.2018$county)
                             ] = "VILLAGE OF MAINE"
  # sen.2018$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", sen.2018$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  sen.2018$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2018$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  sen.2018$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2018$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  sen.2018$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2018$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  sen.2018$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2018$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # sen.2018.look = with(sen.2018, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(sen.2018.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # sen.2018$congress_district = na.locf(sen.2018$congress_district)
  sen.2018$county = na.locf(sen.2018$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  sen.2018$county = trim.trailing(sen.2018$county)
  
  sen.2018$municipality_name = toupper(sen.2018$municipality_name)
  sen.2018$county = toupper(sen.2018$county)
  
  sen.2018$muni_county = paste(sen.2018$municipality_name, sen.2018$county)
  sen.2018$muni_county_orig = paste(sen.2018$municipality_orig, sen.2018$county)
  ...
  sen.2018$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", sen.2018$reporting_unit)
  sen.2018$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("WARDS","WARD",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("  "," ",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("&","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("\\bAND\\b","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub(",","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub(" - ","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub(" -","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("- ","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub(" ","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("WD","",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("COMBINED","",sen.2018$reporting_unit)
  
  sen.2018$reporting_unit = toupper(sen.2018$reporting_unit)
  sen.2018$reporting_unit = paste(sen.2018$reporting_unit, sen.2018$county, sep = "-")
  
  sen.2018$reporting_unit = gsub(" ","-",sen.2018$reporting_unit)
  sen.2018$reporting_unit = gsub("--","-",sen.2018$reporting_unit)
  
  sen.2018.look = with(sen.2018, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  names(sen.2018.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(sen.20182,sen.20183,sen.20184)
  
  sen.2018.v = sen.2018
  # sen.2018.v$county = NULL
  sen.2018.v$county = NULL
  sen.2018.v$municipality_name = NULL
  sen.2018.v$municipality_type = NULL
  sen.2018.v$municipalityno = NULL
  sen.2018.v$municipality = NULL
  sen.2018.v$municipality_orig = NULL
  # sen.2018.v$reporting_unit = NULL
  sen.2018.v$order = NULL
  sen.2018.v$hindi = NULL
  # sen.2018.v$assemblydistrict = NULL
  
  sen.2018.l <- sen.2018.v
  sen.2018.l$muni_county = NULL
  sen.2018.l$reporting_unit <- NULL
  sen.2018.l$muni_county_orig = NULL
  sen.2018.l$reporting_unit_orig <- NULL
  sen.2018.l$votes.tot <- NULL
  sen.2018.l$x_3 <- NULL
  require(tidyr)
  sen.2018.l = gather(sen.2018.l, cand, votes_rec)
  sen.2018.l$reporting_unit <- rep(sen.2018.v$reporting_unit)
  # colnames(sen.2018.l) = c("reporting_unit", "cand", "votes.rec")
  
  # names(sen.2018.v)
  
  # unique(sen.2018.l$cand)
  
  sen.2018.l$cand.group = NA
  sen.2018.l$cand.group[grep("tammy_baldwin",sen.2018.l$cand)] = "democrat"
  sen.2018.l$cand.group[grep("leah_vukmir",sen.2018.l$cand)] = "republican"
  sen.2018.l$cand.group[which(!grepl("county",sen.2018.l$cand) &
                                !grepl("muni",sen.2018.l$cand) & 
                                !grepl("district",sen.2018.l$cand) &
                                !grepl("tammy_baldwin",sen.2018.l$cand) & !grepl("leah_vukmir",sen.2018.l$cand) &
                                !grepl("reporting",sen.2018.l$cand) 
  )] = "other"
  
  # Joining back county and municipality names
  sen.2018.l = plyr::join(sen.2018.l, sen.2018.look, by = "reporting_unit", match = "first")
  
  sen.2018.l$votes_rec = as.numeric(as.integer(sen.2018.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(sen.2018.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  sen.2018.l = plyr::join(sen.2018.l,tot.votes.look,by="reporting_unit",match="first")
  sen.2018.l$votes_perc = (sen.2018.l$votes_rec / sen.2018.l$votes_tot) * 100
  
  sen.2018.l$muni_county = paste(sen.2018.l$municipality, sen.2018.l$county)
  
  # sen.2018.l$votes_perc = (sen.2018.l$votes_rec / sen.2018.l$votes_tot) * 100
  
  sen.2018.l$votes_tot <- NULL
  sen.2018.melt = melt(sen.2018.l)
  sen.2018.cast = dcast(sen.2018.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  sen.2018.l = sen.2018.cast
  sen.2018.l[,grep("_NA", names(sen.2018.l))] <- NULL
  
  sen.2018.l = plyr::join(sen.2018.l,tot.votes.look,by="reporting_unit",match="first")
  
  # sen.2018.l$trump_prim_won = with(sen.2018.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # sen.2018.l$votes_tot_cruz = NULL
  # sen.2018.l$votes_tot_trump = NULL
  # sen.2018.l$votes_tot_kasich = NULL
  
  colnames(sen.2018.l)[grep("votes_tot_other",colnames(sen.2018.l))] = "votes_tot"
  
  # rm(sen.2018.cast,sen.2018.melt,
  # sen.2018.mun.group ,
  # sen.2018, sen.2018.look, sen.2018.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2018 gab stats
  gabdata.2018.sen = read.xlsx2(
    "2018/EL-190 2018 General Election report 2019-05-07.xlsx",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
  ...
  # Preparing basic data
  gabdata.2018.sen = clean_names(gabdata.2018.sen)
  
  # Removing the totals row
  gabdata.2018.sen <- gabdata.2018.sen[gabdata.2018.sen$polling_place_name != "TOTALS",]
  
  gabdata.2018.sen$reporting_unit_orig = gabdata.2018.sen$reporting_unit
  # gabdata.2018.sen$reporting_unit = toupper(gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$municipality = toupper(gabdata.2018.sen$municipality)
  gabdata.2018.sen$county = toupper(gabdata.2018.sen$county)
  
  gabdata.2018.sen$county = gsub(" COUNTY","", gabdata.2018.sen$county)
  gabdata.2018.sen$muni_county = paste(gabdata.2018.sen$municipality, gabdata.2018.sen$county)
  
  gabdata.2018.sen$reporting_unit = paste(gabdata.2018.sen$municipality,gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = toupper(gabdata.2018.sen$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2018.sen$municipality[grep("TOWN OF WINDSOR", gabdata.2018.sen$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2018.sen$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2018.sen$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2018.sen$county[grep("TOWN OF MAINE", gabdata.2018.sen$municipality)] = "MARATHON"
  gabdata.2018.sen$municipality[grepl("TOWN OF MAINE", gabdata.2018.sen$municipality) &
                                  grepl("MARATHON", gabdata.2018.sen$county)
                                ] = "VILLAGE OF MAINE"
  # gabdata.2018.sen$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2018.sen$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2018.sen$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2018.sen$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2018.sen$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2018.sen$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2018.sen$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2018.sen$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2018.sen$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2018.sen$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2018.sen$muni_county = paste(gabdata.2018.sen$municipality, gabdata.2018.sen$county)
  
  gabdata.2018.sen$reporting_unit = paste(gabdata.2018.sen$reporting_unit, gabdata.2018.sen$county)
  
  gabdata.2018.sen$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit <- gsub("\\bWDS\\b", "WARD", gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2018.sen$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2018.sen$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("WARDS","WARD",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("  "," ",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("&","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("\\bAND\\b","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub(",","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub(" - ","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub(" -","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("- ","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub(" ","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("WD","",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("COMBINED","",gabdata.2018.sen$reporting_unit)
  
  gabdata.2018.sen$reporting_unit = gsub(" ","-",gabdata.2018.sen$reporting_unit)
  gabdata.2018.sen$reporting_unit = gsub("--","-",gabdata.2018.sen$reporting_unit)
  
  gabdata.2018.sen$registrants = with(gabdata.2018.sen, open_registrants + late_registrants + election_day_registrants)
  
  gabdata.repunit <- gabdata.2018.sen
  
  
  # ward.gabdata.group = dplyr::group_by(gabdata.2018.sen,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2018.sen.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(sen.2018.l$muni_county))
  # setdiff(unique(sen.2018.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # sen.2018.l$muni_county[grep("VILLAGE OF MAINE MARATHON",sen.2018.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # sen.2018.l$municipality[grep("TOWN OF MAINE MARATHON",sen.2018.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(sen.2018.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(sen.2018.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  gabdata.join$total_ballots = ifelse(gabdata.repunit$total_ballots == 0,
                                      gabdata.repunit$total_voters,
                                      gabdata.repunit$total_ballots)
  gabdata.join$tch_votes = with(gabdata.repunit, dre_touch_screen);   gabdata.join$tch_prop = with(gabdata.repunit, dre_touch_screen / total_ballots)
  gabdata.join$tch_prop = ifelse(gabdata.join$tch_prop>1, 1, gabdata.join$tch_prop)
  ...
  table(sen.2018.l$reporting_unit %in% gabdata.join$reporting_unit) # all there
  
  # Not in function from here: https://stackoverflow.com/questions/5831794/opposite-of-in
  '%!in%' <- function(x,y)!('%in%'(x,y))
  
  
  table(gabdata.join$reporting_unit %in% sen.2018.l$reporting_unit )
  
  
  sen.2018.new = plyr::join(sen.2018.l, gabdata.join, by = "reporting_unit")
  sen.2018.l = sen.2018.new
  
  sen.2018.l[is.na(sen.2018.l)] = NA
  rm(sen.2018.new)
  
  setdiff(sen.2018.l$reporting_unit, gabdata.join$reporting_unit) # nothing
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  sen.2018.dem = plyr::join(sen.2018.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  sen.2018.dem = plyr::join(sen.2018.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
  off_vote_machines = read_excel("voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",sen.2018.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, sen.2018.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(sen.2018.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(sen.2018.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  sen.2018.dem = plyr::join(sen.2018.dem, off_vote_machines, by = "muni_county")
  
  sen.2018.dem$tch_model_off_full = NA
  sen.2018.dem$tch_model_off_full = with(sen.2018.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", sen.2018.dem$tch_model_off_full))
  sen.2018.dem$tch_model_off_full = with(sen.2018.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",sen.2018.dem$tch_model_off_full))
  sen.2018.dem$tch_model_off_full = with(sen.2018.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",sen.2018.dem$tch_model_off_full))
  sen.2018.dem$tch_model_off_full = with(sen.2018.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",sen.2018.dem$tch_model_off_full))
  sen.2018.dem$tch_model_off_full[is.na(sen.2018.dem$tch_model_off_full)] = "aNone"
  sen.2018.dem$tch_model_off_full = with(sen.2018.dem, ifelse(is.na(tch_use_off),
                                                              NA,sen.2018.dem$tch_model_off_full))
  
  colnames(sen.2018.dem)[grep("tch.use",colnames(sen.2018.dem),fixed = T)] = "tch_use_verivote"
  colnames(sen.2018.dem)[grep("os.use",colnames(sen.2018.dem),fixed = T)] = "os_use_verivote"
  colnames(sen.2018.dem)[grep("bmd.use",colnames(sen.2018.dem),fixed = T)] = "bmd_use_verivote"
  colnames(sen.2018.dem)[grep("X._collegedegreepersons25._2013",colnames(sen.2018.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2018.sen.repunit.df = sen.2018.dem
  # model.2018.sen.df = subset(model.2018.sen.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2018.sen.repunit.df$log.pop.sq.mile.2010 = log(model.2018.sen.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2018.sen.repunit.df))
  
  model.2018.sen.repunit.df2 = with(model.2018.sen.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2018.sen.repunit.df2) = model.2018.sen.repunit.df2$reporting_unit
  
  model.2018.sen.repunit.df = model.2018.sen.repunit.df2
  
  # model.2018.sen.repunit.df$MCD_FIPS = as.character(model.2018.sen.repunit.df$MCD_FIPS)
  ...
  model.2018.sen.repunit.df = plyr::join(model.2018.sen.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2018.sen.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2018.sen.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2018.sen.df,"model_2018_sen.csv")
write.csv(model.2018.sen.repunit.df,"Prepped files/2018/model_2018_sen_repunit.csv")

# Sec State 2018 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
getwd()

state.2018 = xlsx::read.xlsx2(
  "2018/Secretary of State_WardByWard_withDistricts 2018 General_0.xlsx",
  stringsAsFactors = F, sheetIndex = 1)

# Replace blanks with NA
state.2018 = data.frame(apply(state.2018, 2, function(x) gsub("^$|^ $", NA, x)))
state.2018 = clean_names(state.2018)

state.2018[,1:4] <- NULL

{ # Preparing basic data
  names(state.2018)[3] <- c("reporting_unit")
  
  state.2018[,7:length(state.2018)] <- lapply(state.2018[,7:length(state.2018)], as.numeric)
  state.2018$votes.tot <- rowSums(state.2018[,7:length(state.2018)], na.rm = T)
  
  state.2018$county[state.2018$county == ""] <- NA
  
  require(zoo)
  state.2018$county <- na.locf(state.2018$county)
  
  state.2018$reporting_unit = as.character(state.2018$reporting_unit)
  state.2018$reporting_unit_orig <- state.2018$reporting_unit
  state.2018$reporting_unit = ifelse(state.2018$reporting_unit == "","Wards 1",state.2018$reporting_unit)
  state.2018$reporting_unit = toupper(state.2018$reporting_unit)
  state.2018$county = toupper(state.2018$county)
  
  state.2018 <- subset(state.2018, reporting_unit != "COUNTY TOTALS:")
  state.2018 <- subset(state.2018, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  # state.2018 <- as.data.frame(lapply(state.2018, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(state.2018$reporting_unit)*2,2)
  
  require(stringr)
  state.2018$reporting_unit[which(grepl("[0-9]", state.2018$reporting_unit) == F)] <- paste(state.2018$reporting_unit[which(grepl("[0-9]", state.2018$reporting_unit) == F)], "1", sep = " ")
  
  state.2018$reporting_unit <- gsub("WARD WD", "WARD", state.2018$reporting_unit)
  state.2018$reporting_unit <- gsub("\\bWD\\b", "WARD", state.2018$reporting_unit)
  state.2018$reporting_unit <- gsub("\\bWDS\\b", "WARD", state.2018$reporting_unit)
  state.2018$reporting_unit <- gsub("\\bWARD\\b", "WARD", state.2018$reporting_unit)
  
  state.2018$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", state.2018$reporting_unit)
  state.2018$reporting_unit <- gsub("WARDS([0-9])", "WARD \\1", state.2018$reporting_unit)
  
  
  state.2018$municipality_name <- unlist(str_split(str = state.2018$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  state.2018$municipality_name <- gsub(" WARDS","", state.2018$municipality_name)
  state.2018$municipality_name <- gsub(" WARD","", state.2018$municipality_name)
  state.2018$municipality_name <- na.locf(state.2018$municipality_name)
  state.2018$municipality_orig = state.2018$municipality_name
  
  
  # state.2018$municipality_name = with(state.2018, paste(municipality_type, "of", municipality,sep=" "))
  # state.2018$reporting_unit = with(state.2018, paste(municipality_name, reporting_unit))
  # colnames(state.2018)[1:3] = c("county","municipality_name","reporting_unit")
  # state.2018$municipality_name = toupper(state.2018$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  state.2018$municipality_name[grep("TOWN OF WINDSOR", state.2018$municipality_name)] = "VILLAGE OF WINDSOR"
  state.2018$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", state.2018$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # state.2018$county[grep("TOWN OF MAINE", state.2018$municipality_name)] = "MARATHON"
  state.2018$municipality_name[grepl("TOWN OF MAINE", state.2018$municipality_name) &
                               grepl("MARATHON", state.2018$county)
                             ] = "VILLAGE OF MAINE"
  # state.2018$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", state.2018$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  state.2018$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", state.2018$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  state.2018$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", state.2018$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  state.2018$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", state.2018$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  state.2018$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", state.2018$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # state.2018.look = with(state.2018, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(state.2018.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # state.2018$congress_district = na.locf(state.2018$congress_district)
  state.2018$county = na.locf(state.2018$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  state.2018$county = trim.trailing(state.2018$county)
  
  state.2018$municipality_name = toupper(state.2018$municipality_name)
  state.2018$county = toupper(state.2018$county)
  
  state.2018$muni_county = paste(state.2018$municipality_name, state.2018$county)
  state.2018$muni_county_orig = paste(state.2018$municipality_orig, state.2018$county)
  ...
  state.2018$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", state.2018$reporting_unit)
  state.2018$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("WARDS","WARD",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("  "," ",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("&","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("\\bAND\\b","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub(",","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub(" - ","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub(" -","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("- ","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub(" ","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("WD","",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("COMBINED","",state.2018$reporting_unit)
  
  state.2018$reporting_unit = toupper(state.2018$reporting_unit)
  state.2018$reporting_unit = paste(state.2018$reporting_unit, state.2018$county, sep = "-")
  
  state.2018$reporting_unit = gsub(" ","-",state.2018$reporting_unit)
  state.2018$reporting_unit = gsub("--","-",state.2018$reporting_unit)
  
  state.2018.look = with(state.2018, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  names(state.2018.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(state.20182,state.20183,state.20184)
  
  state.2018.v = state.2018
  # state.2018.v$county = NULL
  state.2018.v$county = NULL
  state.2018.v$municipality_name = NULL
  state.2018.v$municipality_type = NULL
  state.2018.v$municipalityno = NULL
  state.2018.v$municipality = NULL
  state.2018.v$municipality_orig = NULL
  # state.2018.v$reporting_unit = NULL
  state.2018.v$order = NULL
  state.2018.v$hindi = NULL
  # state.2018.v$assemblydistrict = NULL
  
  state.2018.l <- state.2018.v
  state.2018.l$muni_county = NULL
  state.2018.l$reporting_unit <- NULL
  state.2018.l$muni_county_orig = NULL
  state.2018.l$reporting_unit_orig <- NULL
  state.2018.l$votes.tot <- NULL
  state.2018.l$x_3 <- NULL
  require(tidyr)
  state.2018.l = gather(state.2018.l, cand, votes_rec)
  state.2018.l$reporting_unit <- rep(state.2018.v$reporting_unit)
  # colnames(state.2018.l) = c("reporting_unit", "cand", "votes.rec")
  
  # names(state.2018.v)
  
  # unique(state.2018.l$cand)
  
  state.2018.l$cand.group = NA
  state.2018.l$cand.group[grep("doug_la_follette",state.2018.l$cand)] = "democrat"
  state.2018.l$cand.group[grep("jay_schroeder",state.2018.l$cand)] = "republican"
  state.2018.l$cand.group[which(!grepl("contest",state.2018.l$cand) &
                                !grepl("muni",state.2018.l$cand) & 
                                !grepl("district",state.2018.l$cand) &
                                !grepl("doug_la_follette",state.2018.l$cand) & !grepl("jay_schroeder",state.2018.l$cand) &
                                !grepl("reporting",state.2018.l$cand) &
                                  !grepl("order",state.2018.l$cand) 
  )] = "other"
  
  # Joining back county and municipality names
  state.2018.l = plyr::join(state.2018.l, state.2018.look, by = "reporting_unit", match = "first")
  
  state.2018.l$votes_rec = as.numeric(as.integer(state.2018.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(state.2018.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  state.2018.l = plyr::join(state.2018.l,tot.votes.look,by="reporting_unit",match="first")
  state.2018.l$votes_perc = (state.2018.l$votes_rec / state.2018.l$votes_tot) * 100
  
  state.2018.l$muni_county = paste(state.2018.l$municipality, state.2018.l$county)
  
  # state.2018.l$votes_perc = (state.2018.l$votes_rec / state.2018.l$votes_tot) * 100
  
  state.2018.l$votes_tot <- NULL
  state.2018.melt = melt(state.2018.l)
  state.2018.cast = dcast(state.2018.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  state.2018.l = state.2018.cast
  state.2018.l[,grep("_NA", names(state.2018.l))] <- NULL
  
  state.2018.l = plyr::join(state.2018.l,tot.votes.look,by="reporting_unit",match="first")
  
  # state.2018.l$trump_prim_won = with(state.2018.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # state.2018.l$votes_tot_cruz = NULL
  # state.2018.l$votes_tot_trump = NULL
  # state.2018.l$votes_tot_kasich = NULL
  
  colnames(state.2018.l)[grep("votes_tot_other",colnames(state.2018.l))] = "votes_tot"
  
  # rm(state.2018.cast,state.2018.melt,
  # state.2018.mun.group ,
  # state.2018, state.2018.look, state.2018.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2018 gab stats
  gabdata.2018.state = read.xlsx2(
    "2018/EL-190 2018 General Election report 2019-05-07.xlsx",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
  ...
  # Preparing basic data
  gabdata.2018.state = clean_names(gabdata.2018.state)
  
  # Removing the totals row
  gabdata.2018.state <- gabdata.2018.state[gabdata.2018.state$polling_place_name != "TOTALS",]
  
  gabdata.2018.state$reporting_unit_orig = gabdata.2018.state$reporting_unit
  # gabdata.2018.state$reporting_unit = toupper(gabdata.2018.state$reporting_unit)
  gabdata.2018.state$municipality = toupper(gabdata.2018.state$municipality)
  gabdata.2018.state$county = toupper(gabdata.2018.state$county)
  
  gabdata.2018.state$county = gsub(" COUNTY","", gabdata.2018.state$county)
  gabdata.2018.state$muni_county = paste(gabdata.2018.state$municipality, gabdata.2018.state$county)
  
  gabdata.2018.state$reporting_unit = paste(gabdata.2018.state$municipality,gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = toupper(gabdata.2018.state$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2018.state$municipality[grep("TOWN OF WINDSOR", gabdata.2018.state$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2018.state$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2018.state$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2018.state$county[grep("TOWN OF MAINE", gabdata.2018.state$municipality)] = "MARATHON"
  gabdata.2018.state$municipality[grepl("TOWN OF MAINE", gabdata.2018.state$municipality) &
                                  grepl("MARATHON", gabdata.2018.state$county)
                                ] = "VILLAGE OF MAINE"
  # gabdata.2018.state$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2018.state$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2018.state$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2018.state$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2018.state$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2018.state$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2018.state$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2018.state$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2018.state$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2018.state$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2018.state$muni_county = paste(gabdata.2018.state$municipality, gabdata.2018.state$county)
  
  gabdata.2018.state$reporting_unit = paste(gabdata.2018.state$reporting_unit, gabdata.2018.state$county)
  
  gabdata.2018.state$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit <- gsub("\\bWDS\\b", "WARD", gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2018.state$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2018.state$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("WARDS","WARD",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("  "," ",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("&","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("\\bAND\\b","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub(",","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub(" - ","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub(" -","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("- ","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub(" ","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("WD","",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("COMBINED","",gabdata.2018.state$reporting_unit)
  
  gabdata.2018.state$reporting_unit = gsub(" ","-",gabdata.2018.state$reporting_unit)
  gabdata.2018.state$reporting_unit = gsub("--","-",gabdata.2018.state$reporting_unit)
  
  gabdata.2018.state$registrants = with(gabdata.2018.state, open_registrants + late_registrants + election_day_registrants)
  
  gabdata.repunit <- gabdata.2018.state
  
  
  # ward.gabdata.group = dplyr::group_by(gabdata.2018.state,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2018.state.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(state.2018.l$muni_county))
  # setdiff(unique(state.2018.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # state.2018.l$muni_county[grep("VILLAGE OF MAINE MARATHON",state.2018.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # state.2018.l$municipality[grep("TOWN OF MAINE MARATHON",state.2018.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(state.2018.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(state.2018.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  table(state.2018.l$reporting_unit %in% gabdata.join$reporting_unit) # all there
  
  # Not in function from here: https://stackoverflow.com/questions/5831794/opposite-of-in
  '%!in%' <- function(x,y)!('%in%'(x,y))
  
  
  table(gabdata.join$reporting_unit %in% state.2018.l$reporting_unit )
  
  
  state.2018.new = plyr::join(state.2018.l, gabdata.join, by = "reporting_unit")
  state.2018.l = state.2018.new
  
  state.2018.l[is.na(state.2018.l)] = NA
  rm(state.2018.new)
  
  setdiff(state.2018.l$reporting_unit, gabdata.join$reporting_unit) # nothing
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  state.2018.dem = plyr::join(state.2018.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  state.2018.dem = plyr::join(state.2018.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
  off_vote_machines = read_excel("voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",state.2018.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, state.2018.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(state.2018.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(state.2018.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  state.2018.dem = plyr::join(state.2018.dem, off_vote_machines, by = "muni_county")
  
  state.2018.dem$tch_model_off_full = NA
  state.2018.dem$tch_model_off_full = with(state.2018.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", state.2018.dem$tch_model_off_full))
  state.2018.dem$tch_model_off_full = with(state.2018.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",state.2018.dem$tch_model_off_full))
  state.2018.dem$tch_model_off_full = with(state.2018.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",state.2018.dem$tch_model_off_full))
  state.2018.dem$tch_model_off_full = with(state.2018.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",state.2018.dem$tch_model_off_full))
  state.2018.dem$tch_model_off_full[is.na(state.2018.dem$tch_model_off_full)] = "aNone"
  state.2018.dem$tch_model_off_full = with(state.2018.dem, ifelse(is.na(tch_use_off),
                                                              NA,state.2018.dem$tch_model_off_full))
  
  colnames(state.2018.dem)[grep("tch.use",colnames(state.2018.dem),fixed = T)] = "tch_use_verivote"
  colnames(state.2018.dem)[grep("os.use",colnames(state.2018.dem),fixed = T)] = "os_use_verivote"
  colnames(state.2018.dem)[grep("bmd.use",colnames(state.2018.dem),fixed = T)] = "bmd_use_verivote"
  colnames(state.2018.dem)[grep("X._collegedegreepersons25._2013",colnames(state.2018.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2018.state.repunit.df = state.2018.dem
  # model.2018.state.df = subset(model.2018.state.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2018.state.repunit.df$log.pop.sq.mile.2010 = log(model.2018.state.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2018.state.repunit.df))
  
  model.2018.state.repunit.df2 = with(model.2018.state.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2018.state.repunit.df2) = model.2018.state.repunit.df2$reporting_unit
  
  model.2018.state.repunit.df = model.2018.state.repunit.df2
  
  # model.2018.state.repunit.df$MCD_FIPS = as.character(model.2018.state.repunit.df$MCD_FIPS)
  ...
  model.2018.state.repunit.df = plyr::join(model.2018.state.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2018.state.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2018.state.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2018.state.df,"model_2018_state.csv")
write.csv(model.2018.state.repunit.df,"Prepped files/2018/model_2018_state_repunit.csv")

# Presidential 2016 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
getwd()
...
# Replace blanks with NA
pres.2016 = data.frame(apply(pres.2016, 2, function(x) gsub("^$|^ $", NA, x)))
pres.2016[,5:length(pres.2016)] = apply(pres.2016[,5:length(pres.2016)], 2, function(x) as.character(x))
pres.2016[,5:length(pres.2016)] = apply(pres.2016[,5:length(pres.2016)], 2, function(x) as.numeric(x))
pres.2016 = clean_names(pres.2016)

#### Warning message:
#   non-unique values when setting 'row.names': 'CITY-OF-FITCHBURG-WARD-14-DANE', 'VILLAGE-OF-WHITEFISH-BAY-WARD-12-MILWAUKEE'

# str(pres.2016)
pres.2016.g <- group_by(pres.2016, reporting_unit, county_name, municipality_name, original_or_recount)
pres.2016.m <- summarise_all(pres.2016.g, sum)



{ # Preparing basic data
  names(pres.2016)[1] <- "county"
  names(pres.2016)[5] <- "votes.tot"
  
  pres.2016$county[pres.2016$county == ""] <- NA
  
  require(zoo)
  pres.2016$county <- na.locf(pres.2016$county)
  
  pres.2016$reporting_unit = as.character(pres.2016$reporting_unit)
  pres.2016$reporting_unit_orig <- pres.2016$reporting_unit
  pres.2016$reporting_unit = ifelse(pres.2016$reporting_unit == "","Wards 1",pres.2016$reporting_unit)
  pres.2016$reporting_unit = toupper(pres.2016$reporting_unit)
  pres.2016$county = toupper(pres.2016$county)
  
  pres.2016 <- subset(pres.2016, reporting_unit != "COUNTY TOTALS:")
  pres.2016 <- subset(pres.2016, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  pres.2016 <- as.data.frame(lapply(pres.2016, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(pres.2016$reporting_unit)*2,2)
  
  require(stringr)
  pres.2016$reporting_unit[which(grepl("[0-9]", pres.2016$reporting_unit) == F)] <- paste(pres.2016$reporting_unit[which(grepl("[0-9]", pres.2016$reporting_unit) == F)], "1", sep = " ")
  
  pres.2016$reporting_unit <- gsub("WARD WD", "WARD", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("\\bWD\\b", "WARD", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("\\bWDS\\b", "WARD", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("\\bWARD\\b", "WARD", pres.2016$reporting_unit)
  
  pres.2016$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", pres.2016$reporting_unit)
  
  
  pres.2016$municipality_name <- unlist(str_split(str = pres.2016$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  pres.2016$municipality_name <- gsub(" WARDS","", pres.2016$municipality_name)
  pres.2016$municipality_name <- gsub(" WARD","", pres.2016$municipality_name)
  pres.2016$municipality_name <- na.locf(pres.2016$municipality_name)
  pres.2016$municipality_orig = pres.2016$municipality_name
  
  
  # pres.2016$municipality_name = with(pres.2016, paste(municipality_type, "of", municipality,sep=" "))
  # pres.2016$reporting_unit = with(pres.2016, paste(municipality_name, reporting_unit))
  # colnames(pres.2016)[1:3] = c("county","municipality_name","reporting_unit")
  # pres.2016$municipality_name = toupper(pres.2016$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  pres.2016$municipality_name[grep("TOWN OF WINDSOR", pres.2016$municipality_name)] = "VILLAGE OF WINDSOR"
  pres.2016$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", pres.2016$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # pres.2016$county[grep("TOWN OF MAINE", pres.2016$municipality_name)] = "MARATHON"
  pres.2016$municipality_name[grepl("TOWN OF MAINE", pres.2016$municipality_name) &
                               grepl("MARATHON", pres.2016$county)
                             ] = "VILLAGE OF MAINE"
  # pres.2016$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", pres.2016$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  pres.2016$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", pres.2016$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  pres.2016$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", pres.2016$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  pres.2016$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", pres.2016$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  pres.2016$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", pres.2016$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # pres.2016.look = with(pres.2016, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(pres.2016.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # pres.2016$congress_district = na.locf(pres.2016$congress_district)
  pres.2016$county = na.locf(pres.2016$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  pres.2016$county = trim.trailing(pres.2016$county)
  
  pres.2016$municipality_name = toupper(pres.2016$municipality_name)
  pres.2016$county = toupper(pres.2016$county)
  
  pres.2016$muni_county = paste(pres.2016$municipality_name, pres.2016$county)
  pres.2016$muni_county_orig = paste(pres.2016$municipality_orig, pres.2016$county)
  ...
  pres.2016$reporting_unit <- gsub("WARD WD", "WARD", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("\\bWD\\b", "WARD", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("\\bWDS\\b", "WARDS", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("\\bWARD\\b", "WARD", pres.2016$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  
  pres.2016$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", pres.2016$reporting_unit)
  pres.2016$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", pres.2016$reporting_unit)
  # pres.2016$reporting_unit = gsub("WARDS","WARD",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("  "," ",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("&","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("\\bAND\\b","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub(",","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub(" - ","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub(" -","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("- ","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub(" ","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("WD","",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("COMBINED","",pres.2016$reporting_unit)
  
  pres.2016$reporting_unit = toupper(pres.2016$reporting_unit)
  pres.2016$reporting_unit = paste(pres.2016$reporting_unit, pres.2016$county, sep = "-")
  
  pres.2016$reporting_unit = gsub(" ","-",pres.2016$reporting_unit)
  pres.2016$reporting_unit = gsub("--","-",pres.2016$reporting_unit)
  
  pres.2016.look = with(pres.2016, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(pres.2016.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(pres.20162,pres.20163,pres.20164)
  
  pres.2016.v = pres.2016
  # pres.2016.v$county = NULL
  pres.2016.v$county = NULL
  pres.2016.v$municipality_name = NULL
  pres.2016.v$municipality_type = NULL
  pres.2016.v$municipalityno = NULL
  pres.2016.v$municipality = NULL
  pres.2016.v$municipality_orig = NULL
  # pres.2016.v$reporting_unit = NULL
  pres.2016.v$order = NULL
  pres.2016.v$hindi = NULL
  # pres.2016.v$assemblydistrict = NULL
  pres.2016.v = subset(  pres.2016.v, original_or_recount == "Recount")
  
  pres.2016.l <- pres.2016.v
  pres.2016.l$muni_county = NULL
  pres.2016.l$reporting_unit <- NULL
  pres.2016.l$muni_county_orig = NULL
  pres.2016.l$reporting_unit_orig <- NULL
  pres.2016.l$votes.tot <- NULL
  pres.2016.l$x_3 <- NULL
  pres.2016.l = gather(pres.2016.l, cand, votes_rec)
  pres.2016.l$reporting_unit <- rep(pres.2016.v$reporting_unit)
  # colnames(pres.2016.l) = c("reporting_unit", "cand", "votes.rec")
 
  pres.2016.l$original_or_recount = NULL
 
  pres.2016.l$cand.group = NA
  pres.2016.l$cand.group[grep("hillary",pres.2016.l$cand)] = "democrat"
  pres.2016.l$cand.group[grep("donald",pres.2016.l$cand)] = "republican"
  pres.2016.l$cand.group[grep("darrell",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("rocky",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("gary",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("monica",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("jill",pres.2016.l$cand)] = "other"
  
  # There were a bunch of write-in names for the Reporting unit level results
  pres.2016.l$cand.group[grep("cherunda",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("evan",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("michael_a",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("marshall",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("chris",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("laurence",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("hoefling",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("joseph",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("emidio",pres.2016.l$cand)] = "other"
  pres.2016.l$cand.group[grep("scatter",pres.2016.l$cand)] = "other"
  
  # Joining back county and municipality names
  pres.2016.l = plyr::join(pres.2016.l, pres.2016.look, by = "reporting_unit", match = "first")
  
  pres.2016.l$votes_rec = as.numeric(as.integer(pres.2016.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(pres.2016.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  pres.2016.l = plyr::join(pres.2016.l,tot.votes.look,by="reporting_unit",match="first")
  pres.2016.l$votes_perc = (pres.2016.l$votes_rec / pres.2016.l$votes_tot) * 100
  
  pres.2016.l$muni_county = paste(pres.2016.l$municipality, pres.2016.l$county)
  
  # pres.2016.l$votes_perc = (pres.2016.l$votes_rec / pres.2016.l$votes_tot) * 100
  
  pres.2016.l$votes_tot <- NULL
  pres.2016.melt = melt(pres.2016.l)
  pres.2016.cast = dcast(pres.2016.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  pres.2016.l = pres.2016.cast
  pres.2016.l[,grep("_NA", names(pres.2016.l))] <- NULL
  
  pres.2016.l = plyr::join(pres.2016.l,tot.votes.look,by="reporting_unit",match="first")
  
  # pres.2016.l$trump_prim_won = with(pres.2016.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # pres.2016.l$votes_tot_cruz = NULL
  # pres.2016.l$votes_tot_trump = NULL
  # pres.2016.l$votes_tot_kasich = NULL
  
  colnames(pres.2016.l)[grep("votes_tot_other",colnames(pres.2016.l))] = "votes_tot"
  
  # rm(pres.2016.cast,pres.2016.melt,
  # pres.2016.mun.group ,
  # pres.2016, pres.2016.look, pres.2016.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2016 gab stats
  gabdata.2016.pres = read.csv(
    "2016 Presidential and General Election EL-190 2017-10-05.csv",
    stringsAsFactors = F,
    header = T,
    strip.white = T
  ...
  # names(gabdata.2016.pres)
  
  # Preparing basic data
  gabdata.2016.pres = clean_names(gabdata.2016.pres)
  
  #### Warning message:
  #   non-unique values when setting 'row.names': 'CITY-OF-FITCHBURG-WARD-14-DANE', 'VILLAGE-OF-WHITEFISH-BAY-WARD-12-MILWAUKEE'
  
  # THE PROBLEM ABOVE IS FROM THE REPORTING UNIT CONVERSION THAT FOLLOWS, NOT THAT THERE ARE MULTIPLE VERSIONS OF THE SAME WARD
  
  # https://stackoverflow.com/questions/5863097/selecting-only-numeric-columns-from-a-data-frame
  
  # gabdata.2016.pres.g <- group_by(gabdata.2016.pres, hindi, county, municipality, reporting_unit)
  # gabdata.2016.pres.m <- dplyr::summarise_if(gabdata.2016.pres.g, is.numeric, sum, na.rm = T)
  
  
  
  gabdata.2016.pres$reporting_unit_orig = gabdata.2016.pres$reporting_unit
  # gabdata.2016.pres$reporting_unit = toupper(gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$municipality = toupper(gabdata.2016.pres$municipality)
  gabdata.2016.pres$county = toupper(gabdata.2016.pres$county)
  
  gabdata.2016.pres$county = gsub(" COUNTY","", gabdata.2016.pres$county)
  gabdata.2016.pres$muni_county = paste(gabdata.2016.pres$municipality, gabdata.2016.pres$county)
  
  gabdata.2016.pres$reporting_unit = paste(gabdata.2016.pres$municipality,gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = toupper(gabdata.2016.pres$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2016.pres$municipality[grep("TOWN OF WINDSOR", gabdata.2016.pres$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2016.pres$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2016.pres$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2016.pres$county[grep("TOWN OF MAINE", gabdata.2016.pres$municipality)] = "MARATHON"
  gabdata.2016.pres$municipality[grepl("TOWN OF MAINE", gabdata.2016.pres$municipality) &
                                  grepl("MARATHON", gabdata.2016.pres$county)
                                ] = "VILLAGE OF MAINE"
  # gabdata.2016.pres$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2016.pres$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2016.pres$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2016.pres$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2016.pres$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2016.pres$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2016.pres$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2016.pres$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2016.pres$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2016.pres$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2016.pres$muni_county = paste(gabdata.2016.pres$municipality, gabdata.2016.pres$county)
  
  gabdata.2016.pres$reporting_unit = paste(gabdata.2016.pres$reporting_unit, gabdata.2016.pres$county)
  
  gabdata.2016.pres$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2016.pres$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2016.pres$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2016.pres$reporting_unit)
  # gabdata.2016.pres$reporting_unit = gsub("WARDS","WARD",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("  "," ",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("&","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("\\bAND\\b","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub(",","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub(" - ","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub(" -","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("- ","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub(" ","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("WD","",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("COMBINED","",gabdata.2016.pres$reporting_unit)
  
  gabdata.2016.pres$reporting_unit = gsub(" ","-",gabdata.2016.pres$reporting_unit)
  gabdata.2016.pres$reporting_unit = gsub("--","-",gabdata.2016.pres$reporting_unit)
  
  gabdata.repunit <- gabdata.2016.pres
  # 
  # ward.gabdata.group = dplyr::group_by(gabdata.2016.pres,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  #                                       absentee_undeliverable = sum(absentee_undeliverable),
  #                                       received_by_election_day = sum(received_by_election_day),
  #                                       counted_absent = sum(counted),
  #                                       rejected_absent = sum(rejected),
  #                                        # late_received_after_the_election = sum(late_received_after_the_election),
  #                                       fwab_received = sum(fwab_received),
  #                                       fwab_counted = sum(fwab_counted),
  #                                       fwab_rejected = sum(fwab_rejected),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2016.pres.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(pres.2016.l$muni_county))
  # setdiff(unique(pres.2016.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # pres.2016.l$muni_county[grep("VILLAGE OF MAINE MARATHON",pres.2016.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # pres.2016.l$municipality[grep("TOWN OF MAINE MARATHON",pres.2016.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(pres.2016.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(pres.2016.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  gabdata.join$total_ballots = ifelse(gabdata.repunit$total_ballots == 0,
                                      gabdata.repunit$total_voters,
                                      gabdata.repunit$total_ballots)
  gabdata.join$tch_votes = with(gabdata.repunit, dre);   gabdata.join$tch_prop = with(gabdata.repunit, dre / total_ballots)
  gabdata.join$tch_prop = ifelse(gabdata.join$tch_prop>1, 1, gabdata.join$tch_prop)
  ...
  gabdata.join$sameday_reg_prop[gabdata.join$sameday_reg_prop == Inf] = NA
  gabdata.join$counted_absent_prop = with(gabdata.repunit,
                                          counted / total_ballots)
  gabdata.join$counted_absent_prop[gabdata.join$counted_absent_prop == Inf] = NA
  gabdata.join$rejected_absent_prop = with(gabdata.repunit,
                                           rejected / total_ballots)
  gabdata.join$absentee_undeliverable = with(gabdata.repunit,
  ...
  table(pres.2016.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  pres.2016.new = plyr::join(pres.2016.l, gabdata.join, by = "reporting_unit")
  pres.2016.l = pres.2016.new
  
  pres.2016.l[is.na(pres.2016.l)] = NA
  rm(pres.2016.new)
  
  setdiff(pres.2016.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  pres.2016.dem = plyr::join(pres.2016.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  pres.2016.dem = plyr::join(pres.2016.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",pres.2016.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, pres.2016.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(pres.2016.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(pres.2016.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  pres.2016.dem = plyr::join(pres.2016.dem, off_vote_machines, by = "muni_county")
  
  pres.2016.dem$tch_model_off_full = NA
  pres.2016.dem$tch_model_off_full = with(pres.2016.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", pres.2016.dem$tch_model_off_full))
  pres.2016.dem$tch_model_off_full = with(pres.2016.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",pres.2016.dem$tch_model_off_full))
  pres.2016.dem$tch_model_off_full = with(pres.2016.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",pres.2016.dem$tch_model_off_full))
  pres.2016.dem$tch_model_off_full = with(pres.2016.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",pres.2016.dem$tch_model_off_full))
  pres.2016.dem$tch_model_off_full[is.na(pres.2016.dem$tch_model_off_full)] = "aNone"
  pres.2016.dem$tch_model_off_full = with(pres.2016.dem, ifelse(is.na(tch_use_off),
                                                              NA,pres.2016.dem$tch_model_off_full))
  
  colnames(pres.2016.dem)[grep("tch.use",colnames(pres.2016.dem),fixed = T)] = "tch_use_verivote"
  colnames(pres.2016.dem)[grep("os.use",colnames(pres.2016.dem),fixed = T)] = "os_use_verivote"
  colnames(pres.2016.dem)[grep("bmd.use",colnames(pres.2016.dem),fixed = T)] = "bmd_use_verivote"
  colnames(pres.2016.dem)[grep("X._collegedegreepersons25._2013",colnames(pres.2016.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2016.pres.repunit.df = pres.2016.dem
  # model.2016.pres.df = subset(model.2016.pres.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2016.pres.repunit.df$log.pop.sq.mile.2010 = log(model.2016.pres.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2016.pres.repunit.df))
  
  model.2016.pres.repunit.df2 = with(model.2016.pres.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2016.pres.repunit.df2) = model.2016.pres.repunit.df2$reporting_unit
  
  model.2016.pres.repunit.df = model.2016.pres.repunit.df2
  
  # model.2016.pres.repunit.df$MCD_FIPS = as.character(model.2016.pres.repunit.df$MCD_FIPS)
  ...
  model.2016.pres.repunit.df = plyr::join(model.2016.pres.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2016.pres.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2016.pres.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2016.pres.df,"model_2016_pres.csv")
write.csv(model.2016.pres.repunit.df,"Prepped files/2016/model_2016_pres_repunit.csv")
...
sen.2016 = read.xlsx(
  "2016/Ward by Ward Report -US Senator.xlsx", sheet = 2, startRow  = 11
)

# Replace blanks with NA
sen.2016 = data.frame(apply(sen.2016, 2, function(x) gsub("^$|^ $", NA, x)))
sen.2016[,3:length(sen.2016)] = apply(sen.2016[,3:length(sen.2016)], 2, function(x) as.character(x))
sen.2016[,3:length(sen.2016)] = apply(sen.2016[,3:length(sen.2016)], 2, function(x) as.numeric(x))
sen.2016 = clean_names(sen.2016)
...
{ # Preparing basic data
  names(sen.2016)[1:3] <- c("county", "reporting_unit", "votes.tot")
  
  
  require(zoo)
  sen.2016$county <- na.locf(sen.2016$county)
  
  
  # str(sen.2016)
  sen.2016.g <- group_by(sen.2016, reporting_unit, county)
  sen.2016.m <- summarise_all(sen.2016.g, sum)

  sen.2016$county[sen.2016$county == ""] <- NA

  sen.2016$reporting_unit = as.character(sen.2016$reporting_unit)
  sen.2016$reporting_unit_orig <- sen.2016$reporting_unit
  sen.2016$reporting_unit = ifelse(sen.2016$reporting_unit == "","Wards 1",sen.2016$reporting_unit)
  sen.2016$reporting_unit = toupper(sen.2016$reporting_unit)
  sen.2016$county = toupper(sen.2016$county)
  
  sen.2016 <- subset(sen.2016, reporting_unit != "COUNTY TOTALS:")
  sen.2016 <- subset(sen.2016, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  sen.2016 <- as.data.frame(lapply(sen.2016, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(sen.2016$reporting_unit)*2,2)
  
  require(stringr)
  sen.2016$reporting_unit[which(grepl("[0-9]", sen.2016$reporting_unit) == F)] <- paste(sen.2016$reporting_unit[which(grepl("[0-9]", sen.2016$reporting_unit) == F)], "1", sep = " ")
  
  sen.2016$reporting_unit <- gsub("WARD WD", "WARD", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("\\bWD\\b", "WARD", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("\\bWDS\\b", "WARD", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("\\bWARD\\b", "WARD", sen.2016$reporting_unit)
  
  sen.2016$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", sen.2016$reporting_unit)
  
  
  sen.2016$municipality_name <- unlist(str_split(str = sen.2016$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  sen.2016$municipality_name <- gsub(" WARDS","", sen.2016$municipality_name)
  sen.2016$municipality_name <- gsub(" WARD","", sen.2016$municipality_name)
  sen.2016$municipality_name <- na.locf(sen.2016$municipality_name)
  sen.2016$municipality_orig = sen.2016$municipality_name
  
  
  # sen.2016$municipality_name = with(sen.2016, paste(municipality_type, "of", municipality,sep=" "))
  # sen.2016$reporting_unit = with(sen.2016, paste(municipality_name, reporting_unit))
  # colnames(sen.2016)[1:3] = c("county","municipality_name","reporting_unit")
  # sen.2016$municipality_name = toupper(sen.2016$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  sen.2016$municipality_name[grep("TOWN OF WINDSOR", sen.2016$municipality_name)] = "VILLAGE OF WINDSOR"
  sen.2016$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", sen.2016$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # sen.2016$county[grep("TOWN OF MAINE", sen.2016$municipality_name)] = "MARATHON"
  sen.2016$municipality_name[grepl("TOWN OF MAINE", sen.2016$municipality_name) &
                                grepl("MARATHON", sen.2016$county)
                              ] = "VILLAGE OF MAINE"
  # sen.2016$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", sen.2016$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  sen.2016$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2016$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  sen.2016$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2016$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  sen.2016$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2016$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  sen.2016$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2016$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # sen.2016.look = with(sen.2016, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(sen.2016.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # sen.2016$congress_district = na.locf(sen.2016$congress_district)
  sen.2016$county = na.locf(sen.2016$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  sen.2016$county = trim.trailing(sen.2016$county)
  
  sen.2016$municipality_name = toupper(sen.2016$municipality_name)
  sen.2016$county = toupper(sen.2016$county)
  
  sen.2016$muni_county = paste(sen.2016$municipality_name, sen.2016$county)
  sen.2016$muni_county_orig = paste(sen.2016$municipality_orig, sen.2016$county)
  ...
  sen.2016$reporting_unit <- gsub("WARD WD", "WARD", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("\\bWD\\b", "WARD", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("\\bWDS\\b", "WARDS", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("\\bWARD\\b", "WARD", sen.2016$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  
  sen.2016$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", sen.2016$reporting_unit)
  sen.2016$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", sen.2016$reporting_unit)
  # sen.2016$reporting_unit = gsub("WARDS","WARD",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("  "," ",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("&","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("\\bAND\\b","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub(",","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub(" - ","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub(" -","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("- ","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub(" ","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("WD","",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("COMBINED","",sen.2016$reporting_unit)
  
  sen.2016$reporting_unit = toupper(sen.2016$reporting_unit)
  sen.2016$reporting_unit = paste(sen.2016$reporting_unit, sen.2016$county, sep = "-")
  
  sen.2016$reporting_unit = gsub(" ","-",sen.2016$reporting_unit)
  sen.2016$reporting_unit = gsub("--","-",sen.2016$reporting_unit)
  
  sen.2016.look = with(sen.2016, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(sen.2016.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(sen.20162,sen.20163,sen.20164)
  
  sen.2016.v = sen.2016
  # sen.2016.v$county = NULL
  sen.2016.v$county = NULL
  sen.2016.v$municipality_name = NULL
  sen.2016.v$municipality_type = NULL
  sen.2016.v$municipalityno = NULL
  sen.2016.v$municipality = NULL
  sen.2016.v$municipality_orig = NULL
  # sen.2016.v$reporting_unit = NULL
  sen.2016.v$order = NULL
  sen.2016.v$hindi = NULL
  # sen.2016.v$assemblydistrict = NULL
  # sen.2016.v = subset(  sen.2016.v, original_or_recount == "Recount")
  
  sen.2016.l <- sen.2016.v
  sen.2016.l$muni_county = NULL
  sen.2016.l$reporting_unit <- NULL
  sen.2016.l$muni_county_orig = NULL
  sen.2016.l$reporting_unit_orig <- NULL
  sen.2016.l$votes.tot <- NULL
  sen.2016.l$x_3 <- NULL
  sen.2016.l = gather(sen.2016.l, cand, votes_rec)
  sen.2016.l$reporting_unit <- rep(sen.2016.v$reporting_unit)
  # colnames(sen.2016.l) = c("reporting_unit", "cand", "votes.rec")
  
  # sen.2016.l$original_or_recount = NULL
  
  sen.2016.l$cand.group = NA
  sen.2016.l$cand.group[grep("feingold",sen.2016.l$cand)] = "democrat"
  sen.2016.l$cand.group[grep("johnson",sen.2016.l$cand)] = "republican"
  sen.2016.l$cand.group[grep("anderson",sen.2016.l$cand)] = "other"
  sen.2016.l$cand.group[grep("schiess",sen.2016.l$cand)] = "other"
  sen.2016.l$cand.group[grep("scatter",sen.2016.l$cand)] = "other"
  
  # Joining back county and municipality names
  sen.2016.l = plyr::join(sen.2016.l, sen.2016.look, by = "reporting_unit", match = "first")
  
  sen.2016.l$votes_rec = as.numeric(as.integer(sen.2016.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(sen.2016.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  sen.2016.l = plyr::join(sen.2016.l,tot.votes.look,by="reporting_unit",match="first")
  sen.2016.l$votes_perc = (sen.2016.l$votes_rec / sen.2016.l$votes_tot) * 100
  
  sen.2016.l$muni_county = paste(sen.2016.l$municipality, sen.2016.l$county)
  
  # sen.2016.l$votes_perc = (sen.2016.l$votes_rec / sen.2016.l$votes_tot) * 100
  
  sen.2016.l$votes_tot <- NULL
  sen.2016.melt = melt(sen.2016.l)
  sen.2016.cast = dcast(sen.2016.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  sen.2016.l = sen.2016.cast
  sen.2016.l[,grep("_NA", names(sen.2016.l))] <- NULL
  
  sen.2016.l = plyr::join(sen.2016.l,tot.votes.look,by="reporting_unit",match="first")
  
  # sen.2016.l$trump_prim_won = with(sen.2016.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # sen.2016.l$votes_tot_cruz = NULL
  # sen.2016.l$votes_tot_trump = NULL
  # sen.2016.l$votes_tot_kasich = NULL
  
  colnames(sen.2016.l)[grep("votes_tot_other",colnames(sen.2016.l))] = "votes_tot"
  
  # rm(sen.2016.cast,sen.2016.melt,
  # sen.2016.mun.group ,
  # sen.2016, sen.2016.look, sen.2016.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2016 gab stats
  gabdata.2016.sen = read.csv(
    "2016 Presidential and General Election EL-190 2017-10-05.csv",
    stringsAsFactors = F,
    header = T,
    strip.white = T
  ...
  # names(gabdata.2016.sen)
  
  # Preparing basic data
  gabdata.2016.sen = clean_names(gabdata.2016.sen)
  ...
  # gabdata.2016.sen.g <- group_by(gabdata.2016.sen, hindi, county, municipality, reporting_unit)
  # gabdata.2016.sen.m <- dplyr::summarise_if(gabdata.2016.sen.g, is.numeric, sum, na.rm = T)
  
  
  
  gabdata.2016.sen$reporting_unit_orig = gabdata.2016.sen$reporting_unit
  # gabdata.2016.sen$reporting_unit = toupper(gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$municipality = toupper(gabdata.2016.sen$municipality)
  gabdata.2016.sen$county = toupper(gabdata.2016.sen$county)
  
  gabdata.2016.sen$county = gsub(" COUNTY","", gabdata.2016.sen$county)
  gabdata.2016.sen$muni_county = paste(gabdata.2016.sen$municipality, gabdata.2016.sen$county)
  
  gabdata.2016.sen$reporting_unit = paste(gabdata.2016.sen$municipality,gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = toupper(gabdata.2016.sen$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2016.sen$municipality[grep("TOWN OF WINDSOR", gabdata.2016.sen$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2016.sen$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2016.sen$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2016.sen$county[grep("TOWN OF MAINE", gabdata.2016.sen$municipality)] = "MARATHON"
  gabdata.2016.sen$municipality[grepl("TOWN OF MAINE", gabdata.2016.sen$municipality) &
                                   grepl("MARATHON", gabdata.2016.sen$county)
                                 ] = "VILLAGE OF MAINE"
  # gabdata.2016.sen$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2016.sen$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2016.sen$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2016.sen$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2016.sen$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2016.sen$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2016.sen$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2016.sen$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2016.sen$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2016.sen$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2016.sen$muni_county = paste(gabdata.2016.sen$municipality, gabdata.2016.sen$county)
  
  gabdata.2016.sen$reporting_unit = paste(gabdata.2016.sen$reporting_unit, gabdata.2016.sen$county)
  
  gabdata.2016.sen$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2016.sen$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2016.sen$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2016.sen$reporting_unit)
  # gabdata.2016.sen$reporting_unit = gsub("WARDS","WARD",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("  "," ",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("&","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("\\bAND\\b","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub(",","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub(" - ","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub(" -","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("- ","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub(" ","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("WD","",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("COMBINED","",gabdata.2016.sen$reporting_unit)
  
  gabdata.2016.sen$reporting_unit = gsub(" ","-",gabdata.2016.sen$reporting_unit)
  gabdata.2016.sen$reporting_unit = gsub("--","-",gabdata.2016.sen$reporting_unit)
  
  gabdata.repunit <- gabdata.2016.sen
  # 
  # ward.gabdata.group = dplyr::group_by(gabdata.2016.sen,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2016.sen.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(sen.2016.l$muni_county))
  # setdiff(unique(sen.2016.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # sen.2016.l$muni_county[grep("VILLAGE OF MAINE MARATHON",sen.2016.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # sen.2016.l$municipality[grep("TOWN OF MAINE MARATHON",sen.2016.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(sen.2016.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(sen.2016.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  table(sen.2016.l$reporting_unit %in% gabdata.join$reporting_unit)
  which(sen.2016.l$reporting_unit %!in% gabdata.join$reporting_unit)

  # All of the following 9 simply don't exist
  # [1] "VILLAGE-OF-LAC-LA-BELLE-WARD-2-JEFFERSON" "TOWN-OF-PLYMOUTH-WARD-1-JUNEAU"          
  # [3] "TOWN-OF-RANDALL-WARDS-17-KENOSHA"         "VILLAGE-OF-NORTH-BAY-WARD-1-RACINE"      
  # [5] "TOWN-OF-UNION-WARDS-14-ROCK"              "TOWN-OF-HAYWARD-WARDS-18-SAWYER"         
  # [7] "TOWN-OF-CALEDONIA-WARDS-12-TREMPEALEAU"   "CITY-OF-BURLINGTON-WARD-9-WALWORTH"      
  # [9] "CITY-OF-BURLINGTON-WARD-10-WALWORTH"
  
  sen.2016.new = plyr::join(sen.2016.l, gabdata.join, by = "reporting_unit")
  sen.2016.l = sen.2016.new
  
  sen.2016.l[is.na(sen.2016.l)] = NA
  rm(sen.2016.new)
  
  setdiff(sen.2016.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  sen.2016.dem = plyr::join(sen.2016.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  sen.2016.dem = plyr::join(sen.2016.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",sen.2016.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, sen.2016.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(sen.2016.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(sen.2016.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  sen.2016.dem = plyr::join(sen.2016.dem, off_vote_machines, by = "muni_county")
  
  sen.2016.dem$tch_model_off_full = NA
  sen.2016.dem$tch_model_off_full = with(sen.2016.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                                "Dominion (Premier)-Accuvote TSX", sen.2016.dem$tch_model_off_full))
  sen.2016.dem$tch_model_off_full = with(sen.2016.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                                "Dominion (Sequoia)/Command Central-Edge",sen.2016.dem$tch_model_off_full))
  sen.2016.dem$tch_model_off_full = with(sen.2016.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                                "Dominion (Sequoia)/Command Central-Edge",sen.2016.dem$tch_model_off_full))
  sen.2016.dem$tch_model_off_full = with(sen.2016.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                                "ES&S iVotronic",sen.2016.dem$tch_model_off_full))
  sen.2016.dem$tch_model_off_full[is.na(sen.2016.dem$tch_model_off_full)] = "aNone"
  sen.2016.dem$tch_model_off_full = with(sen.2016.dem, ifelse(is.na(tch_use_off),
                                                                NA,sen.2016.dem$tch_model_off_full))
  
  colnames(sen.2016.dem)[grep("tch.use",colnames(sen.2016.dem),fixed = T)] = "tch_use_verivote"
  colnames(sen.2016.dem)[grep("os.use",colnames(sen.2016.dem),fixed = T)] = "os_use_verivote"
  colnames(sen.2016.dem)[grep("bmd.use",colnames(sen.2016.dem),fixed = T)] = "bmd_use_verivote"
  colnames(sen.2016.dem)[grep("X._collegedegreepersons25._2013",colnames(sen.2016.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2016.sen.repunit.df = sen.2016.dem
  # model.2016.sen.df = subset(model.2016.sen.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2016.sen.repunit.df$log.pop.sq.mile.2010 = log(model.2016.sen.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2016.sen.repunit.df))
  
  model.2016.sen.repunit.df2 = with(model.2016.sen.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                            sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                            county_paper_or_paperplusmachine,
                                                                            ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                            pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2016.sen.repunit.df2) = model.2016.sen.repunit.df2$reporting_unit
  
  model.2016.sen.repunit.df = model.2016.sen.repunit.df2
  
  # model.2016.sen.repunit.df$MCD_FIPS = as.character(model.2016.sen.repunit.df$MCD_FIPS)
  ...
  model.2016.sen.repunit.df = plyr::join(model.2016.sen.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.spat.o <-
  #   readOGR(
  #     dsn = "Prepped files/wisc.senent.orig.geojson",
  #     layer = "OGRGeoJSON",
  #     disambiguateFIDs = T,
  #     stringsAsFactors = F
  #   )
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2016.sen.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.senent.rep2016.sen.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2016.sen.df,"model_2016_sen.csv")
write.csv(model.2016.sen.repunit.df,"Prepped files/2016/model_2016_sen_repunit.csv")

# Congressional 2016 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
getwd()
...
read_excel_allsheets <- function(filename, tibble = FALSE) {
  ...
  x <- lapply(sheets, function(X) readxl::read_excel(filename, sheet = X, skip = 8,
                                                     n_max = 760,
                                                     trim_ws = T))
...
mysheets <- read_excel_allsheets("2016/Ward by Ward Report -Congress.xlsx")

sheet_names <- list(NA)

# FOR CONGRESS VOTE IN 2016, SHEET 4 HAD A REPUBLICAN WRITE IN. PROBABLY BEST JUST NOT INCLUDING. SHEET 5 DID NOT HAVE A REPUBLICAN CANDIDATE, SO SHOULD REMOVE TOO. 
...
for (i in c(2:3,6:9)) {
  ...
  sheet_names[[i-1]] <- names(sheet)
  
  sheet_w_no <- paste("Sheet", i, ".", sep = "")
  names(sheet) <- gsub(sheet_w_no, "", names(sheet))
  sheet <- clean_names(sheet)
  
  names(sheet)[1:3] <- c("county", "reporting_unit", "total_votes_cast")
  names(sheet)[length(sheet)] <- "scattering"
...
cong.2016 <- rbind(mysheets[[2]], mysheets[[3]], #mysheets[[4]], mysheets[[5]],
                   mysheets[[6]], mysheets[[7]], mysheets[[8]], mysheets[[9]])

rm(mysheets)
# Now need to combine the sheets together!

# Replace blanks with NA
cong.2016 = data.frame(apply(cong.2016, 2, function(x) gsub("^$|^ $", NA, x)))
cong.2016 = clean_names(cong.2016)

cong.2016[,3:length(cong.2016)] = apply(cong.2016[,3:length(cong.2016)], 2, function(x) as.character(x))
cong.2016[,3:length(cong.2016)] = apply(cong.2016[,3:length(cong.2016)], 2, function(x) as.numeric(x))
cong.2016 = clean_names(cong.2016)
...
{ # Preparing basic data
  names(cong.2016)[1:3] <- c("county", "reporting_unit", "votes.tot")
  
  
  require(zoo)
  cong.2016$county <- na.locf(cong.2016$county)
  
  
  # str(cong.2016)
  cong.2016.g <- group_by(cong.2016, reporting_unit, county)
  cong.2016.m <- summarise_all(cong.2016.g, sum)
  
  cong.2016$county[cong.2016$county == ""] <- NA
  
  cong.2016$reporting_unit = as.character(cong.2016$reporting_unit)
  cong.2016$reporting_unit_orig <- cong.2016$reporting_unit
  cong.2016$reporting_unit = ifelse(cong.2016$reporting_unit == "","Wards 1",cong.2016$reporting_unit)
  cong.2016$reporting_unit = toupper(cong.2016$reporting_unit)
  cong.2016$county = toupper(cong.2016$county)
  
  cong.2016 <- subset(cong.2016, reporting_unit != "COUNTY TOTALS:")
  cong.2016 <- subset(cong.2016, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  cong.2016 <- as.data.frame(lapply(cong.2016, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(cong.2016$reporting_unit)*2,2)
  
  require(stringr)
  cong.2016$reporting_unit[which(grepl("[0-9]", cong.2016$reporting_unit) == F)] <- paste(cong.2016$reporting_unit[which(grepl("[0-9]", cong.2016$reporting_unit) == F)], "1", sep = " ")
  
  cong.2016$reporting_unit <- gsub("WARD WD", "WARD", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("\\bWD\\b", "WARD", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("\\bWDS\\b", "WARD", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("\\bWARD\\b", "WARD", cong.2016$reporting_unit)
  
  cong.2016$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", cong.2016$reporting_unit)
  
  
  cong.2016$municipality_name <- unlist(str_split(str = cong.2016$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  cong.2016$municipality_name <- gsub(" WARDS","", cong.2016$municipality_name)
  cong.2016$municipality_name <- gsub(" WARD","", cong.2016$municipality_name)
  cong.2016$municipality_name <- na.locf(cong.2016$municipality_name)
  cong.2016$municipality_orig = cong.2016$municipality_name
  
  
  # cong.2016$municipality_name = with(cong.2016, paste(municipality_type, "of", municipality,sep=" "))
  # cong.2016$reporting_unit = with(cong.2016, paste(municipality_name, reporting_unit))
  # colnames(cong.2016)[1:3] = c("county","municipality_name","reporting_unit")
  # cong.2016$municipality_name = toupper(cong.2016$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  cong.2016$municipality_name[grep("TOWN OF WINDSOR", cong.2016$municipality_name)] = "VILLAGE OF WINDSOR"
  cong.2016$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", cong.2016$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # cong.2016$county[grep("TOWN OF MAINE", cong.2016$municipality_name)] = "MARATHON"
  cong.2016$municipality_name[grepl("TOWN OF MAINE", cong.2016$municipality_name) &
                               grepl("MARATHON", cong.2016$county)
                             ] = "VILLAGE OF MAINE"
  # cong.2016$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", cong.2016$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  cong.2016$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", cong.2016$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  cong.2016$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", cong.2016$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  cong.2016$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", cong.2016$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  cong.2016$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", cong.2016$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # cong.2016.look = with(cong.2016, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(cong.2016.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # cong.2016$congress_district = na.locf(cong.2016$congress_district)
  cong.2016$county = na.locf(cong.2016$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  cong.2016$county = trim.trailing(cong.2016$county)
  
  cong.2016$municipality_name = toupper(cong.2016$municipality_name)
  cong.2016$county = toupper(cong.2016$county)
  
  cong.2016$muni_county = paste(cong.2016$municipality_name, cong.2016$county)
  cong.2016$muni_county_orig = paste(cong.2016$municipality_orig, cong.2016$county)
  ...
  cong.2016$reporting_unit <- gsub("WARD WD", "WARD", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("\\bWD\\b", "WARD", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("\\bWDS\\b", "WARDS", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("\\bWARD\\b", "WARD", cong.2016$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  
  cong.2016$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", cong.2016$reporting_unit)
  cong.2016$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", cong.2016$reporting_unit)
  # cong.2016$reporting_unit = gsub("WARDS","WARD",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("  "," ",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("&","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("\\bAND\\b","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub(",","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub(" - ","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub(" -","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("- ","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub(" ","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("WD","",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("COMBINED","",cong.2016$reporting_unit)
  
  cong.2016$reporting_unit = toupper(cong.2016$reporting_unit)
  cong.2016$reporting_unit = paste(cong.2016$reporting_unit, cong.2016$county, sep = "-")
  
  cong.2016$reporting_unit = gsub(" ","-",cong.2016$reporting_unit)
  cong.2016$reporting_unit = gsub("--","-",cong.2016$reporting_unit)
  
  cong.2016.look = with(cong.2016, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(cong.2016.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(cong.20162,cong.20163,cong.20164)
  
  cong.2016.v = cong.2016
  # cong.2016.v$county = NULL
  cong.2016.v$county = NULL
  cong.2016.v$municipality_name = NULL
  cong.2016.v$municipality_type = NULL
  cong.2016.v$municipalityno = NULL
  cong.2016.v$municipality = NULL
  cong.2016.v$municipality_orig = NULL
  # cong.2016.v$reporting_unit = NULL
  cong.2016.v$order = NULL
  cong.2016.v$hindi = NULL
  # cong.2016.v$assemblydistrict = NULL
  # cong.2016.v = subset(  cong.2016.v, original_or_recount == "Recount")
  
  cong.2016.l <- cong.2016.v
  cong.2016.l$muni_county = NULL
  cong.2016.l$reporting_unit <- NULL
  cong.2016.l$muni_county_orig = NULL
  cong.2016.l$reporting_unit_orig <- NULL
  cong.2016.l$votes.tot <- NULL
  cong.2016.l$x_3 <- NULL
  cong.2016.l = gather(cong.2016.l, cand, votes_rec)
  cong.2016.l$reporting_unit <- rep(cong.2016.v$reporting_unit)
  # colnames(cong.2016.l) = c("reporting_unit", "cand", "votes.rec")
  
  # cong.2016.l$original_or_recount = NULL
  
  cong.2016.l$cand.group = NA
  cong.2016.l$cand.group[grep("dem",cong.2016.l$cand)] = "democrat"
  cong.2016.l$cand.group[grep("rep",cong.2016.l$cand)] = "republican"
  cong.2016.l$cand.group[grep("other",cong.2016.l$cand)] = "other"
  cong.2016.l$cand.group[grep("scattering",cong.2016.l$cand)] = "other"
  
  # Joining back county and municipality names
  cong.2016.l = plyr::join(cong.2016.l, cong.2016.look, by = "reporting_unit", match = "first")
  
  cong.2016.l$votes_rec = as.numeric(as.integer(cong.2016.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(cong.2016.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  cong.2016.l = plyr::join(cong.2016.l,tot.votes.look,by="reporting_unit",match="first")
  cong.2016.l$votes_perc = (cong.2016.l$votes_rec / cong.2016.l$votes_tot) * 100
  
  cong.2016.l$muni_county = paste(cong.2016.l$municipality, cong.2016.l$county)
  
  # cong.2016.l$votes_perc = (cong.2016.l$votes_rec / cong.2016.l$votes_tot) * 100
  
  cong.2016.l$votes_tot <- NULL
  cong.2016.melt = melt(cong.2016.l)
  cong.2016.cast = dcast(cong.2016.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  cong.2016.l = cong.2016.cast
  cong.2016.l[,grep("_NA", names(cong.2016.l))] <- NULL
  
  cong.2016.l = plyr::join(cong.2016.l,tot.votes.look,by="reporting_unit",match="first")
  
  # cong.2016.l$trump_prim_won = with(cong.2016.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # cong.2016.l$votes_tot_cruz = NULL
  # cong.2016.l$votes_tot_trump = NULL
  # cong.2016.l$votes_tot_kasich = NULL
  
  colnames(cong.2016.l)[grep("votes_tot_other",colnames(cong.2016.l))] = "votes_tot"
  
  # rm(cong.2016.cast,cong.2016.melt,
  # cong.2016.mun.group ,
  # cong.2016, cong.2016.look, cong.2016.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2016 gab stats
  gabdata.2016.cong = read.csv(
    "2016 Presidential and General Election EL-190 2017-10-05.csv",
    stringsAsFactors = F,
    header = T,
    strip.white = T
  ...
  # names(gabdata.2016.cong)
  
  # Preparing basic data
  gabdata.2016.cong = clean_names(gabdata.2016.cong)
  ...
  # gabdata.2016.cong.g <- group_by(gabdata.2016.cong, hindi, county, municipality, reporting_unit)
  # gabdata.2016.cong.m <- dplyr::summarise_if(gabdata.2016.cong.g, is.numeric, sum, na.rm = T)
  
  
  
  gabdata.2016.cong$reporting_unit_orig = gabdata.2016.cong$reporting_unit
  # gabdata.2016.cong$reporting_unit = toupper(gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$municipality = toupper(gabdata.2016.cong$municipality)
  gabdata.2016.cong$county = toupper(gabdata.2016.cong$county)
  
  gabdata.2016.cong$county = gsub(" COUNTY","", gabdata.2016.cong$county)
  gabdata.2016.cong$muni_county = paste(gabdata.2016.cong$municipality, gabdata.2016.cong$county)
  
  gabdata.2016.cong$reporting_unit = paste(gabdata.2016.cong$municipality,gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = toupper(gabdata.2016.cong$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2016.cong$municipality[grep("TOWN OF WINDSOR", gabdata.2016.cong$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2016.cong$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2016.cong$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2016.cong$county[grep("TOWN OF MAINE", gabdata.2016.cong$municipality)] = "MARATHON"
  gabdata.2016.cong$municipality[grepl("TOWN OF MAINE", gabdata.2016.cong$municipality) &
                                  grepl("MARATHON", gabdata.2016.cong$county)
                                ] = "VILLAGE OF MAINE"
  # gabdata.2016.cong$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2016.cong$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2016.cong$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2016.cong$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2016.cong$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2016.cong$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2016.cong$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2016.cong$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2016.cong$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2016.cong$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2016.cong$muni_county = paste(gabdata.2016.cong$municipality, gabdata.2016.cong$county)
  
  gabdata.2016.cong$reporting_unit = paste(gabdata.2016.cong$reporting_unit, gabdata.2016.cong$county)
  
  gabdata.2016.cong$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2016.cong$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2016.cong$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2016.cong$reporting_unit)
  # gabdata.2016.cong$reporting_unit = gsub("WARDS","WARD",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("  "," ",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("&","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("\\bAND\\b","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub(",","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub(" - ","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub(" -","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("- ","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub(" ","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("WD","",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("COMBINED","",gabdata.2016.cong$reporting_unit)
  
  gabdata.2016.cong$reporting_unit = gsub(" ","-",gabdata.2016.cong$reporting_unit)
  gabdata.2016.cong$reporting_unit = gsub("--","-",gabdata.2016.cong$reporting_unit)
  
  gabdata.repunit <- gabdata.2016.cong
  # 
  # ward.gabdata.group = dplyr::group_by(gabdata.2016.cong,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2016.cong.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(cong.2016.l$muni_county))
  # setdiff(unique(cong.2016.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # cong.2016.l$muni_county[grep("VILLAGE OF MAINE MARATHON",cong.2016.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # cong.2016.l$municipality[grep("TOWN OF MAINE MARATHON",cong.2016.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(cong.2016.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(cong.2016.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  table(cong.2016.l$reporting_unit %in% gabdata.join$reporting_unit)
  which(cong.2016.l$reporting_unit %!in% gabdata.join$reporting_unit)
  ...
  cong.2016.new = plyr::join(cong.2016.l, gabdata.join, by = "reporting_unit")
  cong.2016.l = cong.2016.new
  
  cong.2016.l[is.na(cong.2016.l)] = NA
  rm(cong.2016.new)
  
  setdiff(cong.2016.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  cong.2016.dem = plyr::join(cong.2016.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  cong.2016.dem = plyr::join(cong.2016.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",cong.2016.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, cong.2016.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(cong.2016.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(cong.2016.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  cong.2016.dem = plyr::join(cong.2016.dem, off_vote_machines, by = "muni_county")
  
  cong.2016.dem$tch_model_off_full = NA
  cong.2016.dem$tch_model_off_full = with(cong.2016.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", cong.2016.dem$tch_model_off_full))
  cong.2016.dem$tch_model_off_full = with(cong.2016.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",cong.2016.dem$tch_model_off_full))
  cong.2016.dem$tch_model_off_full = with(cong.2016.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",cong.2016.dem$tch_model_off_full))
  cong.2016.dem$tch_model_off_full = with(cong.2016.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",cong.2016.dem$tch_model_off_full))
  cong.2016.dem$tch_model_off_full[is.na(cong.2016.dem$tch_model_off_full)] = "aNone"
  cong.2016.dem$tch_model_off_full = with(cong.2016.dem, ifelse(is.na(tch_use_off),
                                                              NA,cong.2016.dem$tch_model_off_full))
  
  colnames(cong.2016.dem)[grep("tch.use",colnames(cong.2016.dem),fixed = T)] = "tch_use_verivote"
  colnames(cong.2016.dem)[grep("os.use",colnames(cong.2016.dem),fixed = T)] = "os_use_verivote"
  colnames(cong.2016.dem)[grep("bmd.use",colnames(cong.2016.dem),fixed = T)] = "bmd_use_verivote"
  colnames(cong.2016.dem)[grep("X._collegedegreepersons25._2013",colnames(cong.2016.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2016.cong.repunit.df = cong.2016.dem
  # model.2016.cong.df = subset(model.2016.cong.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2016.cong.repunit.df$log.pop.sq.mile.2010 = log(model.2016.cong.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2016.cong.repunit.df))
  
  model.2016.cong.repunit.df2 = with(model.2016.cong.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2016.cong.repunit.df2) = model.2016.cong.repunit.df2$reporting_unit
  
  model.2016.cong.repunit.df = model.2016.cong.repunit.df2
  
  # model.2016.cong.repunit.df$MCD_FIPS = as.character(model.2016.cong.repunit.df$MCD_FIPS)
  ...
  model.2016.cong.repunit.df = plyr::join(model.2016.cong.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.spat.o <-
  #   readOGR(
  #     dsn = "Prepped files/wisc.congent.orig.geojson",
  #     layer = "OGRGeoJSON",
  #     disambiguateFIDs = T,
  #     stringsAsFactors = F
  #   )
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2016.cong.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.congent.rep2016.cong.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2016.cong.df,"model_2016_cong.csv")
write.csv(model.2016.cong.repunit.df,"Prepped files/2016/model_2016_cong_repunit.csv")

# Spring 2016 Republican primary data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files/Spring primary")
getwd()
...
# ADD IN 2016 SENATOR HERE
# ADD IN 2016 CONGRESS HERE
# Governor 2014 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
getwd()
...
{ # Making model df
 ...
 model.2014.gov.repunit.df2 = with(model.2014.gov.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                 sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                 county_paper_or_paperplusmachine,
                                 ratio_nocollege_college, log.pop.sq.mile.2010,
                                 pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
...
{ # Making model df
 ...
 model.2014.cong.repunit.df2 = with(model.2014.cong.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                     sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                     county_paper_or_paperplusmachine,
                                     ratio_nocollege_college, log.pop.sq.mile.2010,
                                     pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
...
# ADD IN 2014 SEC STATE HERE
# Presidential 2012 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
getwd()
...
# Replace blanks with NA
pres.2012 = data.frame(apply(pres.2012, 2, function(x) gsub("^$|^ $", NA, x)))
pres.2012[,7:length(pres.2012)] = apply(pres.2012[,7:length(pres.2012)], 2, function(x) as.character(x))
pres.2012[,7:length(pres.2012)] = apply(pres.2012[,7:length(pres.2012)], 2, function(x) as.numeric(x))
pres.2012 = clean_names(pres.2012)

# str(pres.2012)
names(pres.2012)[3] <- "reporting_unit" 
# pres.2012.g <- group_by(pres.2012, reporting_unit, county_name, municipality_name)
# pres.2012 <- summarise_if(pres.2012.g, is.numeric, sum, na.rm = T)


{ # Preparing basic data
  names(pres.2012)[1] <- "county"
  pres.2012$votes.tot <- rowSums(pres.2012[,7:length(pres.2012)], na.rm = T)
  
  pres.2012$county[pres.2012$county == ""] <- NA
  
  require(zoo)
  pres.2012$county <- na.locf(pres.2012$county)
  
  pres.2012$reporting_unit = as.character(pres.2012$reporting_unit)
  pres.2012$reporting_unit_orig <- pres.2012$reporting_unit
  pres.2012$reporting_unit = ifelse(pres.2012$reporting_unit == "","Wards 1",pres.2012$reporting_unit)
  pres.2012$reporting_unit = toupper(pres.2012$reporting_unit)
  pres.2012$county = toupper(pres.2012$county)
  
  pres.2012 <- subset(pres.2012, reporting_unit != "COUNTY TOTALS:")
  pres.2012 <- subset(pres.2012, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  pres.2012 <- as.data.frame(lapply(pres.2012, function(y) (gsub(",", "", y))), stringsAsFactors = F)
  
  # Get the municipality names from the reporting unit names    
  # odd_indexes<-seq(1,length(pres.2012$reporting_unit)*2,2)
  
  str(pres.2012$reporting_unit)
  
  require(stringr)
  pres.2012$reporting_unit[which(grepl("[0-9]", pres.2012$reporting_unit) == F)] <- 
    paste(pres.2012$reporting_unit[which(grepl("[0-9]", pres.2012$reporting_unit) == F)], "1", sep = " ")
  
  pres.2012$reporting_unit <- gsub("WARD WD", "WARD", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("\\bWD\\b", "WARD", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("\\bWDS\\b", "WARDS", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("\\bWARD\\b", "WARD", pres.2012$reporting_unit)
  
  pres.2012$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", pres.2012$reporting_unit)
  
  
  # pres.2012$municipality_name <- unlist(str_split(str = pres.2012$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  pres.2012$municipality_name <- gsub(" WARDS","", pres.2012$municipality_name)
  pres.2012$municipality_name <- gsub(" WARD","", pres.2012$municipality_name)
  pres.2012$municipality_name <- na.locf(pres.2012$municipality_name)
  pres.2012$municipality_orig = pres.2012$municipality_name
  
  
  # pres.2012$municipality_name = with(pres.2012, paste(municipality_type, "of", municipality,sep=" "))
  # pres.2012$reporting_unit = with(pres.2012, paste(municipality_name, reporting_unit))
  # colnames(pres.2012)[1:3] = c("county","municipality_name","reporting_unit")
  # pres.2012$municipality_name = toupper(pres.2012$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  pres.2012$municipality_name[grep("TOWN OF WINDSOR", pres.2012$municipality_name)] = "VILLAGE OF WINDSOR"
  pres.2012$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", pres.2012$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # pres.2012$county[grep("TOWN OF MAINE", pres.2012$municipality_name)] = "MARATHON"
  pres.2012$municipality_name[grepl("TOWN OF MAINE", pres.2012$municipality_name) &
                                grepl("MARATHON", pres.2012$county)
                              ] = "VILLAGE OF MAINE"
  # pres.2012$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", pres.2012$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  pres.2012$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", pres.2012$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  pres.2012$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", pres.2012$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  pres.2012$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", pres.2012$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  pres.2012$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", pres.2012$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  
  
  # pres.2012.look = with(pres.2012, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(pres.2012.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # pres.2012$congress_district = na.locf(pres.2012$congress_district)
  pres.2012$county = na.locf(pres.2012$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  pres.2012$county = trim.trailing(pres.2012$county)
  
  pres.2012$municipality_name = toupper(pres.2012$municipality_name)
  pres.2012$county = toupper(pres.2012$county)
  
  pres.2012$muni_county = paste(pres.2012$municipality_name, pres.2012$county)
  pres.2012$muni_county_orig = paste(pres.2012$municipality_orig, pres.2012$county)
  ...
  pres.2012$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", pres.2012$reporting_unit)
  pres.2012$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", pres.2012$reporting_unit)
  # pres.2012$reporting_unit = gsub("WARDS","WARD",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("  "," ",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("&","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("\\bAND\\b","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub(",","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub(" - ","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub(" -","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("- ","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub(" ","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("WD","",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("COMBINED","",pres.2012$reporting_unit)
  
  pres.2012$reporting_unit = toupper(pres.2012$reporting_unit)
  pres.2012$reporting_unit = paste(pres.2012$reporting_unit, pres.2012$county, sep = "-")
  
  pres.2012$reporting_unit = gsub(" ","-",pres.2012$reporting_unit)
  pres.2012$reporting_unit = gsub("--","-",pres.2012$reporting_unit)
  
  pres.2012.look = with(pres.2012, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(pres.2012.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
 
  
  pres.2012.v = pres.2012
  # pres.2012.v$county = NULL
  pres.2012.v$county = NULL
  pres.2012.v$municipality_name = NULL
  pres.2012.v$municipality_type = NULL
  pres.2012.v$municipalityno = NULL
  pres.2012.v$municipality = NULL
  pres.2012.v$municipality_orig = NULL
  # pres.2012.v$reporting_unit = NULL
  pres.2012.v$order = NULL
  pres.2012.v$hindi = NULL
  # pres.2012.v$assemblydistrict = NULL
  # pres.2012.v = subset(  pres.2012.v, original_or_recount == "Recount")
  
  pres.2012.l <- pres.2012.v
  pres.2012.l$muni_county = NULL
  pres.2012.l$reporting_unit <- NULL
  pres.2012.l$muni_county_orig = NULL
  pres.2012.l$reporting_unit_orig <- NULL
  pres.2012.l$votes.tot <- NULL
  pres.2012.l$x_3 <- NULL
  pres.2012.l = gather(pres.2012.l, cand, votes_rec)
  pres.2012.l$reporting_unit <- rep(pres.2012.v$reporting_unit)
  # colnames(pres.2012.l) = c("reporting_unit", "cand", "votes.rec")
  
  # pres.2012.l$original_or_recount = NULL
  
  pres.2012.l$cand.group = NA
  pres.2012.l$cand.group[grep("obama",pres.2012.l$cand)] = "democrat"
  pres.2012.l$cand.group[grep("romney",pres.2012.l$cand)] = "republican"
  pres.2012.l$cand.group[grep("virgil",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("johnson",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("gloria",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("jerry",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("stein",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("rocky",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("roseanne",pres.2012.l$cand)] = "other"
  pres.2012.l$cand.group[grep("scatter",pres.2012.l$cand)] = "other"
  
  # Joining back county and municipality names
  pres.2012.l = plyr::join(pres.2012.l, pres.2012.look, by = "reporting_unit", match = "first")
  
  pres.2012.l$votes_rec = as.numeric(as.integer(pres.2012.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(pres.2012.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  pres.2012.l = plyr::join(pres.2012.l,tot.votes.look,by="reporting_unit",match="first")
  pres.2012.l$votes_perc = (pres.2012.l$votes_rec / pres.2012.l$votes_tot) * 100
  
  pres.2012.l$muni_county = paste(pres.2012.l$municipality, pres.2012.l$county)
  
  # pres.2012.l$votes_perc = (pres.2012.l$votes_rec / pres.2012.l$votes_tot) * 100
  
  pres.2012.l$votes_tot <- NULL
  pres.2012.melt = melt(pres.2012.l)
  pres.2012.cast = dcast(pres.2012.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  pres.2012.l = pres.2012.cast
  pres.2012.l[,grep("_NA", names(pres.2012.l))] <- NULL
  
  pres.2012.l = plyr::join(pres.2012.l,tot.votes.look,by="reporting_unit",match="first")
  
  # pres.2012.l$trump_prim_won = with(pres.2012.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # pres.2012.l$votes_tot_cruz = NULL
  # pres.2012.l$votes_tot_trump = NULL
  # pres.2012.l$votes_tot_kasich = NULL
  
  colnames(pres.2012.l)[grep("votes_tot_other",colnames(pres.2012.l))] = "votes_tot"
  
  # rm(pres.2012.cast,pres.2012.melt,
  # pres.2012.mun.group ,
  # pres.2012, pres.2012.look, pres.2012.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # GAB  / EDL stats
  gabdata.2012.pres = read.xlsx2(
    "20121106_gab190_statistics_xls_14031.xls",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
  ...
  # Preparing basic data
  gabdata.2012.pres = clean_names(gabdata.2012.pres)
  
  gabdata.2012.pres$reporting_unit_orig = gabdata.2012.pres$reporting_unit
  # gabdata.2012.pres$reporting_unit = toupper(gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$municipality = toupper(gabdata.2012.pres$municipality)
  gabdata.2012.pres$county = toupper(gabdata.2012.pres$county)
  
  gabdata.2012.pres$county = gsub(" COUNTY","", gabdata.2012.pres$county)
  gabdata.2012.pres$muni_county = paste(gabdata.2012.pres$municipality, gabdata.2012.pres$county)
  
  gabdata.2012.pres$reporting_unit = paste(gabdata.2012.pres$municipality,gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = toupper(gabdata.2012.pres$reporting_unit)
  
  gabdata.2012.pres$reporting_unit[which(grepl("[0-9]", gabdata.2012.pres$reporting_unit) == F)] <- 
    paste(gabdata.2012.pres$reporting_unit[which(grepl("[0-9]", gabdata.2012.pres$reporting_unit) == F)], "1", sep = " ")
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2012.pres$municipality[grep("TOWN OF WINDSOR", gabdata.2012.pres$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2012.pres$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2012.pres$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2012.pres$county[grep("TOWN OF MAINE", gabdata.2012.pres$municipality)] = "MARATHON"
  gabdata.2012.pres$municipality[grepl("TOWN OF MAINE", gabdata.2012.pres$municipality) &
                                   grepl("MARATHON", gabdata.2012.pres$county)
                                 ] = "VILLAGE OF MAINE"
  # gabdata.2012.pres$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2012.pres$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2012.pres$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2012.pres$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2012.pres$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2012.pres$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2012.pres$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2012.pres$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2012.pres$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2012.pres$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2012.pres$muni_county = paste(gabdata.2012.pres$municipality, gabdata.2012.pres$county)
  
  gabdata.2012.pres$reporting_unit = paste(gabdata.2012.pres$reporting_unit, gabdata.2012.pres$county)
  
  gabdata.2012.pres$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2012.pres$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2012.pres$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2012.pres$reporting_unit)
  # gabdata.2012.pres$reporting_unit = gsub("WARDS","WARD",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub("  "," ",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub("&","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub("\\bAND\\b","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub(",","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub(" - ","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub(" -","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub("- ","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub(" ","-",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub("WD","",gabdata.2012.pres$reporting_unit)
  gabdata.2012.pres$reporting_unit = gsub("COMBINED","",gabdata.2012.pres$reporting_unit)
  ...
  # gabdata.2012.pres$reporting_unit[grepl("CITY-OF-MENASHA-WARDS-1A-2-4-7-WINNEBAGO", gabdata.2012.pres$reporting_unit)] = "TOWN-OF-MENASHA-WARDS-1-2-4-7-WINNEBAGO"
  # gabdata.2012.pres$reporting_unit[grepl("CITY-OF-MENASHA-WARDS-3-5-6-WINNEBAGO", gabdata.2012.pres$reporting_unit)] = "TOWN-OF-MENASHA-WARDS-3-5-6-WINNEBAGO"
  
  gabdata.repunit <- gabdata.2012.pres
...
{ # Matching voting statistics with the election statistics
  ...
  # table(pres.2012.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  pres.2012.new = plyr::join(pres.2012.l, gabdata.join, by = "reporting_unit")
  pres.2012.l = pres.2012.new
  
  pres.2012.l[is.na(pres.2012.l)] = NA
  rm(pres.2012.new)
  
  setdiff(pres.2012.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  pres.2012.dem = plyr::join(pres.2012.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  pres.2012.dem = plyr::join(pres.2012.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",pres.2012.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, pres.2012.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(pres.2012.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(pres.2012.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  pres.2012.dem = plyr::join(pres.2012.dem, off_vote_machines, by = "muni_county")
  
  pres.2012.dem$tch_model_off_full = NA
  pres.2012.dem$tch_model_off_full = with(pres.2012.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                                "Dominion (Premier)-Accuvote TSX", pres.2012.dem$tch_model_off_full))
  pres.2012.dem$tch_model_off_full = with(pres.2012.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                                "Dominion (Sequoia)/Command Central-Edge",pres.2012.dem$tch_model_off_full))
  pres.2012.dem$tch_model_off_full = with(pres.2012.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                                "Dominion (Sequoia)/Command Central-Edge",pres.2012.dem$tch_model_off_full))
  pres.2012.dem$tch_model_off_full = with(pres.2012.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                                "ES&S iVotronic",pres.2012.dem$tch_model_off_full))
  pres.2012.dem$tch_model_off_full[is.na(pres.2012.dem$tch_model_off_full)] = "aNone"
  pres.2012.dem$tch_model_off_full = with(pres.2012.dem, ifelse(is.na(tch_use_off),
                                                                NA,pres.2012.dem$tch_model_off_full))
  
  colnames(pres.2012.dem)[grep("tch.use",colnames(pres.2012.dem),fixed = T)] = "tch_use_verivote"
  colnames(pres.2012.dem)[grep("os.use",colnames(pres.2012.dem),fixed = T)] = "os_use_verivote"
  colnames(pres.2012.dem)[grep("bmd.use",colnames(pres.2012.dem),fixed = T)] = "bmd_use_verivote"
  colnames(pres.2012.dem)[grep("X._collegedegreepersons25._2013",colnames(pres.2012.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2012.pres.repunit.df = pres.2012.dem
  # model.2012.pres.df = subset(model.2012.pres.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2012.pres.repunit.df$log.pop.sq.mile.2010 = log(model.2012.pres.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2012.pres.repunit.df))
  
  model.2012.pres.repunit.df2 = with(model.2012.pres.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                            sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                            county_paper_or_paperplusmachine,
                                                                            ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                            pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2012.pres.repunit.df2) = model.2012.pres.repunit.df2$reporting_unit
  
  model.2012.pres.repunit.df = model.2012.pres.repunit.df2
  
  # model.2012.pres.repunit.df$MCD_FIPS = as.character(model.2012.pres.repunit.df$MCD_FIPS)
  ...
  model.2012.pres.repunit.df = plyr::join(model.2012.pres.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2012.pres.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2012.pres.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

write.csv(model.2012.pres.repunit.df,"Prepped files/2012/model_2012_pres_repunit.csv")


# ADD IN 2012 SENATOR HERE
# Senator 2012 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files/")
getwd()

sen.2012 = read_excel(
  "2012/Ward by Ward_11.6.12.xls", sheet = 3, skip = 9
)
# Remove columns of all NAs
sen.2012 <- sen.2012[,colSums(is.na(sen.2012))<nrow(sen.2012)]
# Remove first line of candidate names
sen.2012 <- sen.2012[-1,]

# Replace blanks with NA
sen.2012 = data.frame(apply(sen.2012, 2, function(x) gsub("^$|^ $", NA, x)))
sen.2012[,3:length(sen.2012)] = apply(sen.2012[,3:length(sen.2012)], 2, function(x) as.character(x))
sen.2012[,3:length(sen.2012)] = apply(sen.2012[,3:length(sen.2012)], 2, function(x) as.numeric(x))
sen.2012 = clean_names(sen.2012)

# str(sen.2012)
names(sen.2012)[1:3] <- c("county","reporting_unit","votes.tot")
# sen.2012.g <- group_by(sen.2012, reporting_unit, county_name, municipality_name)
# sen.2012 <- summarise_if(sen.2012.g, is.numeric, sum, na.rm = T)


{ # Preparing basic data
 
  sen.2012$county[sen.2012$county == ""] <- NA
  
  require(zoo)
  sen.2012$county <- na.locf(sen.2012$county)
  
  sen.2012$reporting_unit = as.character(sen.2012$reporting_unit)
  sen.2012$reporting_unit_orig <- sen.2012$reporting_unit
  sen.2012$reporting_unit = ifelse(sen.2012$reporting_unit == "","Wards 1",sen.2012$reporting_unit)
  sen.2012$reporting_unit = toupper(sen.2012$reporting_unit)
  sen.2012$county = toupper(sen.2012$county)
  
  sen.2012 <- subset(sen.2012, reporting_unit != "COUNTY TOTALS:")
  sen.2012 <- subset(sen.2012, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  sen.2012 <- as.data.frame(lapply(sen.2012, function(y) (gsub(",", "", y))), stringsAsFactors = F)
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(sen.2012$reporting_unit)*2,2)
  
  require(stringr)
  sen.2012$reporting_unit[which(grepl("[0-9]", sen.2012$reporting_unit) == F)] <- 
    paste(sen.2012$reporting_unit[which(grepl("[0-9]", sen.2012$reporting_unit) == F)], "1", sep = " ")
  
  sen.2012$reporting_unit <- gsub("WARD WD", "WARD", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("\\bWD\\b", "WARD", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("\\bWDS\\b", "WARDS", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("\\bWARD\\b", "WARD", sen.2012$reporting_unit)
  
  sen.2012$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", sen.2012$reporting_unit)
  
  
  sen.2012$municipality_name <- unlist(str_split(str = sen.2012$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  sen.2012$municipality_name <- gsub(" WARDS","", sen.2012$municipality_name)
  sen.2012$municipality_name <- gsub(" WARD","", sen.2012$municipality_name)
  sen.2012$municipality_name <- na.locf(sen.2012$municipality_name)
  sen.2012$municipality_orig = sen.2012$municipality_name
  
  
  # sen.2012$municipality_name = with(sen.2012, paste(municipality_type, "of", municipality,sep=" "))
  # sen.2012$reporting_unit = with(sen.2012, paste(municipality_name, reporting_unit))
  # colnames(sen.2012)[1:3] = c("county","municipality_name","reporting_unit")
  # sen.2012$municipality_name = toupper(sen.2012$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  sen.2012$municipality_name[grep("TOWN OF WINDSOR", sen.2012$municipality_name)] = "VILLAGE OF WINDSOR"
  sen.2012$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", sen.2012$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # sen.2012$county[grep("TOWN OF MAINE", sen.2012$municipality_name)] = "MARATHON"
  sen.2012$municipality_name[grepl("TOWN OF MAINE", sen.2012$municipality_name) &
                                grepl("MARATHON", sen.2012$county)
                              ] = "VILLAGE OF MAINE"
  # sen.2012$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", sen.2012$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  sen.2012$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2012$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  sen.2012$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2012$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  sen.2012$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2012$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  sen.2012$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2012$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  
  
  # sen.2012.look = with(sen.2012, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(sen.2012.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # sen.2012$congress_district = na.locf(sen.2012$congress_district)
  sen.2012$county = na.locf(sen.2012$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  sen.2012$county = trim.trailing(sen.2012$county)
  
  sen.2012$municipality_name = toupper(sen.2012$municipality_name)
  sen.2012$county = toupper(sen.2012$county)
  
  sen.2012$muni_county = paste(sen.2012$municipality_name, sen.2012$county)
  sen.2012$muni_county_orig = paste(sen.2012$municipality_orig, sen.2012$county)
  ...
  sen.2012$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", sen.2012$reporting_unit)
  sen.2012$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", sen.2012$reporting_unit)
  # sen.2012$reporting_unit = gsub("WARDS","WARD",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("  "," ",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("&","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("\\bAND\\b","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub(",","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub(" - ","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub(" -","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("- ","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub(" ","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("WD","",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("COMBINED","",sen.2012$reporting_unit)
  
  sen.2012$reporting_unit = toupper(sen.2012$reporting_unit)
  sen.2012$reporting_unit = paste(sen.2012$reporting_unit, sen.2012$county, sep = "-")
  
  sen.2012$reporting_unit = gsub(" ","-",sen.2012$reporting_unit)
  sen.2012$reporting_unit = gsub("--","-",sen.2012$reporting_unit)
  
  sen.2012.look = with(sen.2012, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(sen.2012.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  
  sen.2012.v = sen.2012
  # sen.2012.v$county = NULL
  sen.2012.v$county = NULL
  sen.2012.v$municipality_name = NULL
  sen.2012.v$municipality_type = NULL
  sen.2012.v$municipalityno = NULL
  sen.2012.v$municipality = NULL
  sen.2012.v$municipality_orig = NULL
  # sen.2012.v$reporting_unit = NULL
  sen.2012.v$order = NULL
  sen.2012.v$hindi = NULL
  # sen.2012.v$assemblydistrict = NULL
  # sen.2012.v = subset(  sen.2012.v, original_or_recount == "Recount")
  
  sen.2012.l <- sen.2012.v
  sen.2012.l$muni_county = NULL
  sen.2012.l$reporting_unit <- NULL
  sen.2012.l$muni_county_orig = NULL
  sen.2012.l$reporting_unit_orig <- NULL
  sen.2012.l$votes.tot <- NULL
  sen.2012.l$x_3 <- NULL
  sen.2012.l = gather(sen.2012.l, cand, votes_rec)
  sen.2012.l$reporting_unit <- rep(sen.2012.v$reporting_unit)
  # colnames(sen.2012.l) = c("reporting_unit", "cand", "votes.rec")
  
  # sen.2012.l$original_or_recount = NULL
  
  sen.2012.l$cand.group = NA
  sen.2012.l$cand.group[grep("dem",sen.2012.l$cand)] = "democrat"
  sen.2012.l$cand.group[grep("rep",sen.2012.l$cand)] = "republican"
  sen.2012.l$cand.group[grep("ind",sen.2012.l$cand)] = "other"
  sen.2012.l$cand.group[grep("con",sen.2012.l$cand)] = "other"
  sen.2012.l$cand.group[grep("x_5",sen.2012.l$cand)] = "other"
  
  # Joining back county and municipality names
  sen.2012.l = plyr::join(sen.2012.l, sen.2012.look, by = "reporting_unit", match = "first")
  
  sen.2012.l$votes_rec = as.numeric(as.integer(sen.2012.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(sen.2012.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  sen.2012.l = plyr::join(sen.2012.l,tot.votes.look,by="reporting_unit",match="first")
  sen.2012.l$votes_perc = (sen.2012.l$votes_rec / sen.2012.l$votes_tot) * 100
  
  sen.2012.l$muni_county = paste(sen.2012.l$municipality, sen.2012.l$county)
  
  # sen.2012.l$votes_perc = (sen.2012.l$votes_rec / sen.2012.l$votes_tot) * 100
  
  sen.2012.l$votes_tot <- NULL
  sen.2012.melt = melt(sen.2012.l)
  sen.2012.cast = dcast(sen.2012.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  sen.2012.l = sen.2012.cast
  sen.2012.l[,grep("_NA", names(sen.2012.l))] <- NULL
  
  sen.2012.l = plyr::join(sen.2012.l,tot.votes.look,by="reporting_unit",match="first")
  
  # sen.2012.l$trump_prim_won = with(sen.2012.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # sen.2012.l$votes_tot_cruz = NULL
  # sen.2012.l$votes_tot_trump = NULL
  # sen.2012.l$votes_tot_kasich = NULL
  
  colnames(sen.2012.l)[grep("votes_tot_other",colnames(sen.2012.l))] = "votes_tot"
  
  # rm(sen.2012.cast,sen.2012.melt,
  # sen.2012.mun.group ,
  # sen.2012, sen.2012.look, sen.2012.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # GAB  / EDL stats
  gabdata.2012.sen = read.xlsx2(
    "20121106_gab190_statistics_xls_14031.xls",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
  ...
  # Preparing basic data
  gabdata.2012.sen = clean_names(gabdata.2012.sen)
  
  gabdata.2012.sen$reporting_unit_orig = gabdata.2012.sen$reporting_unit
  # gabdata.2012.sen$reporting_unit = toupper(gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$municipality = toupper(gabdata.2012.sen$municipality)
  gabdata.2012.sen$county = toupper(gabdata.2012.sen$county)
  
  gabdata.2012.sen$county = gsub(" COUNTY","", gabdata.2012.sen$county)
  gabdata.2012.sen$muni_county = paste(gabdata.2012.sen$municipality, gabdata.2012.sen$county)
  
  gabdata.2012.sen$reporting_unit = paste(gabdata.2012.sen$municipality,gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = toupper(gabdata.2012.sen$reporting_unit)
  
  gabdata.2012.sen$reporting_unit[which(grepl("[0-9]", gabdata.2012.sen$reporting_unit) == F)] <- 
    paste(gabdata.2012.sen$reporting_unit[which(grepl("[0-9]", gabdata.2012.sen$reporting_unit) == F)], "1", sep = " ")
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2012.sen$municipality[grep("TOWN OF WINDSOR", gabdata.2012.sen$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2012.sen$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2012.sen$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2012.sen$county[grep("TOWN OF MAINE", gabdata.2012.sen$municipality)] = "MARATHON"
  gabdata.2012.sen$municipality[grepl("TOWN OF MAINE", gabdata.2012.sen$municipality) &
                                   grepl("MARATHON", gabdata.2012.sen$county)
                                 ] = "VILLAGE OF MAINE"
  # gabdata.2012.sen$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2012.sen$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2012.sen$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2012.sen$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2012.sen$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2012.sen$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2012.sen$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2012.sen$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2012.sen$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2012.sen$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2012.sen$muni_county = paste(gabdata.2012.sen$municipality, gabdata.2012.sen$county)
  
  gabdata.2012.sen$reporting_unit = paste(gabdata.2012.sen$reporting_unit, gabdata.2012.sen$county)
  
  gabdata.2012.sen$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2012.sen$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2012.sen$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2012.sen$reporting_unit)
  # gabdata.2012.sen$reporting_unit = gsub("WARDS","WARD",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub("  "," ",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub("&","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub("\\bAND\\b","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub(",","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub(" - ","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub(" -","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub("- ","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub(" ","-",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub("WD","",gabdata.2012.sen$reporting_unit)
  gabdata.2012.sen$reporting_unit = gsub("COMBINED","",gabdata.2012.sen$reporting_unit)
  ...
  # gabdata.2012.sen$reporting_unit[grepl("CITY-OF-MENASHA-WARDS-1A-2-4-7-WINNEBAGO", gabdata.2012.sen$reporting_unit)] = "TOWN-OF-MENASHA-WARDS-1-2-4-7-WINNEBAGO"
  # gabdata.2012.sen$reporting_unit[grepl("CITY-OF-MENASHA-WARDS-3-5-6-WINNEBAGO", gabdata.2012.sen$reporting_unit)] = "TOWN-OF-MENASHA-WARDS-3-5-6-WINNEBAGO"
  
  gabdata.repunit <- gabdata.2012.sen
...
{ # Matching voting statistics with the election statistics
  ...
  # table(sen.2012.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  sen.2012.new = plyr::join(sen.2012.l, gabdata.join, by = "reporting_unit")
  sen.2012.l = sen.2012.new
  
  sen.2012.l[is.na(sen.2012.l)] = NA
  rm(sen.2012.new)
  
  setdiff(sen.2012.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  sen.2012.dem = plyr::join(sen.2012.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  sen.2012.dem = plyr::join(sen.2012.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",sen.2012.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, sen.2012.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(sen.2012.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(sen.2012.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  sen.2012.dem = plyr::join(sen.2012.dem, off_vote_machines, by = "muni_county")
  
  sen.2012.dem$tch_model_off_full = NA
  sen.2012.dem$tch_model_off_full = with(sen.2012.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                                "Dominion (Premier)-Accuvote TSX", sen.2012.dem$tch_model_off_full))
  sen.2012.dem$tch_model_off_full = with(sen.2012.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                                "Dominion (Sequoia)/Command Central-Edge",sen.2012.dem$tch_model_off_full))
  sen.2012.dem$tch_model_off_full = with(sen.2012.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                                "Dominion (Sequoia)/Command Central-Edge",sen.2012.dem$tch_model_off_full))
  sen.2012.dem$tch_model_off_full = with(sen.2012.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                                "ES&S iVotronic",sen.2012.dem$tch_model_off_full))
  sen.2012.dem$tch_model_off_full[is.na(sen.2012.dem$tch_model_off_full)] = "aNone"
  sen.2012.dem$tch_model_off_full = with(sen.2012.dem, ifelse(is.na(tch_use_off),
                                                                NA,sen.2012.dem$tch_model_off_full))
  
  colnames(sen.2012.dem)[grep("tch.use",colnames(sen.2012.dem),fixed = T)] = "tch_use_verivote"
  colnames(sen.2012.dem)[grep("os.use",colnames(sen.2012.dem),fixed = T)] = "os_use_verivote"
  colnames(sen.2012.dem)[grep("bmd.use",colnames(sen.2012.dem),fixed = T)] = "bmd_use_verivote"
  colnames(sen.2012.dem)[grep("X._collegedegreepersons25._2013",colnames(sen.2012.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2012.sen.repunit.df = sen.2012.dem
  # model.2012.sen.df = subset(model.2012.sen.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2012.sen.repunit.df$log.pop.sq.mile.2010 = log(model.2012.sen.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2012.sen.repunit.df))
  
  model.2012.sen.repunit.df2 = with(model.2012.sen.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                            sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                            county_paper_or_paperplusmachine,
                                                                            ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                            pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2012.sen.repunit.df2) = model.2012.sen.repunit.df2$reporting_unit
  
  model.2012.sen.repunit.df = model.2012.sen.repunit.df2
  
  # model.2012.sen.repunit.df$MCD_FIPS = as.character(model.2012.sen.repunit.df$MCD_FIPS)
  ...
  model.2012.sen.repunit.df = plyr::join(model.2012.sen.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2012.sen.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.senent.rep2012.sen.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

write.csv(model.2012.sen.repunit.df,"Prepped files/2012/model_2012_sen_repunit.csv")


# Congressional 2012 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files/")
getwd()

excel_sheets("2012/Ward by Ward_11.6.12.xls")
...
read_excel_allsheets <- function(filename, tibble = FALSE) {
  ...
  # then just pass tibble = TRUE
  sheets <- readxl::excel_sheets(filename)
  x <- lapply(sheets[1:20], function(X) readxl::read_excel(filename, sheet = X, skip = 8,
                                                     n_max = 760,
                                                     trim_ws = T))
  if(!tibble) x <- lapply(x, as.data.frame)
  names(x) <- sheets[1:20]
...
mysheets <- read_excel_allsheets("2012/Ward by Ward_11.6.12.xls")
...
for (i in 4:11) {
  ...
  sheet_w_no <- paste("Sheet", i-1, ".", sep = "")
  names(sheet) <- gsub(sheet_w_no, "", names(sheet))
  sheet <- clean_names(sheet)
...
{ # Making model df
  ...
  model.2012.cong.repunit.df2 = with(model.2012.cong.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2012.cong.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.congent.rep2012.cong.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec
...
gov.2010 = read_excel(
  "2010/2010 Fall General - All Races - Ward-by-Ward_0.xls",
  skip = 10, sheet = "Sheet1",
  trim_ws = T
)

# 
gov.2010 = data.frame(apply(gov.2010, 2, function(x) gsub("^$|^ $", NA, x)))
gov.2010 = clean_names(gov.2010)


{ # Preparing basic data
  names(gov.2010)[1:3] <- c("county","reporting_unit","votes.tot")
  
  gov.2010$county[gov.2010$county == ""] <- NA
  
  require(zoo)
  gov.2010$county <- na.locf(gov.2010$county)
  
  gov.2010$reporting_unit = as.character(gov.2010$reporting_unit)
  gov.2010$reporting_unit_orig <- gov.2010$reporting_unit
  gov.2010$reporting_unit = ifelse(gov.2010$reporting_unit == "","Wards 1",gov.2010$reporting_unit)
  gov.2010$reporting_unit = toupper(gov.2010$reporting_unit)
  gov.2010$county = toupper(gov.2010$county)
  
  gov.2010 <- subset(gov.2010, reporting_unit != "COUNTY TOTALS:")
  gov.2010 <- subset(gov.2010, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  gov.2010 <- as.data.frame(lapply(gov.2010, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(gov.2010$reporting_unit)*2,2)
  
  require(stringr)
  gov.2010$reporting_unit[which(grepl("[0-9]", gov.2010$reporting_unit) == F)] <- paste(gov.2010$reporting_unit[which(grepl("[0-9]", gov.2010$reporting_unit) == F)], "1", sep = " ")
  
  gov.2010$reporting_unit <- gsub("WARD WD", "WARD", gov.2010$reporting_unit)
  gov.2010$reporting_unit <- gsub("\\bWD\\b", "WARD", gov.2010$reporting_unit)
  gov.2010$reporting_unit <- gsub("\\bWDS\\b", "WARD", gov.2010$reporting_unit)
  gov.2010$reporting_unit <- gsub("\\bWARD\\b", "WARD", gov.2010$reporting_unit)
  
  gov.2010$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", gov.2010$reporting_unit)
  gov.2010$reporting_unit <- gsub("WARDS([0-9])", "WARD \\1", gov.2010$reporting_unit)
  
  
  gov.2010$municipality_name <- unlist(str_split(str = gov.2010$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  gov.2010$municipality_name <- gsub(" WARDS","", gov.2010$municipality_name)
  gov.2010$municipality_name <- gsub(" WARD","", gov.2010$municipality_name)
  gov.2010$municipality_name <- na.locf(gov.2010$municipality_name)
  gov.2010$municipality_orig = gov.2010$municipality_name
  
  
  # gov.2010$municipality_name = with(gov.2010, paste(municipality_type, "of", municipality,sep=" "))
  # gov.2010$reporting_unit = with(gov.2010, paste(municipality_name, reporting_unit))
  # colnames(gov.2010)[1:3] = c("county","municipality_name","reporting_unit")
  # gov.2010$municipality_name = toupper(gov.2010$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gov.2010$municipality_name[grep("TOWN OF WINDSOR", gov.2010$municipality_name)] = "VILLAGE OF WINDSOR"
  gov.2010$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gov.2010$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gov.2010$county[grep("TOWN OF MAINE", gov.2010$municipality_name)] = "MARATHON"
  gov.2010$municipality_name[grepl("TOWN OF MAINE", gov.2010$municipality_name) &
                               grepl("MARATHON", gov.2010$county)
                             ] = "VILLAGE OF MAINE"
  # gov.2010$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gov.2010$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gov.2010$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gov.2010$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  gov.2010$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gov.2010$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  gov.2010$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gov.2010$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gov.2010$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gov.2010$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # gov.2010.look = with(gov.2010, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(gov.2010.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # gov.2010$congress_district = na.locf(gov.2010$congress_district)
  gov.2010$county = na.locf(gov.2010$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  gov.2010$county = trim.trailing(gov.2010$county)
  
  gov.2010$municipality_name = toupper(gov.2010$municipality_name)
  gov.2010$county = toupper(gov.2010$county)
  
  gov.2010$muni_county = paste(gov.2010$municipality_name, gov.2010$county)
  gov.2010$muni_county_orig = paste(gov.2010$municipality_orig, gov.2010$county)
  ...
  gov.2010$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gov.2010$reporting_unit)
  gov.2010$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("WARDS","WARD",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("  "," ",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("&","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("\\bAND\\b","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub(",","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub(" - ","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub(" -","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("- ","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub(" ","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("WD","",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("COMBINED","",gov.2010$reporting_unit)
  
  gov.2010$reporting_unit = toupper(gov.2010$reporting_unit)
  gov.2010$reporting_unit = paste(gov.2010$reporting_unit, gov.2010$county, sep = "-")
  
  gov.2010$reporting_unit = gsub(" ","-",gov.2010$reporting_unit)
  gov.2010$reporting_unit = gsub("--","-",gov.2010$reporting_unit)
  
  gov.2010.look = with(gov.2010, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(gov.2010.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(gov.20102,gov.20103,gov.20104)
  
  gov.2010.v = gov.2010
  # gov.2010.v$county = NULL
  gov.2010.v$county = NULL
  gov.2010.v$municipality_name = NULL
  gov.2010.v$municipality_type = NULL
  gov.2010.v$municipalityno = NULL
  gov.2010.v$municipality = NULL
  gov.2010.v$municipality_orig = NULL
  # gov.2010.v$reporting_unit = NULL
  gov.2010.v$order = NULL
  gov.2010.v$hindi = NULL
  # gov.2010.v$assemblydistrict = NULL
  
  gov.2010.l <- gov.2010.v
  gov.2010.l$muni_county = NULL
  gov.2010.l$reporting_unit <- NULL
  gov.2010.l$muni_county_orig = NULL
  gov.2010.l$reporting_unit_orig <- NULL
  gov.2010.l$votes.tot <- NULL
  gov.2010.l$x_3 <- NULL
  gov.2010.l = gather(gov.2010.l, cand, votes_rec)
  gov.2010.l$reporting_unit <- rep(gov.2010.v$reporting_unit)
  # colnames(gov.2010.l) = c("reporting_unit", "cand", "votes.rec")
  
  # names(gov.2010.v)
  
  gov.2010.l$cand.group = NA
  gov.2010.l$cand.group[grep("tom_barrett",gov.2010.l$cand)] = "democrat"
  gov.2010.l$cand.group[grep("walker",gov.2010.l$cand)] = "republican"
  gov.2010.l$cand.group[grep("no_candidate",gov.2010.l$cand)] = "other"
  gov.2010.l$cand.group[grep("write_in",gov.2010.l$cand)] = "other"
  gov.2010.l$cand.group[grep("scatter",gov.2010.l$cand)] = "other"
  
  # Joining back county and municipality names
  gov.2010.l = plyr::join(gov.2010.l, gov.2010.look, by = "reporting_unit", match = "first")
  
  gov.2010.l$votes_rec = as.numeric(as.integer(gov.2010.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(gov.2010.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  gov.2010.l = plyr::join(gov.2010.l,tot.votes.look,by="reporting_unit",match="first")
  gov.2010.l$votes_perc = (gov.2010.l$votes_rec / gov.2010.l$votes_tot) * 100
  
  gov.2010.l$muni_county = paste(gov.2010.l$municipality, gov.2010.l$county)
  
  # gov.2010.l$votes_perc = (gov.2010.l$votes_rec / gov.2010.l$votes_tot) * 100
  
  gov.2010.l$votes_tot <- NULL
  gov.2010.melt = melt(gov.2010.l)
  gov.2010.cast = dcast(gov.2010.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  gov.2010.l = gov.2010.cast
  gov.2010.l[,grep("_NA", names(gov.2010.l))] <- NULL
  
  gov.2010.l = plyr::join(gov.2010.l,tot.votes.look,by="reporting_unit",match="first")
  
  # gov.2010.l$trump_prim_won = with(gov.2010.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # gov.2010.l$votes_tot_cruz = NULL
  # gov.2010.l$votes_tot_trump = NULL
  # gov.2010.l$votes_tot_kasich = NULL
  
  colnames(gov.2010.l)[grep("votes_tot_other",colnames(gov.2010.l))] = "votes_tot"
  
  # rm(gov.2010.cast,gov.2010.melt,
  # gov.2010.mun.group ,
  # gov.2010, gov.2010.look, gov.2010.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2010 gab stats
  gabdata.2010.gov = read.xlsx2(
    "2010/20101102_general_gab190_xls_40096.xls",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
    colClasses = NA # Thanks: http://www.briandalessandro.com/blog/how-to-read-numeric-columns-with-read-xlsx2-in-r/
  ...
  # Preparing basic data
  gabdata.2010.gov = clean_names(gabdata.2010.gov)
  gabdata.2010.gov$reporting_unit_orig = gabdata.2010.gov$reporting_unit
  # gabdata.2010.gov$reporting_unit = toupper(gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$municipality = toupper(gabdata.2010.gov$municipality)
  gabdata.2010.gov$county = toupper(gabdata.2010.gov$county)
  
  gabdata.2010.gov$county = gsub(" COUNTY","", gabdata.2010.gov$county)
  gabdata.2010.gov$muni_county = paste(gabdata.2010.gov$municipality, gabdata.2010.gov$county)
  
  gabdata.2010.gov$reporting_unit = paste(gabdata.2010.gov$municipality,gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = toupper(gabdata.2010.gov$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2010.gov$municipality[grep("TOWN OF WINDSOR", gabdata.2010.gov$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2010.gov$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2010.gov$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2010.gov$county[grep("TOWN OF MAINE", gabdata.2010.gov$municipality)] = "MARATHON"
  gabdata.2010.gov$municipality[grepl("TOWN OF MAINE", gabdata.2010.gov$municipality) &
                                  grepl("MARATHON", gabdata.2010.gov$county)
                                ] = "VILLAGE OF MAINE"
  # gabdata.2010.gov$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2010.gov$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2010.gov$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2010.gov$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2010.gov$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2010.gov$reporting_unit)
                                ] = "VILLAGE OF FOX CROSSING"
  gabdata.2010.gov$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2010.gov$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2010.gov$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2010.gov$reporting_unit)
                                  ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2010.gov$muni_county = paste(gabdata.2010.gov$municipality, gabdata.2010.gov$county)
  
  gabdata.2010.gov$reporting_unit = paste(gabdata.2010.gov$reporting_unit, gabdata.2010.gov$county)
  
  gabdata.2010.gov$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit <- gsub("\\bWDS\\b", "WARD", gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2010.gov$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2010.gov$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("WARDS","WARD",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("  "," ",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("&","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("\\bAND\\b","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub(",","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub(" - ","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub(" -","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("- ","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub(" ","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("WD","",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("COMBINED","",gabdata.2010.gov$reporting_unit)
  
  gabdata.2010.gov$reporting_unit = gsub(" ","-",gabdata.2010.gov$reporting_unit)
  gabdata.2010.gov$reporting_unit = gsub("--","-",gabdata.2010.gov$reporting_unit)
  
  gabdata.repunit <- gabdata.2010.gov
  # 
  # ward.gabdata.group = dplyr::group_by(gabdata.2010.gov,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       ed_rs = sum(ed_rs),
  #                                       total_ballots = sum(total_ballots),
  #                                       total_electors = sum(total_electors),
  #                                       paper_ballots = sum(paper_ballots),
  #                                       optical_scan_ballots = sum(optical_scan_ballots),
  #                                       dre_touchscreen_auto_mark = sum(dre_touchscreen_auto_mark),
  #                                       auto_mark = sum(auto_mark),
  #                                       total_election_inspectors = sum(total_election_inspectors),
  #                                       x16_17 = sum(x16_17),
  #                                       x18_25 = sum(x18_25),
  ...
  #                                       received_by_election_day = sum(received_by_election_day),
  #                                       counted_absent = sum(absentee_counted),
  #                                       rejected_absent = sum(absentee_rejected_late),
  #                                        # late_received_after_the_election = sum(late_received_after_the_election),
  #                                       fwab_received = sum(fwab_received),
  #                                       fwab_counted = sum(fwab_counted),
  #                                       fwab_rejected = sum(fwab_rejected),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2010.gov.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(gov.2010.l$muni_county))
  # setdiff(unique(gov.2010.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # gov.2010.l$muni_county[grep("VILLAGE OF MAINE MARATHON",gov.2010.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # gov.2010.l$municipality[grep("TOWN OF MAINE MARATHON",gov.2010.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(gov.2010.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(gov.2010.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  
  # sort(names(gabdata.repunit))
  
  gabdata.join = with(gabdata.repunit, data.frame(reporting_unit,muni_county, hindi))
  gabdata.join$total_ballots = ifelse(gabdata.repunit$total_ballots == 0,
                                      gabdata.repunit$total_electors,
                                      gabdata.repunit$total_ballots)
  gabdata.join$tch_votes = with(gabdata.repunit, dre_touchscreen_auto_mark);   gabdata.join$tch_prop = with(gabdata.repunit, dre_touchscreen_auto_mark / total_ballots)
  gabdata.join$tch_prop = ifelse(gabdata.join$tch_prop>1, 1, gabdata.join$tch_prop)
  ...
  gabdata.join$counted_absent_prop[gabdata.join$counted_absent_prop == Inf] = NA
  gabdata.join$rejected_absent_prop = with(gabdata.repunit,
                                           absentee_rejected_late / total_ballots)
  gabdata.join$absentee_undeliverable = with(gabdata.repunit,
                                             absentee_undeliverable / total_ballots)
  gabdata.join$registrants = with(gabdata.repunit, registrants)
  gabdata.join$late_registrants = with(gabdata.repunit, late_registrants / total_ballots)
  ...
  table(gov.2010.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  gov.2010.new = plyr::join(gov.2010.l, gabdata.join, by = "reporting_unit")
  gov.2010.l = gov.2010.new
  
  gov.2010.l[is.na(gov.2010.l)] = NA
  rm(gov.2010.new)
  
  setdiff(gov.2010.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  gov.2010.dem = plyr::join(gov.2010.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  gov.2010.dem = plyr::join(gov.2010.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
  off_vote_machines = read_excel("voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",gov.2010.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, gov.2010.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(gov.2010.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(gov.2010.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  gov.2010.dem = plyr::join(gov.2010.dem, off_vote_machines, by = "muni_county")
  
  gov.2010.dem$tch_model_off_full = NA
  gov.2010.dem$tch_model_off_full = with(gov.2010.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", gov.2010.dem$tch_model_off_full))
  gov.2010.dem$tch_model_off_full = with(gov.2010.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",gov.2010.dem$tch_model_off_full))
  gov.2010.dem$tch_model_off_full = with(gov.2010.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",gov.2010.dem$tch_model_off_full))
  gov.2010.dem$tch_model_off_full = with(gov.2010.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",gov.2010.dem$tch_model_off_full))
  gov.2010.dem$tch_model_off_full[is.na(gov.2010.dem$tch_model_off_full)] = "aNone"
  gov.2010.dem$tch_model_off_full = with(gov.2010.dem, ifelse(is.na(tch_use_off),
                                                              NA,gov.2010.dem$tch_model_off_full))
  
  colnames(gov.2010.dem)[grep("tch.use",colnames(gov.2010.dem),fixed = T)] = "tch_use_verivote"
  colnames(gov.2010.dem)[grep("os.use",colnames(gov.2010.dem),fixed = T)] = "os_use_verivote"
  colnames(gov.2010.dem)[grep("bmd.use",colnames(gov.2010.dem),fixed = T)] = "bmd_use_verivote"
  colnames(gov.2010.dem)[grep("X._collegedegreepersons25._2013",colnames(gov.2010.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2010.gov.repunit.df = gov.2010.dem
  # model.2010.gov.df = subset(model.2010.gov.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2010.gov.repunit.df$log.pop.sq.mile.2010 = log(model.2010.gov.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2010.gov.repunit.df))
  
  model.2010.gov.repunit.df2 = with(model.2010.gov.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2010.gov.repunit.df2) = model.2010.gov.repunit.df2$reporting_unit
  
  model.2010.gov.repunit.df = model.2010.gov.repunit.df2
  
  # model.2010.gov.repunit.df$MCD_FIPS = as.character(model.2010.gov.repunit.df$MCD_FIPS)
  ...
  model.2010.gov.repunit.df = plyr::join(model.2010.gov.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2010.gov.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2010.gov.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2010.gov.df,"model_2010_gov.csv")
write.csv(model.2010.gov.repunit.df,"Prepped files/2010/model_2010_gov_repunit.csv")
...
sen.2010 = read_excel(
  "2010/2010 Fall General - All Races - Ward-by-Ward_0.xls",
  skip = 10, sheet = "Sheet5",
  trim_ws = T
)

# 
sen.2010 = data.frame(apply(sen.2010, 2, function(x) gsub("^$|^ $", NA, x)))
sen.2010 = clean_names(sen.2010)


{ # Preparing basic data
  names(sen.2010)[1:3] <- c("county","reporting_unit","votes.tot")
  
  sen.2010$county[sen.2010$county == ""] <- NA
  
  require(zoo)
  sen.2010$county <- na.locf(sen.2010$county)
  
  sen.2010$reporting_unit = as.character(sen.2010$reporting_unit)
  sen.2010$reporting_unit_orig <- sen.2010$reporting_unit
  sen.2010$reporting_unit = ifelse(sen.2010$reporting_unit == "","Wards 1",sen.2010$reporting_unit)
  sen.2010$reporting_unit = toupper(sen.2010$reporting_unit)
  sen.2010$county = toupper(sen.2010$county)
  
  sen.2010 <- subset(sen.2010, reporting_unit != "COUNTY TOTALS:")
  sen.2010 <- subset(sen.2010, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  sen.2010 <- as.data.frame(lapply(sen.2010, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(sen.2010$reporting_unit)*2,2)
  
  require(stringr)
  sen.2010$reporting_unit[which(grepl("[0-9]", sen.2010$reporting_unit) == F)] <- paste(sen.2010$reporting_unit[which(grepl("[0-9]", sen.2010$reporting_unit) == F)], "1", sep = " ")
  
  sen.2010$reporting_unit <- gsub("WARD WD", "WARD", sen.2010$reporting_unit)
  sen.2010$reporting_unit <- gsub("\\bWD\\b", "WARD", sen.2010$reporting_unit)
  sen.2010$reporting_unit <- gsub("\\bWDS\\b", "WARD", sen.2010$reporting_unit)
  sen.2010$reporting_unit <- gsub("\\bWARD\\b", "WARD", sen.2010$reporting_unit)
  
  sen.2010$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", sen.2010$reporting_unit)
  sen.2010$reporting_unit <- gsub("WARDS([0-9])", "WARD \\1", sen.2010$reporting_unit)
  
  
  sen.2010$municipality_name <- unlist(str_split(str = sen.2010$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  sen.2010$municipality_name <- gsub(" WARDS","", sen.2010$municipality_name)
  sen.2010$municipality_name <- gsub(" WARD","", sen.2010$municipality_name)
  sen.2010$municipality_name <- na.locf(sen.2010$municipality_name)
  sen.2010$municipality_orig = sen.2010$municipality_name
  
  
  # sen.2010$municipality_name = with(sen.2010, paste(municipality_type, "of", municipality,sep=" "))
  # sen.2010$reporting_unit = with(sen.2010, paste(municipality_name, reporting_unit))
  # colnames(sen.2010)[1:3] = c("county","municipality_name","reporting_unit")
  # sen.2010$municipality_name = toupper(sen.2010$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  sen.2010$municipality_name[grep("TOWN OF WINDSOR", sen.2010$municipality_name)] = "VILLAGE OF WINDSOR"
  sen.2010$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", sen.2010$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # sen.2010$county[grep("TOWN OF MAINE", sen.2010$municipality_name)] = "MARATHON"
  sen.2010$municipality_name[grepl("TOWN OF MAINE", sen.2010$municipality_name) &
                               grepl("MARATHON", sen.2010$county)
                             ] = "VILLAGE OF MAINE"
  # sen.2010$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", sen.2010$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  sen.2010$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2010$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  sen.2010$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2010$reporting_unit)
                             ] = "VILLAGE OF FOX CROSSING"
  sen.2010$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", sen.2010$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  sen.2010$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", sen.2010$reporting_unit)
                          ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # sen.2010.look = with(sen.2010, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(sen.2010.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # sen.2010$congress_district = na.locf(sen.2010$congress_district)
  sen.2010$county = na.locf(sen.2010$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  sen.2010$county = trim.trailing(sen.2010$county)
  
  sen.2010$municipality_name = toupper(sen.2010$municipality_name)
  sen.2010$county = toupper(sen.2010$county)
  
  sen.2010$muni_county = paste(sen.2010$municipality_name, sen.2010$county)
  sen.2010$muni_county_orig = paste(sen.2010$municipality_orig, sen.2010$county)
  ...
  sen.2010$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", sen.2010$reporting_unit)
  sen.2010$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("WARDS","WARD",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("  "," ",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("&","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("\\bAND\\b","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub(",","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub(" - ","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub(" -","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("- ","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub(" ","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("WD","",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("COMBINED","",sen.2010$reporting_unit)
  
  sen.2010$reporting_unit = toupper(sen.2010$reporting_unit)
  sen.2010$reporting_unit = paste(sen.2010$reporting_unit, sen.2010$county, sep = "-")
  
  sen.2010$reporting_unit = gsub(" ","-",sen.2010$reporting_unit)
  sen.2010$reporting_unit = gsub("--","-",sen.2010$reporting_unit)
  
  sen.2010.look = with(sen.2010, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(sen.2010.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(sen.20102,sen.20103,sen.20104)
  
  sen.2010.v = sen.2010
  # sen.2010.v$county = NULL
  sen.2010.v$county = NULL
  sen.2010.v$municipality_name = NULL
  sen.2010.v$municipality_type = NULL
  sen.2010.v$municipalityno = NULL
  sen.2010.v$municipality = NULL
  sen.2010.v$municipality_orig = NULL
  # sen.2010.v$reporting_unit = NULL
  sen.2010.v$order = NULL
  sen.2010.v$hindi = NULL
  # sen.2010.v$assemblydistrict = NULL
  
  sen.2010.l <- sen.2010.v
  sen.2010.l$muni_county = NULL
  sen.2010.l$reporting_unit <- NULL
  sen.2010.l$muni_county_orig = NULL
  sen.2010.l$reporting_unit_orig <- NULL
  sen.2010.l$votes.tot <- NULL
  sen.2010.l$x_3 <- NULL
  sen.2010.l = gather(sen.2010.l, cand, votes_rec)
  sen.2010.l$reporting_unit <- rep(sen.2010.v$reporting_unit)
  # colnames(sen.2010.l) = c("reporting_unit", "cand", "votes.rec")
  
  # names(sen.2010.v)
  
  sen.2010.l$cand.group = NA
  sen.2010.l$cand.group[grep("russ",sen.2010.l$cand)] = "democrat"
  sen.2010.l$cand.group[grep("johnson",sen.2010.l$cand)] = "republican"
  sen.2010.l$cand.group[grep("pagels",sen.2010.l$cand)] = "other"
  sen.2010.l$cand.group[grep("laforest",sen.2010.l$cand)] = "other"
  sen.2010.l$cand.group[grep("scatter",sen.2010.l$cand)] = "other"
  
  # Joining back county and municipality names
  sen.2010.l = plyr::join(sen.2010.l, sen.2010.look, by = "reporting_unit", match = "first")
  
  sen.2010.l$votes_rec = as.numeric(as.integer(sen.2010.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(sen.2010.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  sen.2010.l = plyr::join(sen.2010.l,tot.votes.look,by="reporting_unit",match="first")
  sen.2010.l$votes_perc = (sen.2010.l$votes_rec / sen.2010.l$votes_tot) * 100
  
  sen.2010.l$muni_county = paste(sen.2010.l$municipality, sen.2010.l$county)
  
  # sen.2010.l$votes_perc = (sen.2010.l$votes_rec / sen.2010.l$votes_tot) * 100
  
  sen.2010.l$votes_tot <- NULL
  sen.2010.melt = melt(sen.2010.l)
  sen.2010.cast = dcast(sen.2010.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  sen.2010.l = sen.2010.cast
  sen.2010.l[,grep("_NA", names(sen.2010.l))] <- NULL
  
  sen.2010.l = plyr::join(sen.2010.l,tot.votes.look,by="reporting_unit",match="first")
  
  # sen.2010.l$trump_prim_won = with(sen.2010.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # sen.2010.l$votes_tot_cruz = NULL
  # sen.2010.l$votes_tot_trump = NULL
  # sen.2010.l$votes_tot_kasich = NULL
  
  colnames(sen.2010.l)[grep("votes_tot_other",colnames(sen.2010.l))] = "votes_tot"
  
  # rm(sen.2010.cast,sen.2010.melt,
  # sen.2010.mun.group ,
  # sen.2010, sen.2010.look, sen.2010.v,
  # tot.votes.look,tot.votes.look.group)
...
{ # 2010 gab stats
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2010.sen.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(sen.2010.l$muni_county))
  # setdiff(unique(sen.2010.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # sen.2010.l$muni_county[grep("VILLAGE OF MAINE MARATHON",sen.2010.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # sen.2010.l$municipality[grep("TOWN OF MAINE MARATHON",sen.2010.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(sen.2010.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(sen.2010.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  table(sen.2010.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  sen.2010.new = plyr::join(sen.2010.l, gabdata.join, by = "reporting_unit")
  sen.2010.l = sen.2010.new
  
  sen.2010.l[is.na(sen.2010.l)] = NA
  rm(sen.2010.new)
  
  setdiff(sen.2010.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  sen.2010.dem = plyr::join(sen.2010.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  sen.2010.dem = plyr::join(sen.2010.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
  off_vote_machines = read_excel("voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",sen.2010.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, sen.2010.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(sen.2010.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(sen.2010.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  sen.2010.dem = plyr::join(sen.2010.dem, off_vote_machines, by = "muni_county")
  
  sen.2010.dem$tch_model_off_full = NA
  sen.2010.dem$tch_model_off_full = with(sen.2010.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                              "Dominion (Premier)-Accuvote TSX", sen.2010.dem$tch_model_off_full))
  sen.2010.dem$tch_model_off_full = with(sen.2010.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                              "Dominion (Sequoia)/Command Central-Edge",sen.2010.dem$tch_model_off_full))
  sen.2010.dem$tch_model_off_full = with(sen.2010.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                              "Dominion (Sequoia)/Command Central-Edge",sen.2010.dem$tch_model_off_full))
  sen.2010.dem$tch_model_off_full = with(sen.2010.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                              "ES&S iVotronic",sen.2010.dem$tch_model_off_full))
  sen.2010.dem$tch_model_off_full[is.na(sen.2010.dem$tch_model_off_full)] = "aNone"
  sen.2010.dem$tch_model_off_full = with(sen.2010.dem, ifelse(is.na(tch_use_off),
                                                              NA,sen.2010.dem$tch_model_off_full))
  
  colnames(sen.2010.dem)[grep("tch.use",colnames(sen.2010.dem),fixed = T)] = "tch_use_verivote"
  colnames(sen.2010.dem)[grep("os.use",colnames(sen.2010.dem),fixed = T)] = "os_use_verivote"
  colnames(sen.2010.dem)[grep("bmd.use",colnames(sen.2010.dem),fixed = T)] = "bmd_use_verivote"
  colnames(sen.2010.dem)[grep("X._collegedegreepersons25._2013",colnames(sen.2010.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2010.sen.repunit.df = sen.2010.dem
  # model.2010.sen.df = subset(model.2010.sen.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2010.sen.repunit.df$log.pop.sq.mile.2010 = log(model.2010.sen.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2010.sen.repunit.df))
  
  model.2010.sen.repunit.df2 = with(model.2010.sen.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                          county_paper_or_paperplusmachine,
                                                                          ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2010.sen.repunit.df2) = model.2010.sen.repunit.df2$reporting_unit
  
  model.2010.sen.repunit.df = model.2010.sen.repunit.df2
  
  # model.2010.sen.repunit.df$MCD_FIPS = as.character(model.2010.sen.repunit.df$MCD_FIPS)
  ...
  model.2010.sen.repunit.df = plyr::join(model.2010.sen.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2010.sen.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2010.sen.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2010.sen.df,"model_2010_sen.csv")
write.csv(model.2010.sen.repunit.df,"Prepped files/2010/model_2010_sen_repunit.csv")


# Congressional 2010 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
# getwd()
...
read_excel_allsheets <- function(filename, tibble = FALSE) {
  ...
  # then just pass tibble = TRUE
  sheets <- readxl::excel_sheets(filename)
  x <- lapply(sheets, function(X) readxl::read_excel(filename, sheet = X, skip = 10,
                                                     n_max = 760,
                                                     trim_ws = T))
  if(!tibble) x <- lapply(x, as.data.frame)
  names(x) <- sheets
...
mysheets <- read_excel_allsheets("2010/2010 Fall General - All Races - Ward-by-Ward_0.xls")
...
cong.2010 <- rbind(mysheets[[7]], mysheets[[8]], mysheets[[9]], mysheets[[10]], mysheets[[11]], mysheets[[12]], mysheets[[13]], mysheets[[14]])

rm(mysheets)
# Now need to combine the sheets together!

# Replace blanks with NA
cong.2010 = data.frame(apply(cong.2010, 2, function(x) gsub("^$|^ $", NA, x)))
cong.2010 = clean_names(cong.2010)

{ # Preparing basic data
  names(cong.2010)[1:3] <- c("county","reporting_unit","votes.tot")
  
  cong.2010$county[cong.2010$county == ""] <- NA
  
  require(zoo)
  cong.2010$county <- na.locf(cong.2010$county)
  
  cong.2010$reporting_unit = as.character(cong.2010$reporting_unit)
  cong.2010$reporting_unit_orig <- cong.2010$reporting_unit
  cong.2010$reporting_unit = ifelse(cong.2010$reporting_unit == "","Wards 1",cong.2010$reporting_unit)
  cong.2010$reporting_unit = toupper(cong.2010$reporting_unit)
  cong.2010$county = toupper(cong.2010$county)
  
  cong.2010 <- subset(cong.2010, reporting_unit != "COUNTY TOTALS:")
  cong.2010 <- subset(cong.2010, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  cong.2010 <- as.data.frame(lapply(cong.2010, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(cong.2010$reporting_unit)*2,2)
  ...
  # Wards without any number
  cong.2010$reporting_unit[which(grepl("[0-9]", cong.2010$reporting_unit) == F)] <- paste(cong.2010$reporting_unit[which(grepl("[0-9]", cong.2010$reporting_unit) == F)], "1", sep = " ")
  
  cong.2010$reporting_unit <- gsub("WARD WD", "WARD", cong.2010$reporting_unit)
  cong.2010$reporting_unit <- gsub("\\bWD\\b", "WARD", cong.2010$reporting_unit)
  cong.2010$reporting_unit <- gsub("\\bWDS\\b", "WARD", cong.2010$reporting_unit)
  cong.2010$reporting_unit <- gsub("\\bWARD\\b", "WARD", cong.2010$reporting_unit)
  
  cong.2010$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", cong.2010$reporting_unit)
  cong.2010$reporting_unit <- gsub("WARDS([0-9])", "WARD \\1", cong.2010$reporting_unit)
  
  cong.2010$municipality_name <- unlist(str_split(str = cong.2010$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  # cong.2010$municipality_name2 <- unlist(str_split(str = cong.2010$reporting_unit, " [0-9]", n = 2))[odd_indexes + 1]
  # cong.2010$municipality_name3 <- unlist(str_split(str = cong.2010$reporting_unit, " [0-9]", n = 2))[odd_indexes + 2]
  cong.2010$municipality_name <- gsub(" WARDS","", cong.2010$municipality_name)
  cong.2010$municipality_name <- gsub(" WARD","", cong.2010$municipality_name)
  cong.2010$municipality_name <- na.locf(cong.2010$municipality_name)
  cong.2010$municipality_orig = cong.2010$municipality_name
  
  
  # cong.2010$municipality_name = with(cong.2010, paste(municipality_type, "of", municipality,sep=" "))
  # cong.2010$reporting_unit = with(cong.2010, paste(municipality_name, reporting_unit))
  # colnames(cong.2010)[1:3] = c("county","municipality_name","reporting_unit")
  # cong.2010$municipality_name = toupper(cong.2010$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  cong.2010$municipality_name[grep("TOWN OF WINDSOR", cong.2010$municipality_name)] = "VILLAGE OF WINDSOR"
  cong.2010$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", cong.2010$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # cong.2010$county[grep("TOWN OF MAINE", cong.2010$municipality_name)] = "MARATHON"
  cong.2010$municipality_name[grepl("TOWN OF MAINE", cong.2010$municipality_name) &
                                grepl("MARATHON", cong.2010$county)
                              ] = "VILLAGE OF MAINE"
  # cong.2010$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", cong.2010$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  cong.2010$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", cong.2010$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  cong.2010$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", cong.2010$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  cong.2010$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", cong.2010$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  cong.2010$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", cong.2010$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # cong.2010.look = with(cong.2010, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(cong.2010.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # cong.2010$congress_district = na.locf(cong.2010$congress_district)
  cong.2010$county = na.locf(cong.2010$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  cong.2010$county = trim.trailing(cong.2010$county)
  
  cong.2010$municipality_name = toupper(cong.2010$municipality_name)
  cong.2010$county = toupper(cong.2010$county)
  
  cong.2010$muni_county = paste(cong.2010$municipality_name, cong.2010$county)
  cong.2010$muni_county_orig = paste(cong.2010$municipality_orig, cong.2010$county)
  ...
  cong.2010$reporting_unit = gsub("WARDS","WARD",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("  "," ",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("&","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("\\bAND\\b","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub(",","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub(" - ","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub(" -","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("- ","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub(" ","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("WD","",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("COMBINED","",cong.2010$reporting_unit)
  
  cong.2010$reporting_unit = toupper(cong.2010$reporting_unit)
  cong.2010$reporting_unit = paste(cong.2010$reporting_unit, cong.2010$county, sep = "-")
  
  cong.2010$reporting_unit = gsub(" ","-",cong.2010$reporting_unit)
  cong.2010$reporting_unit = gsub("--","-",cong.2010$reporting_unit)
  
  cong.2010.look = with(cong.2010, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(cong.2010.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(cong.20102,cong.20103,cong.20104)
  
  cong.2010.v = cong.2010
  # cong.2010.v$county = NULL
  cong.2010.v$county = NULL
  cong.2010.v$municipality_name = NULL
  cong.2010.v$municipality_type = NULL
  cong.2010.v$municipalityno = NULL
  cong.2010.v$municipality = NULL
  cong.2010.v$municipality_orig = NULL
  # cong.2010.v$reporting_unit = NULL
  cong.2010.v$order = NULL
  cong.2010.v$hindi = NULL
  # cong.2010.v$assemblydistrict = NULL
  
  cong.2010.l <- cong.2010.v
  cong.2010.l$muni_county = NULL
  cong.2010.l$reporting_unit <- NULL
  cong.2010.l$muni_county_orig = NULL
  cong.2010.l$reporting_unit_orig <- NULL
  cong.2010.l$votes.tot <- NULL
  cong.2010.l$x_3 <- NULL
  cong.2010.l = gather(cong.2010.l, cand, votes_rec)
  cong.2010.l$reporting_unit <- rep(cong.2010.v$reporting_unit)
  # colnames(cong.2010.l) = c("reporting_unit", "cand", "votes.rec")
  
  # names(cong.2010.v)
  
  cong.2010.l$cand.group = NA
  cong.2010.l$cand.group[grep("dem",cong.2010.l$cand)] = "democrat"
  cong.2010.l$cand.group[grep("rep",cong.2010.l$cand)] = "republican"
  cong.2010.l$cand.group[grep("other",cong.2010.l$cand)] = "other"
  cong.2010.l$cand.group[grep("scatter",cong.2010.l$cand)] = "other"
  
  # Joining back county and municipality names
  cong.2010.l = plyr::join(cong.2010.l, cong.2010.look, by = "reporting_unit", match = "first")
  
  cong.2010.l$votes_rec = as.numeric(as.integer(cong.2010.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(cong.2010.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  cong.2010.l = plyr::join(cong.2010.l,tot.votes.look,by="reporting_unit",match="first")
  cong.2010.l$votes_perc = (cong.2010.l$votes_rec / cong.2010.l$votes_tot) * 100
  
  cong.2010.l$muni_county = paste(cong.2010.l$municipality, cong.2010.l$county)
  
  # cong.2010.l$votes_perc = (cong.2010.l$votes_rec / cong.2010.l$votes_tot) * 100
  
  cong.2010.l$votes_tot <- NULL
  cong.2010.melt = melt(cong.2010.l)
  cong.2010.cast = dcast(cong.2010.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  cong.2010.l = cong.2010.cast
  cong.2010.l[,grep("_NA", names(cong.2010.l))] <- NULL
  
  cong.2010.l = plyr::join(cong.2010.l,tot.votes.look,by="reporting_unit",match="first")
  
  # cong.2010.l$trump_prim_won = with(cong.2010.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # cong.2010.l$votes_tot_cruz = NULL
  # cong.2010.l$votes_tot_trump = NULL
  # cong.2010.l$votes_tot_kasich = NULL
  
  colnames(cong.2010.l)[grep("votes_tot_other",colnames(cong.2010.l))] = "votes_tot"
  
  # rm(cong.2010.cast,cong.2010.melt,
  # cong.2010.mun.group ,
  # cong.2010, cong.2010.look, cong.2010.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # 2010 gab stats
  gabdata.2010.cong = read.xlsx2(
    "2010/20101102_general_gab190_xls_40096.xls",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
  ...
  # Preparing basic data
  gabdata.2010.cong = clean_names(gabdata.2010.cong)
  gabdata.2010.cong$reporting_unit_orig = gabdata.2010.cong$reporting_unit
  # gabdata.2010.cong$reporting_unit = toupper(gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$municipality = toupper(gabdata.2010.cong$municipality)
  gabdata.2010.cong$county = toupper(gabdata.2010.cong$county)
  
  gabdata.2010.cong$county = gsub(" COUNTY","", gabdata.2010.cong$county)
  gabdata.2010.cong$muni_county = paste(gabdata.2010.cong$municipality, gabdata.2010.cong$county)
  
  gabdata.2010.cong$reporting_unit = paste(gabdata.2010.cong$municipality,gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = toupper(gabdata.2010.cong$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2010.cong$municipality[grep("TOWN OF WINDSOR", gabdata.2010.cong$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2010.cong$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2010.cong$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2010.cong$county[grep("TOWN OF MAINE", gabdata.2010.cong$municipality)] = "MARATHON"
  gabdata.2010.cong$municipality[grepl("TOWN OF MAINE", gabdata.2010.cong$municipality) &
                                   grepl("MARATHON", gabdata.2010.cong$county)
                                 ] = "VILLAGE OF MAINE"
  # gabdata.2010.cong$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2010.cong$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2010.cong$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2010.cong$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2010.cong$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2010.cong$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2010.cong$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2010.cong$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2010.cong$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2010.cong$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2010.cong$reporting_unit = paste(gabdata.2010.cong$reporting_unit, gabdata.2010.cong$county)
  
  gabdata.2010.cong$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit <- gsub("\\bWDS\\b", "WARD", gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2010.cong$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2010.cong$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit <- gsub("WARDS([0-9])", "WARD-\\1", gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("WARDS","WARD",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("  "," ",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("&","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("\\bAND\\b","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub(",","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub(" - ","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub(" -","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("- ","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub(" ","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("WD","",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("COMBINED","",gabdata.2010.cong$reporting_unit)
  
  gabdata.2010.cong$reporting_unit = gsub(" ","-",gabdata.2010.cong$reporting_unit)
  gabdata.2010.cong$reporting_unit = gsub("--","-",gabdata.2010.cong$reporting_unit)
  
  # ward.gabdata.group = dplyr::group_by(gabdata.2010.cong,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                    registrants = sum(registrants),
  #                                    late_registrants = sum(late_registrants),
  #                                    ed_rs = sum(ed_rs),
  ...
  gabdata.repunit <- gabdata.2010.cong
  
  # write.csv(gabdata.repunit,"jun.2016.2010.cong.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(cong.2010.l$muni_county))
  # setdiff(unique(cong.2010.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # cong.2010.l$muni_county[grep("VILLAGE OF MAINE MARATHON",cong.2010.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # cong.2010.l$municipality[grep("TOWN OF MAINE MARATHON",cong.2010.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(cong.2010.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(cong.2010.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
  ...
  table(cong.2010.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  cong.2010.new = plyr::join(cong.2010.l, gabdata.join, by = "reporting_unit")
  cong.2010.l = cong.2010.new
  
  cong.2010.l[is.na(cong.2010.l)] = NA
  rm(cong.2010.new)
  
  setdiff(cong.2010.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  cong.2010.dem = plyr::join(cong.2010.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  cong.2010.dem = plyr::join(cong.2010.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")
  off_vote_machines = read_excel("voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",cong.2010.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, cong.2010.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(cong.2010.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(cong.2010.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  cong.2010.dem = plyr::join(cong.2010.dem, off_vote_machines, by = "muni_county")
  
  cong.2010.dem$tch_model_off_full = NA
  cong.2010.dem$tch_model_off_full = with(cong.2010.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                                "Dominion (Premier)-Accuvote TSX", cong.2010.dem$tch_model_off_full))
  cong.2010.dem$tch_model_off_full = with(cong.2010.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                                "Dominion (Sequoia)/Command Central-Edge",cong.2010.dem$tch_model_off_full))
  cong.2010.dem$tch_model_off_full = with(cong.2010.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                                "Dominion (Sequoia)/Command Central-Edge",cong.2010.dem$tch_model_off_full))
  cong.2010.dem$tch_model_off_full = with(cong.2010.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                                "ES&S iVotronic",cong.2010.dem$tch_model_off_full))
  cong.2010.dem$tch_model_off_full[is.na(cong.2010.dem$tch_model_off_full)] = "aNone"
  cong.2010.dem$tch_model_off_full = with(cong.2010.dem, ifelse(is.na(tch_use_off),
                                                                NA,cong.2010.dem$tch_model_off_full))
  
  colnames(cong.2010.dem)[grep("tch.use",colnames(cong.2010.dem),fixed = T)] = "tch_use_verivote"
  colnames(cong.2010.dem)[grep("os.use",colnames(cong.2010.dem),fixed = T)] = "os_use_verivote"
  colnames(cong.2010.dem)[grep("bmd.use",colnames(cong.2010.dem),fixed = T)] = "bmd_use_verivote"
  colnames(cong.2010.dem)[grep("X._collegedegreepersons25._2013",colnames(cong.2010.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2010.cong.repunit.df = cong.2010.dem
  # model.2010.cong.df = subset(model.2010.cong.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2010.cong.repunit.df$log.pop.sq.mile.2010 = log(model.2010.cong.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2010.cong.repunit.df))
  
  model.2010.cong.repunit.df2 = with(model.2010.cong.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                            sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                            county_paper_or_paperplusmachine,
                                                                            ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                            pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2010.cong.repunit.df2) = model.2010.cong.repunit.df2$reporting_unit
  
  model.2010.cong.repunit.df = model.2010.cong.repunit.df2
  
  # model.2010.cong.repunit.df$MCD_FIPS = as.character(model.2010.cong.repunit.df$MCD_FIPS)
  ...
  model.2010.cong.repunit.df = plyr::join(model.2010.cong.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2010.cong.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2010.cong.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2010.cong.df,"model_2010_cong.csv")
write.csv(model.2010.cong.repunit.df,"Prepped files/2010/model_2010_cong_repunit.csv")

# ADD IN 2010 SEC STATE HERE
# Presidential 2008 data ------------------------------------------------
setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/2008 results")
# getwd()

pres.2008 = read.csv(
  "2008_FallElection_President_WardbyWard.csv",
  stringsAsFactors = F,
  header = T,
  strip.white = T
)
...
pres.2008 = pres.2008[,11:27]

pres.2008 = clean_names(pres.2008)

pres.2008$reporting_unit = ifelse(pres.2008$reporting_unit == "","Ward 1",pres.2008$reporting_unit)
pres.2008$municipality_name = with(pres.2008, paste(municipality_type, "of", municipality,sep=" "))
pres.2008$reporting_unit = with(pres.2008, paste(municipality_name, reporting_unit))

pres.2008 <- select(pres.2008, municipality_name, everything())

# Replace blanks with NA
pres.2008 = data.frame(apply(pres.2008, 2, function(x) gsub("^$|^ $", NA, x)))
pres.2008[,9:length(pres.2008)] = apply(pres.2008[,9:length(pres.2008)], 2, function(x) as.character(x))
pres.2008[,9:length(pres.2008)] = apply(pres.2008[,9:length(pres.2008)], 2, function(x) as.numeric(x))
pres.2008 = clean_names(pres.2008)

# str(pres.2008)
# names(pres.2008)[17] <- "reporting_unit" 
# pres.2008.g <- group_by(pres.2008, reporting_unit, county_name, municipality_name)
# pres.2008 <- summarise_if(pres.2008.g, is.numeric, sum, na.rm = T)

{ # Preparing basic data
  # names(pres.2008)[1] <- "county"
  pres.2008$votes.tot <- rowSums(pres.2008[,9:length(pres.2008)], na.rm = T)
  
  pres.2008$county[pres.2008$county == ""] <- NA
  
  require(zoo)
  pres.2008$county <- na.locf(pres.2008$county)
  
  pres.2008$reporting_unit = as.character(pres.2008$reporting_unit)
  pres.2008$reporting_unit_orig <- pres.2008$reporting_unit
  pres.2008$reporting_unit = ifelse(pres.2008$reporting_unit == "","Wards 1",pres.2008$reporting_unit)
  pres.2008$reporting_unit = toupper(pres.2008$reporting_unit)
  pres.2008$county = toupper(pres.2008$county)
  
  pres.2008 <- subset(pres.2008, reporting_unit != "COUNTY TOTALS:")
  pres.2008 <- subset(pres.2008, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  pres.2008 <- as.data.frame(lapply(pres.2008, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(pres.2008$reporting_unit)*2,2)
  
  require(stringr)
  pres.2008$reporting_unit[which(grepl("[0-9]", pres.2008$reporting_unit) == F)] <- paste(pres.2008$reporting_unit[which(grepl("[0-9]", pres.2008$reporting_unit) == F)], "1", sep = " ")
  
  pres.2008$reporting_unit <- gsub("WARD WD", "WARD", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("\\bWD\\b", "WARD", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("\\bWDS\\b", "WARD", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("\\bWARD\\b", "WARD", pres.2008$reporting_unit)
  
  pres.2008$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", pres.2008$reporting_unit)
  
  
  pres.2008$municipality_name <- unlist(str_split(str = pres.2008$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  pres.2008$municipality_name <- gsub(" WARDS","", pres.2008$municipality_name)
  pres.2008$municipality_name <- gsub(" WARD","", pres.2008$municipality_name)
  pres.2008$municipality_name <- na.locf(pres.2008$municipality_name)
  pres.2008$municipality_orig = pres.2008$municipality_name
  
  
  # pres.2008$municipality_name = with(pres.2008, paste(municipality_type, "of", municipality,sep=" "))
  # pres.2008$reporting_unit = with(pres.2008, paste(municipality_name, reporting_unit))
  # colnames(pres.2008)[1:3] = c("county","municipality_name","reporting_unit")
  # pres.2008$municipality_name = toupper(pres.2008$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  pres.2008$municipality_name[grep("TOWN OF WINDSOR", pres.2008$municipality_name)] = "VILLAGE OF WINDSOR"
  pres.2008$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", pres.2008$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # pres.2008$county[grep("TOWN OF MAINE", pres.2008$municipality_name)] = "MARATHON"
  pres.2008$municipality_name[grepl("TOWN OF MAINE", pres.2008$municipality_name) &
                                grepl("MARATHON", pres.2008$county)
                              ] = "VILLAGE OF MAINE"
  # pres.2008$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", pres.2008$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  pres.2008$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", pres.2008$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  pres.2008$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", pres.2008$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  pres.2008$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", pres.2008$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  pres.2008$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", pres.2008$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
 
  # pres.2008.look = with(pres.2008, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(pres.2008.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # pres.2008$congress_district = na.locf(pres.2008$congress_district)
  pres.2008$county = na.locf(pres.2008$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  pres.2008$county = trim.trailing(pres.2008$county)
  
  pres.2008$municipality_name = toupper(pres.2008$municipality_name)
  pres.2008$county = toupper(pres.2008$county)
  
  pres.2008$muni_county = paste(pres.2008$municipality_name, pres.2008$county)
  pres.2008$muni_county_orig = paste(pres.2008$municipality_orig, pres.2008$county)
  ...
  pres.2008$reporting_unit <- gsub("WARD WD", "WARD", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("\\bWD\\b", "WARD", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("\\bWDS\\b", "WARDS", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("\\bWARD\\b", "WARD", pres.2008$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  
  pres.2008$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", pres.2008$reporting_unit)
  pres.2008$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", pres.2008$reporting_unit)
  # pres.2008$reporting_unit = gsub("WARDS","WARD",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("  "," ",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("&","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("\\bAND\\b","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub(",","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub(" - ","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub(" -","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("- ","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub(" ","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("WD","",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("COMBINED","",pres.2008$reporting_unit)
  
  pres.2008$reporting_unit = toupper(pres.2008$reporting_unit)
  pres.2008$reporting_unit = paste(pres.2008$reporting_unit, pres.2008$county, sep = "-")
  
  pres.2008$reporting_unit = gsub(" ","-",pres.2008$reporting_unit)
  pres.2008$reporting_unit = gsub("--","-",pres.2008$reporting_unit)
  
  pres.2008.look = with(pres.2008, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(pres.2008.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(pres.20082,pres.20083,pres.20084)
  
  pres.2008.v = pres.2008
  # pres.2008.v$county = NULL
  pres.2008.v$county = NULL
  pres.2008.v$municipality_name = NULL
  pres.2008.v$municipality_type = NULL
  pres.2008.v$municipalityno = NULL
  pres.2008.v$municipality = NULL
  pres.2008.v$municipality_orig = NULL
  # pres.2008.v$reporting_unit = NULL
  pres.2008.v$order = NULL
  pres.2008.v$hindi = NULL
  # pres.2008.v$assemblydistrict = NULL
  # pres.2008.v = subset(  pres.2008.v, original_or_recount == "Recount")
  
  pres.2008.l <- pres.2008.v
  pres.2008.l$muni_county = NULL
  pres.2008.l$reporting_unit <- NULL
  pres.2008.l$muni_county_orig = NULL
  pres.2008.l$reporting_unit_orig <- NULL
  pres.2008.l$votes.tot <- NULL
  pres.2008.l$x_3 <- NULL
  pres.2008.l = gather(pres.2008.l, cand, votes_rec)
  pres.2008.l$reporting_unit <- rep(pres.2008.v$reporting_unit)
  # colnames(pres.2008.l) = c("reporting_unit", "cand", "votes.rec")
  
  # pres.2008.l$original_or_recount = NULL
  
  pres.2008.l$cand.group = NA
  pres.2008.l$cand.group[grep("obama",pres.2008.l$cand)] = "democrat"
  pres.2008.l$cand.group[grep("mc_cain",pres.2008.l$cand)] = "republican"
  pres.2008.l$cand.group[grep("rosa",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("wayne",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("stewart",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("robert",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("gonzalez",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("castle",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("darrell",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("klimisch",pres.2008.l$cand)] = "other"
  pres.2008.l$cand.group[grep("scatter",pres.2008.l$cand)] = "other"
  
  # Joining back county and municipality names
  pres.2008.l = plyr::join(pres.2008.l, pres.2008.look, by = "reporting_unit", match = "first")
  
  pres.2008.l$votes_rec = as.numeric(as.integer(pres.2008.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(pres.2008.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  pres.2008.l = plyr::join(pres.2008.l,tot.votes.look,by="reporting_unit",match="first")
  pres.2008.l$votes_perc = (pres.2008.l$votes_rec / pres.2008.l$votes_tot) * 100
  
  pres.2008.l$muni_county = paste(pres.2008.l$municipality, pres.2008.l$county)
  
  # pres.2008.l$votes_perc = (pres.2008.l$votes_rec / pres.2008.l$votes_tot) * 100
  
  pres.2008.l$votes_tot <- NULL
  pres.2008.melt = melt(pres.2008.l)
  pres.2008.cast = dcast(pres.2008.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  pres.2008.l = pres.2008.cast
  pres.2008.l[,grep("_NA", names(pres.2008.l))] <- NULL
  
  pres.2008.l = plyr::join(pres.2008.l,tot.votes.look,by="reporting_unit",match="first")
  
  # pres.2008.l$trump_prim_won = with(pres.2008.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # pres.2008.l$votes_tot_cruz = NULL
  # pres.2008.l$votes_tot_trump = NULL
  # pres.2008.l$votes_tot_kasich = NULL
  
  colnames(pres.2008.l)[grep("votes_tot_other",colnames(pres.2008.l))] = "votes_tot"
  
  # rm(pres.2008.cast,pres.2008.melt,
  # pres.2008.mun.group ,
  # pres.2008, pres.2008.look, pres.2008.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # GAB  / EDL stats
  gabdata.2008.pres = read.xlsx2(
    "2008nov_gab190_statistics_xls_13418MOD.xls",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
    colClasses = NA
  ...
  # names(gabdata.2008.pres)
  
  # Preparing basic data
  gabdata.2008.pres = clean_names(gabdata.2008.pres)
  ...
  # gabdata.2008.pres.g <- group_by(gabdata.2008.pres, hindi, county, municipality, reporting_unit)
  # gabdata.2008.pres.m <- dplyr::summarise_if(gabdata.2008.pres.g, is.numeric, sum, na.rm = T)
  
  # Removing some random weirdness
  
  gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "CITY OF BUFFALO CITY"] <- "CITY OF BUFFALO"
  # gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "CITY OF FOUNTAIN CITY"] <- "CITY OF FOUNTAIN"
  gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "VILLAGE OF DEFOREST"] <- "VILLAGE OF DE FOREST"
  gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "VILLAGE OF LAVALLE"] <- "VILLAGE OF LA VALLE"
  gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "TOWN OF LAND O-LAKES"] <- "TOWN OF LAND O'LAKES"
  gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "TOWN OF POY SIPPI"] <- "TOWN OF POYSIPPI"
  gabdata.2008.pres$municipality[gabdata.2008.pres$municipality == "TOWN OF SAINT LAWRENCE"] <- "TOWN OF ST LAWRENCE"
  
  gabdata.2008.pres$municipality <- gsub("\\.", "", gabdata.2008.pres$municipality) # for St Cloud and St marie, Nazianz
  
  gabdata.2008.pres$county <- gsub("LA CROSSE", "LACROSSE", gabdata.2008.pres$county) # for St Cloud and St marie
  
  gabdata.2008.pres$county <- gsub("MT", "MOUNT", gabdata.2008.pres$county) # for Mount Sterling
  
  # City of Ashland wards 1 - 11 are separate in results but combined in the GAB. Could I combine them all in the presidential results? ***
  # City of Stanley ward 5 just doesn't exist in GAB
  # City of Mount Sterling just doesn't exist in GAB
  # Fitchburg wards don't exist in those combinations in GAB. *** Add 4 & 6, 5 from pres results?
  # Fitchburg ward 13 has no voters
  # Hartford wards don't exist in those combinations in GAB (16 and 39). But there are no voters so doesn't really matter
  # Claybanks doesn't exist in GAB
  # Ixonia doesn't exist in that combo in gab *** add 134, 2 from presidential results?
  # Bristol wards 5-7, *** add 5 & 7, 6 from presidential results?
  # Town of Somers wards 7 & 8. *** Add 7, 8 from presidential results?
  
  
  # Combining wards in the Presedential results
  
  # grouping variables: "reporting_unit"        "reporting_unit_orig"   "muni_county"           "municipality" "municipality_orig"     "county" 
  # columns to average: all votes_perc
  # columns to add: all votes_rec, all votes_tot
  
  gabdata.2008.pres$reporting_unit_orig = gabdata.2008.pres$reporting_unit
  # gabdata.2008.pres$reporting_unit = toupper(gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$municipality = toupper(gabdata.2008.pres$municipality)
  gabdata.2008.pres$county = toupper(gabdata.2008.pres$county)
  
  gabdata.2008.pres$county = gsub(" COUNTY","", gabdata.2008.pres$county)
  gabdata.2008.pres$muni_county = paste(gabdata.2008.pres$municipality, gabdata.2008.pres$county)
  
  gabdata.2008.pres$reporting_unit = paste(gabdata.2008.pres$municipality,gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = toupper(gabdata.2008.pres$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2008.pres$municipality[grep("TOWN OF WINDSOR", gabdata.2008.pres$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2008.pres$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2008.pres$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2008.pres$county[grep("TOWN OF MAINE", gabdata.2008.pres$municipality)] = "MARATHON"
  gabdata.2008.pres$municipality[grepl("TOWN OF MAINE", gabdata.2008.pres$municipality) &
                                   grepl("MARATHON", gabdata.2008.pres$county)
                                 ] = "VILLAGE OF MAINE"
  # gabdata.2008.pres$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2008.pres$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2008.pres$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2008.pres$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2008.pres$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2008.pres$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2008.pres$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2008.pres$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2008.pres$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2008.pres$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"

  gabdata.2008.pres$muni_county = paste(gabdata.2008.pres$municipality, gabdata.2008.pres$county)
  gabdata.2008.pres$reporting_unit = paste(gabdata.2008.pres$reporting_unit, gabdata.2008.pres$county)
  
  gabdata.2008.pres$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2008.pres$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2008.pres$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("([0-9]),([0-9])", "\\1-\\2", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("([0-9]),([0-9])", "\\1-\\2", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2008.pres$reporting_unit)
  # gabdata.2008.pres$reporting_unit = gsub("WARDS","WARD",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("  "," ",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("&","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("\\bAND\\b","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub(",","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub(" - ","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub(" -","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("- ","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub(" ","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("WD","",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("COMBINED","",gabdata.2008.pres$reporting_unit)
  
  gabdata.2008.pres$reporting_unit = gsub(" ","-",gabdata.2008.pres$reporting_unit)
  gabdata.2008.pres$reporting_unit = gsub("--","-",gabdata.2008.pres$reporting_unit)
  
  ### Replacing some more names to get matches
  
  gabdata.2008.pres$reporting_unit[gabdata.2008.pres$reporting_unit == "CITY-OF-FITCHBURG-WARDS-10-14-DANE"] <- "CITY-OF-FITCHBURG-WARDS-10-12-14-DANE"
  
  somers_wards <- c("TOWN-OF-SOMERS-WARD-7-KENOSHA", "TOWN-OF-SOMERS-WARD-8-KENOSHA")  
  pres.2008.l <- replace_repunit_rows(pres.2008.l, somers_wards, "TOWN-OF-SOMERS-WARDS-7-8-KENOSHA")  
  
  bristol_wards <- c("TOWN-OF-BRISTOL-WARDS-5-7-KENOSHA", "TOWN-OF-BRISTOL-WARD-6-KENOSHA")  
  pres.2008.l <- replace_repunit_rows(pres.2008.l, bristol_wards, "TOWN-OF-BRISTOL-WARDS-5-7-KENOSHA")
  
  ixonia_wards <- c("TOWN-OF-IXONIA-WARDS-1-3-4-JEFFERSON", "TOWN-OF-IXONIA-WARD-2-JEFFERSON")  
  pres.2008.l <- replace_repunit_rows(pres.2008.l, ixonia_wards, "TOWN-OF-IXONIA-WARDS-1-4-JEFFERSON")
  
  fitchburg_wards <- c("CITY-OF-FITCHBURG-WARDS-4-6-DANE", "CITY-OF-FITCHBURG-WARD-5-DANE")  
  pres.2008.l <- replace_repunit_rows(pres.2008.l, fitchburg_wards, "CITY-OF-FITCHBURG-WARDS-4-6-DANE")
  
  ashland_wards <- c("CITY-OF-ASHLAND-WARD-1-ASHLAND","CITY-OF-ASHLAND-WARD-2-ASHLAND","CITY-OF-ASHLAND-WARD-3-ASHLAND","CITY-OF-ASHLAND-WARD-4-ASHLAND", "CITY-OF-ASHLAND-WARD-5-ASHLAND","CITY-OF-ASHLAND-WARD-6-ASHLAND", "CITY-OF-ASHLAND-WARD-7-ASHLAND","CITY-OF-ASHLAND-WARD-8-ASHLAND", "CITY-OF-ASHLAND-WARD-9-ASHLAND","CITY-OF-ASHLAND-WARD-10-ASHLAND","CITY-OF-ASHLAND-WARD-11-ASHLAND") 
  pres.2008.l <- replace_repunit_rows(pres.2008.l, ashland_wards, "CITY-OF-ASHLAND-WARDS-1-11-ASHLAND")
  
  gabdata.repunit <- gabdata.2008.pres
...
{ # Matching voting statistics with the election statistics
  ...
  gabdata.join$turnout_perc = with(gabdata.repunit, total_ballots / early_registrants)
  gabdata.join$turnout_perc[gabdata.join$turnout_perc == Inf] = NA
  gabdata.join$sameday_reg_prop = with(gabdata.repunit, ed_rs / early_registrants)
  gabdata.join$sameday_reg_prop[gabdata.join$sameday_reg_prop == Inf] = NA
  gabdata.join$counted_absent_prop = with(gabdata.repunit,
                                          absentee_counted / total_ballots)
  gabdata.join$counted_absent_prop[gabdata.join$counted_absent_prop == Inf] = NA
  ...
  gabdata.join$absentee_undeliverable = with(gabdata.repunit,
                                             absentee_undeliverable / total_ballots)
  gabdata.join$registrants = with(gabdata.repunit, early_registrants)
  gabdata.join$late_registrants = with(gabdata.repunit, late_registrants / total_ballots)
  
  gabdata.join[is.na(gabdata.join)] = NA
  
  # table(pres.2008.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  pres.2008.new = plyr::join(pres.2008.l, gabdata.join, by = "reporting_unit")
  pres.2008.l = pres.2008.new
  
  pres.2008.l[is.na(pres.2008.l)] = NA
  rm(pres.2008.new)
  
  setdiff(pres.2008.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  pres.2008.dem = plyr::join(pres.2008.l, demographics.caps.join, by = "county", match = "first")
  
  elec.snapshot.dem$county = toupper( elec.snapshot.dem$county)
  
  elec.snapshot.dem.join <- with(elec.snapshot.dem, data.frame(county, pct_white, 
                                                               pct_graduate,  inc_2015,unemp_rate,
                                                               pct_male, pct_old, pct_latino, pct_white, pct_black, pct_hs))
  
  # Also joining the data from the Election snapshot
  pres.2008.dem = plyr::join(pres.2008.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",pres.2008.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, pres.2008.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(pres.2008.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(pres.2008.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  pres.2008.dem = plyr::join(pres.2008.dem, off_vote_machines, by = "muni_county")
  
  pres.2008.dem$tch_model_off_full = NA
  pres.2008.dem$tch_model_off_full = with(pres.2008.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                                "Dominion (Premier)-Accuvote TSX", pres.2008.dem$tch_model_off_full))
  pres.2008.dem$tch_model_off_full = with(pres.2008.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                                "Dominion (Sequoia)/Command Central-Edge",pres.2008.dem$tch_model_off_full))
  pres.2008.dem$tch_model_off_full = with(pres.2008.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                                "Dominion (Sequoia)/Command Central-Edge",pres.2008.dem$tch_model_off_full))
  pres.2008.dem$tch_model_off_full = with(pres.2008.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                                "ES&S iVotronic",pres.2008.dem$tch_model_off_full))
  pres.2008.dem$tch_model_off_full[is.na(pres.2008.dem$tch_model_off_full)] = "aNone"
  pres.2008.dem$tch_model_off_full = with(pres.2008.dem, ifelse(is.na(tch_use_off),
                                                                NA,pres.2008.dem$tch_model_off_full))
  
  colnames(pres.2008.dem)[grep("tch.use",colnames(pres.2008.dem),fixed = T)] = "tch_use_verivote"
  colnames(pres.2008.dem)[grep("os.use",colnames(pres.2008.dem),fixed = T)] = "os_use_verivote"
  colnames(pres.2008.dem)[grep("bmd.use",colnames(pres.2008.dem),fixed = T)] = "bmd_use_verivote"
  colnames(pres.2008.dem)[grep("X._collegedegreepersons25._2013",colnames(pres.2008.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2008.pres.repunit.df = pres.2008.dem
  # model.2008.pres.df = subset(model.2008.pres.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2008.pres.repunit.df$log.pop.sq.mile.2010 = log(model.2008.pres.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2008.pres.repunit.df))
  
  model.2008.pres.repunit.df2 = with(model.2008.pres.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                            sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                            county_paper_or_paperplusmachine,
                                                                            ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                            pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2008.pres.repunit.df2) = model.2008.pres.repunit.df2$reporting_unit
  
  model.2008.pres.repunit.df = model.2008.pres.repunit.df2
  
  # model.2008.pres.repunit.df$MCD_FIPS = as.character(model.2008.pres.repunit.df$MCD_FIPS)
  ...
  model.2008.pres.repunit.df = plyr::join(model.2008.pres.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2008.pres.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2008.pres.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2008.pres.df,"model_2008_pres.csv")
write.csv(model.2008.pres.repunit.df,"Prepped files/2008/model_2008_pres_repunit.csv")
...
where_total <- which(cong.2008$reporting_unit == "TOTAL")-1
last_row <- 1
test <- list()
...
for (i in 1:(length(which(cong.2008$reporting_unit == "TOTAL")))){
  ...
  last_row <- where_total[i]+3
  
  if (sum(grepl("Democratic",test[[i]][1,])) > 0) {
    names(test[[i]])[grep("Democratic",test[[i]][1,]): length(test[[i]])] <- test[[i]][1,grep("Democratic",test[[i]][1,]): length(test[[i]])]
  }
  else {
    names(test[[i]])[grep("Republican",test[[i]][1,]): length(test[[i]])] <- test[[i]][1,grep("Republican",test[[i]][1,]): length(test[[i]])]
  }
  
  # Replace blanks with NA
  test[[i]] = data.frame(apply(test[[i]], 2, function(x) gsub("^$|^ $", NA, x)))
  test[[i]][,8:length(test[[i]])] = apply(test[[i]][,8:length(test[[i]])], 2, function(x) as.character(x))
  test[[i]][,8:length(test[[i]])] = apply(test[[i]][,8:length(test[[i]])], 2, function(x) as.numeric(x))
  
  # test[[i]]$other <- rowSums(test[[i]][,which(is.na(names(test[[i]])))], na.rm = T)

   test[[i]]$municipality_1 <- NULL
   test[[i]]$hindi <- NULL
   test[[i]]$order <- NULL
   # test[[i]]$`NA` <- NULL
}
...
require(data.table)
cong.2008.joined <- rbindlist(test, fill = T)
cong.2008.joined <- cong.2008.joined[-1,] 

names(cong.2008.joined)[1:3] <- c("county","municipality_type", "municipality")

cong.2008 <- cong.2008.joined

cong.2008 <- cong.2008[!cong.2008$county == "NAME",]

cong.2008$reporting_unit = as.character(cong.2008$reporting_unit)
cong.2008$reporting_unit = ifelse(cong.2008$reporting_unit == "" | is.na(cong.2008$reporting_unit),"Ward 1",cong.2008$reporting_unit)
cong.2008$municipality_name = with(cong.2008, paste(municipality_type, "of", municipality,sep=" "))
cong.2008$reporting_unit = with(cong.2008, paste(municipality_name, reporting_unit))

cong.2008 <- select(cong.2008, municipality_name, everything())

cong.2008 = clean_names(cong.2008)

{ # Preparing basic data
  # names(cong.2008)[1] <- "county"
  cong.2008$votes.tot <- rowSums(cong.2008[,9:length(cong.2008)], na.rm = T)
  
  cong.2008$county[cong.2008$county == ""] <- NA
  
  require(zoo)
  cong.2008$county <- na.locf(cong.2008$county)
  
  cong.2008$reporting_unit = as.character(cong.2008$reporting_unit)
  cong.2008$reporting_unit_orig <- cong.2008$reporting_unit
  cong.2008$reporting_unit = ifelse(cong.2008$reporting_unit == "","Wards 1",cong.2008$reporting_unit)
  cong.2008$reporting_unit = toupper(cong.2008$reporting_unit)
  cong.2008$county = toupper(cong.2008$county)
  
  cong.2008 <- subset(cong.2008, reporting_unit != "COUNTY TOTALS:")
  cong.2008 <- subset(cong.2008, county != "OFFICE TOTALS:")
  
  # Any commas in numbers will wreck all calculations, need to remove them
  cong.2008 <- as.data.frame(lapply(cong.2008, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(cong.2008$reporting_unit)*2,2)
  
  require(stringr)
  cong.2008$reporting_unit[which(grepl("[0-9]", cong.2008$reporting_unit) == F)] <- paste(cong.2008$reporting_unit[which(grepl("[0-9]", cong.2008$reporting_unit) == F)], "1", sep = " ")
  
  cong.2008$reporting_unit <- gsub("WARD WD", "WARD", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("\\bWD\\b", "WARD", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("\\bWDS\\b", "WARD", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("\\bWARD\\b", "WARD", cong.2008$reporting_unit)
  
  cong.2008$reporting_unit <- gsub("WARD([0-9])", "WARD \\1", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("WARDS([0-9])", "WARDS \\1", cong.2008$reporting_unit)
  
  
  cong.2008$municipality_name <- unlist(str_split(str = cong.2008$reporting_unit, " [0-9]", n = 2))[odd_indexes]
  cong.2008$municipality_name <- gsub(" WARDS","", cong.2008$municipality_name)
  cong.2008$municipality_name <- gsub(" WARD","", cong.2008$municipality_name)
  cong.2008$municipality_name <- na.locf(cong.2008$municipality_name)
  cong.2008$municipality_orig = cong.2008$municipality_name
  
  
  # cong.2008$municipality_name = with(cong.2008, paste(municipality_type, "of", municipality,sep=" "))
  # cong.2008$reporting_unit = with(cong.2008, paste(municipality_name, reporting_unit))
  # colnames(cong.2008)[1:3] = c("county","municipality_name","reporting_unit")
  # cong.2008$municipality_name = toupper(cong.2008$municipality_name)
  
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  cong.2008$municipality_name[grep("TOWN OF WINDSOR", cong.2008$municipality_name)] = "VILLAGE OF WINDSOR"
  cong.2008$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", cong.2008$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # cong.2008$county[grep("TOWN OF MAINE", cong.2008$municipality_name)] = "MARATHON"
  cong.2008$municipality_name[grepl("TOWN OF MAINE", cong.2008$municipality_name) &
                                grepl("MARATHON", cong.2008$county)
                              ] = "VILLAGE OF MAINE"
  # cong.2008$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", cong.2008$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  cong.2008$municipality_name[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", cong.2008$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  cong.2008$municipality_name[grepl("TOWN OF MENASHA WARDS 3, 5, 6", cong.2008$reporting_unit)
                              ] = "VILLAGE OF FOX CROSSING"
  cong.2008$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", cong.2008$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  cong.2008$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", cong.2008$reporting_unit)
                           ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  # cong.2008.look = with(cong.2008, data.frame(county, municipality_name,
  #                                         reporting_unit, muni_county))
  # colnames(cong.2008.look) = c("county","municipality","reporting_unit",
  #                            "muni_county")
  # 
  # require(zoo)
  # cong.2008$congress_district = na.locf(cong.2008$congress_district)
  cong.2008$county = na.locf(cong.2008$county)
  trim.trailing <- function (x) sub("\\s+$", "", x)
  cong.2008$county = trim.trailing(cong.2008$county)
  
  cong.2008$municipality_name = toupper(cong.2008$municipality_name)
  cong.2008$county = toupper(cong.2008$county)
  
  cong.2008$muni_county = paste(cong.2008$municipality_name, cong.2008$county)
  cong.2008$muni_county_orig = paste(cong.2008$municipality_orig, cong.2008$county)
  ...
  cong.2008$reporting_unit <- gsub("WARD WD", "WARD", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("\\bWD\\b", "WARD", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("\\bWDS\\b", "WARDS", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("\\bWARD\\b", "WARD", cong.2008$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  
  cong.2008$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("([0-9]),([0-9])", "\\1\\2", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", cong.2008$reporting_unit)
  cong.2008$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", cong.2008$reporting_unit)
  # cong.2008$reporting_unit = gsub("WARDS","WARD",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("  "," ",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("&","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("\\bAND\\b","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub(",","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub(" - ","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub(" -","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("- ","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub(" ","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("WD","",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("COMBINED","",cong.2008$reporting_unit)
  
  cong.2008$reporting_unit = toupper(cong.2008$reporting_unit)
  cong.2008$reporting_unit = paste(cong.2008$reporting_unit, cong.2008$county, sep = "-")
  
  cong.2008$reporting_unit = gsub(" ","-",cong.2008$reporting_unit)
  cong.2008$reporting_unit = gsub("--","-",cong.2008$reporting_unit)
  
  cong.2008.look = with(cong.2008, data.frame(county, municipality_name, municipality_orig, reporting_unit, reporting_unit_orig, muni_county))
  colnames(cong.2008.look) = c("county","municipality","municipality_orig","reporting_unit", "reporting_unit_orig", "muni_county")
  
  # rm(cong.20082,cong.20083,cong.20084)
  
  cong.2008.v = cong.2008
  # cong.2008.v$county = NULL
  cong.2008.v$county = NULL
  cong.2008.v$municipality_name = NULL
  cong.2008.v$municipality_type = NULL
  cong.2008.v$municipalityno = NULL
  cong.2008.v$municipality = NULL
  cong.2008.v$municipality_orig = NULL
  # cong.2008.v$reporting_unit = NULL
  cong.2008.v$order = NULL
  cong.2008.v$hindi = NULL
  # cong.2008.v$assemblydistrict = NULL
  # cong.2008.v = subset(  cong.2008.v, original_or_recount == "Recount")
  
  cong.2008.l <- cong.2008.v
  cong.2008.l$muni_county = NULL
  cong.2008.l$reporting_unit <- NULL
  cong.2008.l$muni_county_orig = NULL
  cong.2008.l$reporting_unit_orig <- NULL
  cong.2008.l$votes.tot <- NULL
  cong.2008.l$x_3 <- NULL
  cong.2008.l = gather(cong.2008.l, cand, votes_rec)
  cong.2008.l$reporting_unit <- rep(cong.2008.v$reporting_unit)
  # colnames(cong.2008.l) = c("reporting_unit", "cand", "votes.rec")
  
  # cong.2008.l$original_or_recount = NULL
  
  cong.2008.l$cand.group = NA
  cong.2008.l$cand.group[grep("democrat",cong.2008.l$cand)] = "democrat"
  cong.2008.l$cand.group[grep("repub",cong.2008.l$cand)] = "republican"
  cong.2008.l$cand.group[grep("libertar",cong.2008.l$cand)] = "other"
  cong.2008.l$cand.group[grep("independ",cong.2008.l$cand)] = "other"
  cong.2008.l$cand.group[grep("na",cong.2008.l$cand)] = "other"

  # Joining back county and municipality names
  cong.2008.l = plyr::join(cong.2008.l, cong.2008.look, by = "reporting_unit", match = "first")
  
  cong.2008.l$votes_rec = as.numeric(as.integer(cong.2008.l$votes_rec))
  
  
  tot.votes.look.group = dplyr::group_by(cong.2008.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec,na.rm=T)
  )
  
  cong.2008.l = plyr::join(cong.2008.l,tot.votes.look,by="reporting_unit",match="first")
  cong.2008.l$votes_perc = (cong.2008.l$votes_rec / cong.2008.l$votes_tot) * 100
  
  cong.2008.l$muni_county = paste(cong.2008.l$municipality, cong.2008.l$county)
  
  # cong.2008.l$votes_perc = (cong.2008.l$votes_rec / cong.2008.l$votes_tot) * 100
  
  cong.2008.l$votes_tot <- NULL
  cong.2008.melt = melt(cong.2008.l)
  cong.2008.cast = dcast(cong.2008.melt, reporting_unit + reporting_unit_orig + muni_county + municipality + municipality_orig + county ~ variable + cand.group, sum,na.rm=T)
  cong.2008.l = cong.2008.cast
  cong.2008.l[,grep("_NA", names(cong.2008.l))] <- NULL
  
  cong.2008.l = plyr::join(cong.2008.l,tot.votes.look,by="reporting_unit",match="first")
  
  # cong.2008.l$trump_prim_won = with(cong.2008.l,
  #                                        ifelse(votes_perc_trump > votes_perc_kasich &
  #                                                 votes_perc_trump > votes_perc_cruz &
  #                                                 votes_perc_trump > votes_perc_other,1,0))
  # 
  # cong.2008.l$votes_tot_cruz = NULL
  # cong.2008.l$votes_tot_trump = NULL
  # cong.2008.l$votes_tot_kasich = NULL
  
  colnames(cong.2008.l)[grep("votes_tot_other",colnames(cong.2008.l))] = "votes_tot"
  
  # rm(cong.2008.cast,cong.2008.melt,
  # cong.2008.mun.group ,
  # cong.2008, cong.2008.look, cong.2008.v,
  # tot.votes.look,tot.votes.look.group)
  
} # Preparing basic data
{ # GAB  / EDL stats
  gabdata.2008.cong = read.xlsx2(
    "2008nov_gab190_statistics_xls_13418MOD.xls",
    stringsAsFactors = F,
    header = T,
    sheetIndex = 1,
  ...
  # names(gabdata.2008.cong)
  
  # Preparing basic data
  gabdata.2008.cong = clean_names(gabdata.2008.cong)
  ...
  # gabdata.2008.cong.g <- group_by(gabdata.2008.cong, hindi, county, municipality, reporting_unit)
  # gabdata.2008.cong.m <- dplyr::summarise_if(gabdata.2008.cong.g, is.numeric, sum, na.rm = T)
  
  # Removing some random weirdness
  
  gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "CITY OF BUFFALO CITY"] <- "CITY OF BUFFALO"
  # gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "CITY OF FOUNTAIN CITY"] <- "CITY OF FOUNTAIN"
  gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "VILLAGE OF DEFOREST"] <- "VILLAGE OF DE FOREST"
  gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "VILLAGE OF LAVALLE"] <- "VILLAGE OF LA VALLE"
  gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "TOWN OF LAND O-LAKES"] <- "TOWN OF LAND O'LAKES"
  gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "TOWN OF POY SIPPI"] <- "TOWN OF POYSIPPI"
  gabdata.2008.cong$municipality[gabdata.2008.cong$municipality == "TOWN OF SAINT LAWRENCE"] <- "TOWN OF ST LAWRENCE"
  
  gabdata.2008.cong$municipality <- gsub("\\.", "", gabdata.2008.cong$municipality) # for St Cloud and St marie, Nazianz
  
  gabdata.2008.cong$county <- gsub("LA CROSSE", "LACROSSE", gabdata.2008.cong$county) # for St Cloud and St marie
  
  gabdata.2008.cong$county <- gsub("MT", "MOUNT", gabdata.2008.cong$county) # for Mount Sterling
  ...
  gabdata.2008.cong$reporting_unit_orig = gabdata.2008.cong$reporting_unit
  # gabdata.2008.cong$reporting_unit = toupper(gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$municipality = toupper(gabdata.2008.cong$municipality)
  gabdata.2008.cong$county = toupper(gabdata.2008.cong$county)
  
  gabdata.2008.cong$county = gsub(" COUNTY","", gabdata.2008.cong$county)
  gabdata.2008.cong$muni_county = paste(gabdata.2008.cong$municipality, gabdata.2008.cong$county)
  
  gabdata.2008.cong$reporting_unit = paste(gabdata.2008.cong$municipality,gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = toupper(gabdata.2008.cong$reporting_unit)
  
  # The following is fixing a problem I noticed later when trying to match with the 2016 voter df
  gabdata.2008.cong$municipality[grep("TOWN OF WINDSOR", gabdata.2008.cong$municipality)] = "VILLAGE OF WINDSOR"
  gabdata.2008.cong$reporting_unit = gsub("TOWN OF WINDSOR","VILLAGE OF WINDSOR", gabdata.2008.cong$reporting_unit)
  
  # The town in Maine became a village in 2015, and is now in marathon county
  # gabdata.2008.cong$county[grep("TOWN OF MAINE", gabdata.2008.cong$municipality)] = "MARATHON"
  gabdata.2008.cong$municipality[grepl("TOWN OF MAINE", gabdata.2008.cong$municipality) &
                                   grepl("MARATHON", gabdata.2008.cong$county)
                                 ] = "VILLAGE OF MAINE"
  # gabdata.2008.cong$reporting_unit = gsub("TOWN OF MAINE","VILLAGE OF MAINE", gabdata.2008.cong$reporting_unit)
  
  # the town of Menasha was retained only for wards (8-10, 11-13). The rest (1-7) became the village of
  # Fox Crossing, Winnebago
  gabdata.2008.cong$municipality[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2008.cong$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2008.cong$municipality[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2008.cong$reporting_unit)
                                 ] = "VILLAGE OF FOX CROSSING"
  gabdata.2008.cong$reporting_unit[grepl("TOWN OF MENASHA WARDS 1, 2, 4, 7", gabdata.2008.cong$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 1-2,4,7"
  gabdata.2008.cong$reporting_unit[grepl("TOWN OF MENASHA WARDS 3, 5, 6", gabdata.2008.cong$reporting_unit)
                                   ] = "VILLAGE OF FOX CROSSING WARDS 3,5-6"
  
  gabdata.2008.cong$muni_county = paste(gabdata.2008.cong$municipality, gabdata.2008.cong$county)
  gabdata.2008.cong$reporting_unit = paste(gabdata.2008.cong$reporting_unit, gabdata.2008.cong$county)
  
  gabdata.2008.cong$reporting_unit <- gsub("WARD WD", "WARD", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("\\bWD\\b", "WARD", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("\\bWDS\\b", "WARDS", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("\\bWARD\\b", "WARD", gabdata.2008.cong$reporting_unit)
  
  # https://stackoverflow.com/questions/26896971/add-space-between-two-letters-in-a-string-in-r
  
  gabdata.2008.cong$reporting_unit <- gsub("([A-Z]),([0-9])", "\\1\\2", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("([0-9])-([0-9])", "\\1\\2", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("([0-9]),([0-9])", "\\1-\\2", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("([0-9]),([0-9])", "\\1-\\2", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("WARD([0-9])", "WARD-\\1", gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit <- gsub("WARDS([0-9])", "WARDS-\\1", gabdata.2008.cong$reporting_unit)
  # gabdata.2008.cong$reporting_unit = gsub("WARDS","WARD",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub("  "," ",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub("&","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub("\\bAND\\b","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub(",","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub(" - ","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub(" -","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub("- ","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub(" ","-",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub("WD","",gabdata.2008.cong$reporting_unit)
  gabdata.2008.cong$reporting_unit = gsub("COMBINED","",gabdata.2008.cong$reporting_unit)
  ...
  gabdata.2008.cong$reporting_unit[gabdata.2008.cong$reporting_unit == "CITY-OF-FITCHBURG-WARDS-10-14-DANE"] <- "CITY-OF-FITCHBURG-WARDS-10-12-14-DANE"
  
  somers_wards <- c("TOWN-OF-SOMERS-WARD-7-KENOSHA", "TOWN-OF-SOMERS-WARD-8-KENOSHA")  
  cong.2008.l <- replace_repunit_rows(cong.2008.l, somers_wards, "TOWN-OF-SOMERS-WARDS-7-8-KENOSHA")  
  
  bristol_wards <- c("TOWN-OF-BRISTOL-WARDS-5-7-KENOSHA", "TOWN-OF-BRISTOL-WARD-6-KENOSHA")  
  cong.2008.l <- replace_repunit_rows(cong.2008.l, bristol_wards, "TOWN-OF-BRISTOL-WARDS-5-7-KENOSHA")
  
  ixonia_wards <- c("TOWN-OF-IXONIA-WARDS-1-3-4-JEFFERSON", "TOWN-OF-IXONIA-WARD-2-JEFFERSON")  
  cong.2008.l <- replace_repunit_rows(cong.2008.l, ixonia_wards, "TOWN-OF-IXONIA-WARDS-1-4-JEFFERSON")
  
  fitchburg_wards <- c("CITY-OF-FITCHBURG-WARDS-4-6-DANE", "CITY-OF-FITCHBURG-WARD-5-DANE")  
  cong.2008.l <- replace_repunit_rows(cong.2008.l, fitchburg_wards, "CITY-OF-FITCHBURG-WARDS-4-6-DANE")
  
  ashland_wards <- c("CITY-OF-ASHLAND-WARD-1-ASHLAND","CITY-OF-ASHLAND-WARD-2-ASHLAND","CITY-OF-ASHLAND-WARD-3-ASHLAND","CITY-OF-ASHLAND-WARD-4-ASHLAND", "CITY-OF-ASHLAND-WARD-5-ASHLAND","CITY-OF-ASHLAND-WARD-6-ASHLAND", "CITY-OF-ASHLAND-WARD-7-ASHLAND","CITY-OF-ASHLAND-WARD-8-ASHLAND", "CITY-OF-ASHLAND-WARD-9-ASHLAND","CITY-OF-ASHLAND-WARD-10-ASHLAND","CITY-OF-ASHLAND-WARD-11-ASHLAND") 
  cong.2008.l <- replace_repunit_rows(cong.2008.l, ashland_wards, "CITY-OF-ASHLAND-WARDS-1-11-ASHLAND")
  
  gabdata.repunit <- gabdata.2008.cong
...
{ # Matching voting statistics with the election statistics
  ...
  # table(cong.2008.l$reporting_unit %in% gabdata.join$reporting_unit)
  
  cong.2008.new = plyr::join(cong.2008.l, gabdata.join, by = "reporting_unit")
  cong.2008.l = cong.2008.new
  
  cong.2008.l[is.na(cong.2008.l)] = NA
  rm(cong.2008.new)
  
  setdiff(cong.2008.l$reporting_unit, gabdata.join$reporting_unit)
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  cong.2008.dem = plyr::join(cong.2008.l, demographics.caps.join, by = "county", match = "first")
  ...
  # Also joining the data from the Election snapshot
  cong.2008.dem = plyr::join(cong.2008.dem, elec.snapshot.dem.join, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  ...
  off_vote_machines$muni_county = paste(code_3,code_2, sep = " ")
  #
  # unique(grepl("550101201",cong.2008.dem$MCD_FIPS))
  
  # setdiff(off_vote_machines$muni_county, cong.2008.dem$muni_county)
  # off_vote_machines$muni_county[grep("TOWN OF GEORGETOWN  POLK", off_vote_machines$muni_county)] = "TOWN OF GEORGETOWN POLK"
  # off_vote_machines$muni_county[grep("TOWN OF WINDSOR DANE", off_vote_machines$muni_county)] = "VILLAGE OF WINDSOR DANE"
  # off_vote_machines$muni_county[grep("TOWN OF MAINE MARATHON", off_vote_machines$muni_county)] = "VILLAGE OF MAINE MARATHON"
  
  # setdiff(cong.2008.dem$muni_county, off_vote_machines$muni_county) # Difference of 47
  # length(off_vote_machines$muni_county) # 1854
  # length(cong.2008.dem$muni_county) # 1900 Difference of 46. Plus the errant space = 47. Therefore, these places are simply not included in the official list?
  
  off_vote_machines_cut = off_vote_machines
  off_vote_machines$County = NULL
  off_vote_machines$Municipality = NULL
  
  cong.2008.dem = plyr::join(cong.2008.dem, off_vote_machines, by = "muni_county")
  
  cong.2008.dem$tch_model_off_full = NA
  cong.2008.dem$tch_model_off_full = with(cong.2008.dem, ifelse(tch_model_off == "Dominion (Premier)-Accuvote TSX",
                                                                "Dominion (Premier)-Accuvote TSX", cong.2008.dem$tch_model_off_full))
  cong.2008.dem$tch_model_off_full = with(cong.2008.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge",
                                                                "Dominion (Sequoia)/Command Central-Edge",cong.2008.dem$tch_model_off_full))
  cong.2008.dem$tch_model_off_full = with(cong.2008.dem, ifelse(tch_model_off == "Dominion (Sequoia)/Command Central-Edge; ES&S Automark",
                                                                "Dominion (Sequoia)/Command Central-Edge",cong.2008.dem$tch_model_off_full))
  cong.2008.dem$tch_model_off_full = with(cong.2008.dem, ifelse(tch_model_off == "ES&S iVotronic",
                                                                "ES&S iVotronic",cong.2008.dem$tch_model_off_full))
  cong.2008.dem$tch_model_off_full[is.na(cong.2008.dem$tch_model_off_full)] = "aNone"
  cong.2008.dem$tch_model_off_full = with(cong.2008.dem, ifelse(is.na(tch_use_off),
                                                                NA,cong.2008.dem$tch_model_off_full))
  
  colnames(cong.2008.dem)[grep("tch.use",colnames(cong.2008.dem),fixed = T)] = "tch_use_verivote"
  colnames(cong.2008.dem)[grep("os.use",colnames(cong.2008.dem),fixed = T)] = "os_use_verivote"
  colnames(cong.2008.dem)[grep("bmd.use",colnames(cong.2008.dem),fixed = T)] = "bmd_use_verivote"
  colnames(cong.2008.dem)[grep("X._collegedegreepersons25._2013",colnames(cong.2008.dem))] = "coll_deg_plus25_2013"
} # Putting in voting machines
{ # Making model df
  model.2008.cong.repunit.df = cong.2008.dem
  # model.2008.cong.df = subset(model.2008.cong.df, votes_tot > 19)
  
  # Following Nate Cohn's method: https://twitter.com/Nate_Cohn/status/801226924156719104/photo/1?ref_src=twsrc%5Etfw
  # I found it's better to use the non-logged version of pop.sq.mile.2010
  model.2008.cong.repunit.df$log.pop.sq.mile.2010 = log(model.2008.cong.repunit.df$pop_sq_mile_2010)
  ...
  ##
  # sort(names(model.2008.cong.repunit.df))
  
  model.2008.cong.repunit.df2 = with(model.2008.cong.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, hindi, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop, os_prop, counted_absent_prop, tch_votes, rejected_absent_prop, absentee_undeliverable,
                                                                            sameday_reg_prop, total_ballots,registrants,late_registrants, votes_tot,
                                                                            county_paper_or_paperplusmachine,
                                                                            ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                            pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
  ...
  rownames(model.2008.cong.repunit.df2) = model.2008.cong.repunit.df2$reporting_unit
  
  model.2008.cong.repunit.df = model.2008.cong.repunit.df2
  
  # model.2008.cong.repunit.df$MCD_FIPS = as.character(model.2008.cong.repunit.df$MCD_FIPS)
  ...
  model.2008.cong.repunit.df = plyr::join(model.2008.cong.repunit.df,tig_dems_join, by = "hindi", match = "first")
...
{ ### Creating a carto-presentable map
  ...
  # wisc.join = wisc.spat.o.cut
  # 
  # wisc.join@data = plyr::join(wisc.join@data, model.2008.cong.repunit.df, by = "muni_county", match = "first")
  # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
  # wisc.join@data$county = NULL
  # wisc.join@data$MCD_FIPS.1 = NULL
  # 
  ...
  # writeOGR(
  #   wisc.join,
  #   "Prepped files/wisc.present.rep2008.cong.geojson",
  #   layer = "wisc.join",
  #   driver = "GeoJSON"
  # )
} # Creating a carto-presentable map - not nec

# write.csv(model.2008.cong.df,"model_2008_pres.csv")
write.csv(model.2008.cong.repunit.df,"Prepped files/2008/model_2008_cong_repunit.csv")

# Joining all final model files -------------------------------------------

# I won't include the Secretary of State as they have had the same guy since 1974

setwd("C:/Users/s_cas/Dropbox (NEF Consulting)/2016 voting election county results/Wisconsin/All prep together/All processed files")

require(data.table)
model_2008_pres_repunit <- fread("Prepped files/2008/model_2008_pres_repunit.csv")
model_2008_pres_repunit$contest <- "2008_pres"
model_2008_pres_repunit$contest_year <- "2008"
model_2008_pres_repunit$election_type <- "President"
model_2008_pres_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2008_cong_repunit <- fread("Prepped files/2008/model_2008_cong_repunit.csv")
model_2008_cong_repunit$contest <- "2008_cong"
model_2008_cong_repunit$contest_year <- "2008"
model_2008_cong_repunit$election_type <- "Congress"
model_2008_cong_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2010_cong_repunit <- fread("Prepped files/2010/model_2010_cong_repunit.csv")
model_2010_cong_repunit$contest <- "2010_cong"
model_2010_cong_repunit$contest_year <- "2010"
model_2010_cong_repunit$election_type <- "Congress"
model_2010_cong_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2010_sen_repunit <- fread("Prepped files/2010/model_2010_sen_repunit.csv")
model_2010_sen_repunit$contest <- "2010_sen"
model_2010_sen_repunit$contest_year <- "2010"
model_2010_sen_repunit$election_type <- "Senate"
model_2010_sen_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2010_gov_repunit <- fread("Prepped files/2010/model_2010_gov_repunit.csv")
model_2010_gov_repunit$contest <- "2010_gov"
model_2010_gov_repunit$contest_year <- "2010"
model_2010_gov_repunit$election_type <- "Governor"
model_2010_gov_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2012_pres_repunit <- fread("Prepped files/2012/model_2012_pres_repunit.csv")
model_2012_pres_repunit$contest <- "2012_pres"
model_2012_pres_repunit$contest_year <- "2012"
model_2012_pres_repunit$election_type <- "President"
model_2012_pres_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2012_sen_repunit <- fread("Prepped files/2012/model_2012_sen_repunit.csv")
model_2012_sen_repunit$contest <- "2012_sen"
model_2012_sen_repunit$contest_year <- "2012"
model_2012_sen_repunit$election_type <- "Senate"
model_2012_sen_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2012_cong_repunit <- fread("Prepped files/2012/model_2012_cong_repunit.csv")
model_2012_cong_repunit$contest <- "2012_cong"
model_2012_cong_repunit$contest_year <- "2012"
model_2012_cong_repunit$election_type <- "Congress"
model_2012_cong_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2014_cong_repunit <- fread("Prepped files/2014/model_2014_cong_repunit.csv")
model_2014_cong_repunit$contest <- "2014_cong"
model_2014_cong_repunit$contest_year <- "2014"
model_2014_cong_repunit$election_type <- "Congress"
model_2014_cong_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2014_gov_repunit <- fread("Prepped files/2014/model_2014_gov_repunit.csv")
model_2014_gov_repunit$contest <- "2014_gov"
model_2014_gov_repunit$contest_year <- "2014"
model_2014_gov_repunit$election_type <- "Governor"
model_2014_gov_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

#

# model_2016_reprim_repunit <- fread("Prepped files/2016/model_reprim_repunit.csv")
# model_2016_reprim_repunit$contest <- "2016_reprim"
# model_2016_reprim_repunit$contest_year <- "2016"
# model_2016_reprim_repunit$election_type <- "Primary"
# model_2016_reprim_repunit$trump_vs_clinton <- "Not Trump vs Clinton"
# 
# 
# model_2016_demprim_repunit <- fread("Prepped files/2016/model_demprim_repunit.csv")
# model_2016_demprim_repunit$contest <- "2016_demprim"
# model_2016_demprim_repunit$contest_year <- "2016"
# model_2016_demprim_repunit$election_type <- "Primary"
# model_2016_demprim_repunit$trump_vs_clinton <- "Not Trump vs Clintonn"


model_2016_pres_repunit <- fread("Prepped files/2016/model_2016_pres_repunit.csv")
model_2016_pres_repunit$contest <- "2016_pres"
model_2016_pres_repunit$contest_year <- "2016"
model_2016_pres_repunit$election_type <- "President"
model_2016_pres_repunit$trump_vs_clinton <- "Trump vs Clinton"


model_2016_sen_repunit <- fread("Prepped files/2016/model_2016_sen_repunit.csv")
model_2016_sen_repunit$contest <- "2016_sen"
model_2016_sen_repunit$contest_year <- "2016"
model_2016_sen_repunit$election_type <- "Senate"
model_2016_sen_repunit$trump_vs_clinton <- "Not Trump vs Clinton"


model_2016_cong_repunit <- fread("Prepped files/2016/model_2016_cong_repunit.csv")
model_2016_cong_repunit$contest <- "2016_cong"
model_2016_cong_repunit$contest_year <- "2016"
model_2016_cong_repunit$election_type <- "Congress"
model_2016_cong_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

#

model_2018_gov_repunit <- fread("Prepped files/2018/model_2018_gov_repunit.csv")
model_2018_gov_repunit$contest <- "2018_gov"
model_2018_gov_repunit$contest_year <- "2018"
model_2018_gov_repunit$election_type <- "Governor"
model_2018_gov_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2018_sen_repunit <- fread("Prepped files/2018/model_2018_sen_repunit.csv")
model_2018_sen_repunit$contest <- "2018_sen"
model_2018_sen_repunit$contest_year <- "2018"
model_2018_sen_repunit$election_type <- "Senate"
model_2018_sen_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

model_2018_state_repunit <- fread("Prepped files/2018/model_2018_state_repunit.csv")
model_2018_state_repunit$contest <- "2018_state"
model_2018_state_repunit$contest_year <- "2018"
model_2018_state_repunit$election_type <- "Secretary of State"
model_2018_state_repunit$trump_vs_clinton <- "Not Trump vs Clinton"

#

final_model <- rbind(model_2008_pres_repunit, model_2008_cong_repunit,
                     model_2010_cong_repunit, model_2010_sen_repunit, model_2010_gov_repunit, 
                     model_2012_pres_repunit, model_2012_cong_repunit, model_2012_sen_repunit,
                     model_2014_cong_repunit, model_2014_gov_repunit,
                     model_2016_pres_repunit, model_2016_sen_repunit, model_2016_cong_repunit, 
                     model_2018_sen_repunit, model_2018_gov_repunit, model_2018_state_repunit)

write.csv(final_model, "Prepped files/all_contests_2008_2018_repunit.csv")

#

# Models repunit data 2012 2016 original - OLD NOT NEEDED --------------------------------------------------
...
# Models repunit data 2012 2016 recount - OLD NOT NEEDED --------------------------------------------------
