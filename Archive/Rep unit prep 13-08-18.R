...
names(gov.2014)[1:3] <- c("county","reporting_unit","votes.tot")
  ...
  gov.2014.look = with(gov.2014, data.frame(county, municipality_name, reporting_unit, muni_county))
  colnames(gov.2014.look) = c("county","municipality","reporting_unit",
                                    "muni_county")
...
{ # 2014 gab stats
  ...
  gabdata.2014.gov$reporting_unit = gsub("--","-",gabdata.2014.gov$reporting_unit)
   ...
   ward.gabdata.group = dplyr::group_by(gabdata.2014.gov,reporting_unit, muni_county, county, municipality)
...
{ # Matching voting statistics with the election statistics
  ...
  gov.2014.l2 = plyr::join(gov.2014.l, gabdata.join, by = "reporting_unit")
...
{ # Making model df
 ...
 model.2014.gov.repunit.df2 = with(model.2014.gov.repunit.df, data.frame(reporting_unit,county, muni_county, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop,counted_absent_prop,rejected_absent_prop,
                                 sameday_reg_prop, total_ballots,registrants,late_registrants,
                                 county_paper_or_paperplusmachine,
                                 pct_white, ratio_nocollege_college, log.pop.sq.mile.2010,
                                 pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
