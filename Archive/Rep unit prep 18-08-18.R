...
} # Matching voting statistics with election results
{
  ...
  demographics.caps.join <- with(demographics.caps, data.frame(county, county_paper_or_paperplusmachine, medianhouseholdincome_2009.2013, pop_sq_mile_2010, ratio_nocollege_college, coll_deg_plus25_2013))
...
{ # Making model df
  ...
  model.2014.cong.repunit.df = plyr::join(model.2014.cong.repunit.df,tig_dems_join, by = "MCD_FIPS")
...
{ # Getting municipality densities
  ...
  str(wisc.spat.o@data)
  
  wisc.spat.o@data$muni_county
  
  names(wisc.spat.o)[grep("pop", names(wisc.spat.o))]
