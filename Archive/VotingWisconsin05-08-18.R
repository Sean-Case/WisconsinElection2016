...
# Loading 2016 results - not needed ----------------------------------------------------
...
voting.df.2016.votecand.groups = dplyr::group_by(voting.df.2016,county,cand.group)
...
voting.df.2016.votecand.groups = dplyr::group_by(voting.df.2016.test,county)
voting.df.2016.votecand = dplyr::summarise(voting.df.2016.votecand.groups,
...
election.2016.group = dplyr::group_by(voting.df.2016, cand)
...
{ # Preparing basic data
  ...
  ward.2016.o.l = gather(ward.2016.o, cand, votes.rec)
  ward.2016.o.l$muni_county <- rep(ward.2016.o$muni_county)
  ...
  ward.2016.o.l <- subset(ward.2016.o.l, cand != "muni_county")


  # Joining back county and municipality names
  ward.2016.o.l = plyr::join(ward.2016.o.l, ward.2016.look, by = "muni_county", match = "first")

  ward.2016.o.l$votes.rec = as.numeric(as.integer(ward.2016.o.l$votes.rec))
  table(is.na(ward.2016.o.l$votes.rec))

  tot.votes.look.group = dplyr::group_by(ward.2016.o.l,muni_county)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes.tot = sum(votes.rec)
  )
  ward.2016.o.l = plyr::join(ward.2016.o.l,tot.votes.look,by="muni_county",match="first")
  ...
  ward.2016.o.mun.group = dplyr::group_by(ward.2016.o.l,municipality,muni_county,county,cand.group)
...
)
{
  ...
  colnames(ward.2016.regvote)[which(names(ward.2016.regvote) == "muni")] = "municipality"

  ### Limiting to municipality level
  ward.2016.regvote.mun.group = dplyr::group_by(ward.2016.regvote,muni_county,municipality,county)
  ...
  ##
{
  ...
  colnames(ward.2016.regvote)[which(names(ward.2016.regvote) == "muni_county")] = "muni_county_regvote_agreed"
  ...
  ward.2016.o.mun = plyr::join(ward.2016.o.mun, ward.2016.o.l.join, by ="muni_county", match = "first")
...
{ # Loading 2012 ward vote data
  ...
  ward.12.v$senate_district = NULL
  ward.12.v$assembly_district = NULL

  
  
  ward.12.g <- ward.12.v
  ward.12.g$muni_county <- NULL
  str(ward.12.g)
  ward.12.l = gather(ward.12.g, cand, votes.rec)
  ward.12.l$muni_county <- rep(ward.12.v$muni_county)
  ...
  ward.12.l <- subset(ward.12.l, cand != "muni_county")

  # Joining back county and municipality names
  ward.12.l = plyr::join(ward.12.l, ward.12.look, by = "muni_county", match = "first")
  
  
  ward.12.l$votes.rec <- as.numeric(ward.12.l$votes.rec)
  

  tot.votes.look.group = dplyr::group_by(ward.12.l,muni_county)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes.tot = sum(votes.rec)
  )
  
  str(ward.12.l$votes.rec)
  
  ward.12.l = plyr::join(ward.12.l,tot.votes.look,by="muni_county",match="first")
  ward.12.l$votes.perc = (as.numeric(ward.12.l$votes.rec) / as.numeric(ward.12.l$votes.tot)) * 100

  ward.12.l$muni_county = paste(ward.12.l$municipality, ward.12.l$county)

  ward.12.mun.group = dplyr::group_by(ward.12.l,municipality,muni_county,county,cand,cand.group)
...
# Now joining the 2012 data back into the 2016 data
{
 ...
 #  [10] "VILLAGE OF KEWASKUM FOND DU LAC" has 0 votes
  ...
  # [1] "VILLAGE OF HARRISON CALUMET" "VILLAGE OF SOMERS KENOSHA"
  ...
  vote.2012.diffs.join = plyr::join(vote.2012.diffs.join,ward.12.mun.join,by = "municipality")

  back.to.votes.df.join = with(vote.2012.diffs.join, data.frame(muni_county,muni_county_2012_agreed))

  ward.2016.o.mun = plyr::join(ward.2016.o.mun, back.to.votes.df.join, by = "muni_county")
  ...
  ward.12.mun.votes.group = dplyr::group_by(ward.12.mun,cand.group,muni_county)
  ...
  ward.2016.o.mun.g = plyr::join(ward.2016.o.mun.votes.g.cast, ward.12.mun.votes.g.cast, by = "muni_county")
...
{
  ward.2016.2012.o = plyr::join(ward.2016.2012.o, ward.2016.look, by = "muni_county", match = "first")
}

# Checking ending digits of votes # Not needed -----------------------------------------
...
ward.2016.o.l = plyr::join(ward.2016.o.l,vot.equip.join, by = "muni_county_machines_agreed")
...
ward.2016.2012.test = plyr::join(ward.2016.2012.o, county.machines.join, by = "muni_county", match = "first")

ward.2016.2012.o = ward.2016.2012.test
ward.2016.2012.o$county_use_opt_scan[is.na(ward.2016.2012.o$county_use_opt_scan)] = 0

# Loading voting machine data original (Verified voting) # dont run ---------------------------
...
# ward.2016.2012.o = plyr::join(ward.2016.2012.o, back.to.votes.df.join, by = "muni_county")
#
...
# # vot.equip.veri.group = dplyr::group_by(vot.equip.veri.spr,muni_county_veri_mach_agreed)
...
#
# vot.equip.veri.final = plyr::join(vot.equip.veri.cast,vot.equip.veri.cast.base, by = "muni_county_veri_mach_agreed", match = "first")
# vot.equip.veri.final = plyr::join(vot.equip.veri.final,vot.equip.veri.spr.base, by = "muni_county_veri_mach_agreed", match = "first")
#
# ward.2016.2012.o.test = plyr::join(ward.2016.2012.o, vot.equip.veri.final, by = "muni_county_veri_mach_agreed")
#
...
wisc.spat.j@data = plyr::join(wisc.spat.d.df@data, ward.2016.2012.o, by = "MCD_FIPS", match = "first")
...
{ # Preparing basic data
  ...
  ward.2016.r = subset(ward.2016.v, original_or_recount == "Recount")
  ward.2016.r = subset(ward.2016.v, original_or_recount == "Original")
  ...
  ward.2016.r.l = gather(ward.2016.r, cand, votes.rec)
  ward.2016.r.l$muni_county <- rep(ward.2016.r$muni_county)
  ...
  ward.2016.r.l <- subset(ward.2016.r.l, cand != "muni_county")
  
  
  # Joining back county and municipality names
  ward.2016.r.l = plyr::join(ward.2016.r.l, ward.2016.look, by = "muni_county", match = "first")
  
  ward.2016.r.l$votes.rec = as.numeric(as.integer(ward.2016.r.l$votes.rec))
  table(is.na(ward.2016.r.l$votes.rec))
  
  tot.votes.look.group = dplyr::group_by(ward.2016.r.l,muni_county)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes.tot = sum(votes.rec)
  )
  ward.2016.r.l = plyr::join(ward.2016.r.l,tot.votes.look,by="muni_county",match="first")
  ...
  ward.2016.r.mun.group = dplyr::group_by(ward.2016.r.l,municipality,muni_county,county,cand.group)
...
)
{
  ...
  ward.2016.r.mun = plyr::join(ward.2016.r.mun, ward.2016.r.l.join, by ="muni_county", match = "first")
...
# Now joining the 2012 data back into the 2016 data
{
  ...
  ward.2016.r.mun = plyr::join(ward.2016.r.mun, back.to.votes.df.join, by = "muni_county")
  ...
  ward.2016.r.mun.g = plyr::join(ward.2016.r.mun.votes.g.cast, ward.12.mun.votes.g.cast, by = "muni_county")
...
# Rejoining the county data to the ward 2016 2012 data
{
  ward.2016.2012.r = plyr::join(ward.2016.2012.r, ward.2016.look, by = "muni_county", match = "first")
}
...
ward.2016.r.l = plyr::join(ward.2016.r.l,vot.equip.join, by = "muni_county_machines_agreed")
...
ward.2016.2012.test = plyr::join(ward.2016.2012.r, county.machines.join, by = "muni_county", match = "first")
...
# veri.mach.diffs.join = plyr::join(veri.mach.diffs.join,ward.2016.veri.mach.join,by = "municipality")
# back.to.votes.df.join = with(veri.mach.diffs.join, data.frame(muni_county,muni_county_veri_mach_agreed))
# ward.2016.2012.r = plyr::join(ward.2016.2012.r, back.to.votes.df.join, by = "muni_county")
#
...
# vot.equip.veri.final = plyr::join(vot.equip.veri.final,vot.equip.veri.spr.base, by = "muni_county_veri_mach_agreed", match = "first")
#
# ward.2016.2012.r.test = plyr::join(ward.2016.2012.r, vot.equip.veri.final, by = "muni_county_veri_mach_agreed")
#
...
# Joining data together
wisc.spat.j@data = plyr::join(wisc.spat.d.df@data, ward.2016.2012.r, by = "MCD_FIPS", match = "first")
...
# Rejoining the county data to the ward 2016 2012 data
{
  repunit.2016.2012.o = plyr::join(repunit.2016.2012.o, ward.2016.look, by = "muni_county", match = "first")
  
  repunit.2016.2008.o = plyr::join(repunit.2016.2008.o, ward.2016.look, by = "muni_county", match = "first")
}

# 2014 Governor and Paul Ryan district ------------------------------------
...
# Get the municipality names from the reporting unit names    
  ...
  {  # Now joining the 2014 data back into the 2016 data
    ...
    setdiff(unique(ward.14.repu$reporting_unit), unique(ward.2016.o.repu$reporting_unit))
    setdiff(unique(ward.2016.o.repu$reporting_unit), unique(ward.14.repu$reporting_unit))
    ...
    differences = setdiff(unique(ward.2016.o.repu$reporting_unit),unique(ward.14.repu$reporting_unit))
    vote.2014.diffs = subset(ward.2016.o.repu, reporting_unit %in% differences)
    ...
    # ward.2016.o.repu = plyr::join(ward.2016.o.repu, back.to.votes.df.join, by = "reporting_unit")
    # 
    # ward.2016.o.repu$reporting_unit_2014_agreed = as.character(ward.2016.o.repu$reporting_unit_2014_agreed)
    # 
    # ward.2016.o.repu$reporting_unit_2014_agreed = ifelse(is.na(ward.2016.o.repu$reporting_unit_2014_agreed),
    #                                                  ward.2016.o.repu$reporting_unit,ward.2016.o.repu$reporting_unit_2014_agreed)
    # 
    ...
    ward.2016.o.repu.g = plyr::join(ward.2016.o.repu.votes.g.cast, ward.14.repu.votes.g.cast, by = "reporting_unit")
    
    # 2014 turnout is calculated on the (big?) assumption that the number of registered
    # voters was the same in 2014 as in 2016
    # ward.2016.o.repu.g$turnout.2014 =
    # with(ward.2016.o.repu.g, votes.tot2014 / voters.tot2016) * 100
    
    ward.2016.o.repu.g$dem.perc2014 =
      with(ward.2016.o.repu.g,
           democrat_votes.rec2014 / votes.tot2014) * 100
    ward.2016.o.repu.g$rep.perc2014 =
      with(ward.2016.o.repu.g,
           republican_votes.rec2014 / votes.tot2014) * 100
    ward.2016.o.repu.g$oth.perc2014 =
      with(ward.2016.o.repu.g,
           other_votes.rec2014 / votes.tot2014) * 100
    
    ward.2016.o.repu.g$dem.change.2014.num =
      with(ward.2016.o.repu.g,
           democrat_votes.rec - democrat_votes.rec2014)
    ward.2016.o.repu.g$rep.change.2014.num =
      with(ward.2016.o.repu.g,
           republican_votes.rec - republican_votes.rec2014)
    ward.2016.o.repu.g$oth.change.2014.num =
      with(ward.2016.o.repu.g,
           other_votes.rec - other_votes.rec2014)
    
    ward.2016.o.repu.g$dem.change.2014.perc =
      with(ward.2016.o.repu.g,
           democrat_votes.perc -  dem.perc2014)
    ward.2016.o.repu.g$rep.change.2014.perc =
      with(ward.2016.o.repu.g,
           republican_votes.perc - rep.perc2014)
    ward.2016.o.repu.g$oth.change.2014.perc =
      with(ward.2016.o.repu.g,
    ...
    repunit.2016.2014.o = ward.2016.o.repu.g
    
    repunit.2016.2014.o$demwinner = NA
    repunit.2016.2014.o$demwinner = ifelse(repunit.2016.2014.o$democrat_votes.perc >
                                             repunit.2016.2014.o$republican_votes.perc &
                                             !is.na(repunit.2016.2014.o$democrat_votes.perc), 1,0)
    repunit.2016.2014.o$demwinner = as.integer(repunit.2016.2014.o$demwinner)
    
    repunit.2016.2014.o$demwinner2014 = NA
    repunit.2016.2014.o$demwinner2014 = ifelse(repunit.2016.2014.o$dem.perc2014 >
                                                 repunit.2016.2014.o$rep.perc2014 &
                                                 !is.na(repunit.2016.2014.o$dem.perc2014), 1,0)
    repunit.2016.2014.o$demwinner2014 = as.integer(repunit.2016.2014.o$demwinner2014)
  } # Now joining the 2014 data back into the 2016 data
} # Loading 2014 ward vote data

# Loading voting machine data original repunit (Wisconsin official NOT NEEDED?) ---------------------------------------------
# This file from here: http://elections.wi.gov/elections-voting/voting-equipment/voting-equipment-use
# Voting machine equipment for 13 September 2016
vot.equip = read.xlsx(
  "voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
...
# Ward level data 2012 and 2016 recount repunit ----------------------------------------------------
ward.2016 = read.csv(
  "wardwisconsin2016andrecount.csv",
  stringsAsFactors = F,
  header = T,
...
{ # Preparing basic data
  ...
  ward.2016.r.mun.group = dplyr::group_by(ward.2016.r.l,reporting_unit, municipality,muni_county,county,cand.group)
  ward.2016.r.mun = dplyr::summarise(ward.2016.r.mun.group,
                                     votes.rec2 = sum(votes.rec),
                                     votes.tot2 = mean(votes.tot)
  )
  colnames(ward.2016.r.mun)[6] = "votes.rec"
  colnames(ward.2016.r.mun)[7] = "votes.tot"
  
  ward.2016.r.mun$votes.perc = (ward.2016.r.mun$votes.rec / ward.2016.r.mun$votes.tot) * 100
  
  ward.2016.r.mun = ward.2016.r.mun[!is.na(ward.2016.r.mun$cand.group),]
} # Preparing basic data 2016
{
  ## Now bringing in the registered voter data.
  # ward.2016.regvote = read.csv(
  ...
  #   
  #   ### Limiting to municipality level
  #   ward.2016.regvote.mun.group = dplyr::group_by(ward.2016.regvote,reporting_unit,muni_county,municipality,county)
  #   ward.2016.regvote.mun = dplyr::summarise(ward.2016.regvote.mun.group,
  #                                            voter_count2 = sum(voter_count)
  #   )
  #   colnames(ward.2016.regvote.mun)[4] = "voters.tot"
...
{   # 2012 ward data
  ...
  { # Loading 2012 ward vote data
    ...
    ward.12.mun.group = dplyr::group_by(ward.12.l,reporting_unit,municipality,muni_county,county,cand.group)
    ward.12.mun = dplyr::summarise(ward.12.mun.group,
                                   votes.rec2 = sum(votes.rec,na.rm=T),
                                   votes.tot2 = mean(votes.tot,na.rm=T)
    )
  ...
  {  # Now joining the 2012 data back into the 2016 data
    grep("WARDS",ward.2016.r.mun$reporting_unit)
    
    ward.2016.r.mun$reporting_unit_orig =ward.2016.r.mun$reporting_unit
    ward.12.mun$reporting_unit_orig =ward.12.mun$reporting_unit 
    
    ward.2016.r.mun$reporting_unit = gsub("WARDS","WARD",ward.2016.r.mun$reporting_unit_orig)
    ward.12.mun$reporting_unit = gsub("WARDS","WARD",ward.12.mun$reporting_unit_orig)
    ward.2016.r.mun$reporting_unit = gsub("  "," ",ward.2016.r.mun$reporting_unit)
    ward.12.mun$reporting_unit = gsub("  "," ",ward.12.mun$reporting_unit)
    ward.2016.r.mun$reporting_unit = gsub("&","-",ward.2016.r.mun$reporting_unit)
    ward.12.mun$reporting_unit = gsub("&","-",ward.12.mun$reporting_unit)
    
    ward.2016.r.mun$reporting_unit = gsub("AND","-",ward.2016.r.mun$reporting_unit)
    ...
    ward.2016.r.mun$reporting_unit = gsub("WD","",ward.2016.r.mun$reporting_unit)
    ward.12.mun$reporting_unit = gsub("WD","",ward.12.mun$reporting_unit)
    
    ward.2016.r.mun$reporting_unit = gsub("COMBINED","",ward.2016.r.mun$reporting_unit)
    ward.12.mun$reporting_unit = gsub("COMBINED","",ward.12.mun$reporting_unit)
    
    ward.2016.r.mun$reporting_unit = gsub("--","-",ward.2016.r.mun$reporting_unit)
    ward.12.mun$reporting_unit = gsub("--","-",ward.12.mun$reporting_unit)
    
    setdiff(unique(ward.12.mun$reporting_unit), unique(ward.2016.r.mun$reporting_unit))
    setdiff(unique(ward.2016.r.mun$reporting_unit), unique(ward.12.mun$reporting_unit))
    ...
    differences = setdiff(unique(ward.2016.r.mun$reporting_unit),unique(ward.12.mun$reporting_unit))
    vote.2012.diffs = subset(ward.2016.r.mun, reporting_unit %in% differences)
    ...
    # 
    # 
    # vote.2012.diffs.join = plyr::join(vote.2012.diffs.join,ward.12.mun.join,by = "reporting_unit")
    # 
    # back.to.votes.df.join = with(vote.2012.diffs.join, data.frame(reporting_unit,reporting_unit_2012_agreed))
    # 
    # ward.2016.r.mun = plyr::join(ward.2016.r.mun, back.to.votes.df.join, by = "reporting_unit")
    # 
    # ward.2016.r.mun$reporting_unit_2012_agreed = as.character(ward.2016.r.mun$reporting_unit_2012_agreed)
    # 
    # ward.2016.r.mun$reporting_unit_2012_agreed = ifelse(is.na(ward.2016.r.mun$reporting_unit_2012_agreed),
    #                                                  ward.2016.r.mun$reporting_unit,ward.2016.r.mun$reporting_unit_2012_agreed)
    # 
    ## Now renaming the 2012 column for the final join
    # colnames(ward.12.mun)[2] = "muni_county_2012_agreed"
    
    ## Need to sum up the votes by county in 2012
    ward.12.mun.votes.group = dplyr::group_by(ward.12.mun,cand.group,reporting_unit,reporting_unit_orig,municipality, county)
    ward.12.mun.votes.g = dplyr::summarise(ward.12.mun.votes.group,
                                           votes.rec2012 = sum(votes.rec),
                                           votes.tot2012 = mean(votes.tot)
    ...
    ward.2016.r.mun.g = plyr::join(ward.2016.r.mun.votes.g.cast, ward.12.mun.votes.g.cast, by = "reporting_unit")
    ...
    repunit.2016.2012.r = ward.2016.r.mun.g
...
{   # 2008 ward data
  ...
  { # Loading 2008 ward vote data
    ...
    ward.08.l = gather(ward.08.v, reporting_unit);
    colnames(ward.08.l) = c("reporting_unit", "cand", "votes.rec")
    
    ward.08.l$cand.group = NA
    ward.08.l$cand.group[grep("obama",ward.08.l$cand)] = "democrat"
    ward.08.l$cand.group[grep("mccain",ward.08.l$cand)] = "republican"
    ward.08.l$cand.group[grep("rosa",ward.08.l$cand)] = "other"
    ward.08.l$cand.group[grep("wayne",ward.08.l$cand)] = "other"
    ward.08.l$cand.group[grep("stewart",ward.08.l$cand)] = "other"
    ward.08.l$cand.group[grep("robert",ward.08.l$cand)] = "other"
    ...
    ward.08.mun.group = dplyr::group_by(ward.08.l,reporting_unit,municipality,muni_county,county,cand.group)
    ward.08.mun = dplyr::summarise(ward.08.mun.group,
                                   votes.rec2 = sum(votes.rec,na.rm=T),
                                   votes.tot2 = mean(votes.tot,na.rm=T)
    )
    colnames(ward.08.mun)[6] = "votes.rec"
    colnames(ward.08.mun)[7] = "votes.tot"
    
    ward.08.mun$votes.perc = (ward.08.mun$votes.rec / ward.08.mun$votes.tot) * 100
    
    ward.08.mun = ward.08.mun[!is.na(ward.08.mun$cand.group),]
  } # 2008 data basic
  {  # Now joining the 2008 data back into the 2016 data
    # grep("WARDS",ward.2016.r.mun$reporting_unit)
    
    ward.2016.r.mun$reporting_unit_orig =ward.2016.r.mun$reporting_unit_orig
    ward.08.mun$reporting_unit_orig =ward.08.mun$reporting_unit 
    ...
    setdiff(unique(ward.08.mun$reporting_unit), unique(ward.2016.r.mun$reporting_unit))
    setdiff(unique(ward.2016.r.mun$reporting_unit), unique(ward.08.mun$reporting_unit))
    ...
    differences = setdiff(unique(ward.2016.r.mun$reporting_unit),unique(ward.08.mun$reporting_unit))
    vote.2008.diffs = subset(ward.2016.r.mun, reporting_unit %in% differences)
    
    vote.2008.diffs.join = with(vote.2008.diffs, data.frame(municipality, county, reporting_unit))
    vote.2008.diffs.join = vote.2008.diffs.join[!duplicated(vote.2008.diffs.join),]
    
    # ## Joining the reg.vote summarised municipalities back onto the voting municipalities
    # ward.08.mun.join = with(ward.08.mun, data.frame(municipality,reporting_unit))
    # colnames(ward.08.mun.join)[2] = "reporting_unit_2008_agreed"
    # ward.08.mun.join = ward.08.mun.join[!duplicated(ward.08.mun.join$reporting_unit_2008),]
    # 
    # 
    # 
    # vote.2008.diffs.join = plyr::join(vote.2008.diffs.join,ward.08.mun.join,by = "reporting_unit")
    # 
    # back.to.votes.df.join = with(vote.2008.diffs.join, data.frame(reporting_unit,reporting_unit_2008_agreed))
    # 
    # ward.2016.r.mun = plyr::join(ward.2016.r.mun, back.to.votes.df.join, by = "reporting_unit")
    # 
    # ward.2016.r.mun$reporting_unit_2008_agreed = as.character(ward.2016.r.mun$reporting_unit_2008_agreed)
    # 
    # ward.2016.r.mun$reporting_unit_2008_agreed = ifelse(is.na(ward.2016.r.mun$reporting_unit_2008_agreed),
    #                                                  ward.2016.r.mun$reporting_unit,ward.2016.r.mun$reporting_unit_2008_agreed)
    # 
    ## Now renaming the 2008 column for the final join
    # colnames(ward.08.mun)[2] = "muni_county_2008_agreed"
    
    ## Need to sum up the votes by county in 2008
    ward.08.mun.votes.group = dplyr::group_by(ward.08.mun,cand.group,reporting_unit,reporting_unit_orig,municipality, county)
    ward.08.mun.votes.g = dplyr::summarise(ward.08.mun.votes.group,
                                           votes.rec2008 = sum(votes.rec),
                                           votes.tot2008 = mean(votes.tot)
    ...
    # Melting the 2008 df to put the candidates on top
    ward.08.mun.votes.g.melt = melt(ward.08.mun.votes.g)
    ward.08.mun.votes.g.cast = dcast(ward.08.mun.votes.g.melt,
                                     reporting_unit ~ cand.group + variable, sum
    )
    ward.08.mun.votes.g.cast$democrat_votes.tot2008 = NULL
    ward.08.mun.votes.g.cast$republican_votes.tot2008 = NULL
    
    ward.08.mun.votes.g.cast.mean = dcast(ward.08.mun.votes.g.melt,
                                          reporting_unit ~ cand.group + variable, mean
    )
    ward.08.mun.votes.g.cast$other_votes.tot2008 = ward.08.mun.votes.g.cast.mean$other_votes.tot2008
    colnames(ward.08.mun.votes.g.cast)[grepl("votes.tot",colnames(ward.08.mun.votes.g.cast))] =
      "votes.tot2008"
    ...
    ward.2016.r.mun.g = plyr::join(ward.2016.r.mun.votes.g.cast, ward.08.mun.votes.g.cast, by = "reporting_unit")
    
    # 2008 turnout is calculated on the (big?) assumption that the number of registered
    # voters was the same in 2008 as in 2016
    # ward.2016.r.mun.g$turnout.2008 =
    # with(ward.2016.r.mun.g, votes.tot2008 / voters.tot2016) * 100
    
    ward.2016.r.mun.g$dem.perc2008 =
      with(ward.2016.r.mun.g,
           democrat_votes.rec2008 / votes.tot2008) * 100
    ward.2016.r.mun.g$rep.perc2008 =
      with(ward.2016.r.mun.g,
           republican_votes.rec2008 / votes.tot2008) * 100
    ward.2016.r.mun.g$oth.perc2008 =
      with(ward.2016.r.mun.g,
           other_votes.rec2008 / votes.tot2008) * 100
    
    ward.2016.r.mun.g$dem.change.2008.num =
      with(ward.2016.r.mun.g,
           democrat_votes.rec - democrat_votes.rec2008)
    ward.2016.r.mun.g$rep.change.2008.num =
      with(ward.2016.r.mun.g,
           republican_votes.rec - republican_votes.rec2008)
    ward.2016.r.mun.g$oth.change.2008.num =
      with(ward.2016.r.mun.g,
           other_votes.rec - other_votes.rec2008)
    
    ward.2016.r.mun.g$dem.change.2008.perc =
      with(ward.2016.r.mun.g,
           democrat_votes.perc -  dem.perc2008)
    ward.2016.r.mun.g$rep.change.2008.perc =
      with(ward.2016.r.mun.g,
           republican_votes.perc - rep.perc2008)
    ward.2016.r.mun.g$oth.change.2008.perc =
      with(ward.2016.r.mun.g,
           other_votes.perc - oth.perc2008)
    # 
    # ward.2016.r.mun.g$turnout.2016.perc =
    ...
    repunit.2016.2008.r = ward.2016.r.mun.g
...
# Rejoining the county data to the ward 2016 2012 data
{
  repunit.2016.2012.r = plyr::join(repunit.2016.2012.r, ward.2016.look, by = "muni_county", match = "first")
  
  repunit.2016.2008.r = plyr::join(repunit.2016.2008.r, ward.2016.look, by = "muni_county", match = "first")
}

# Loading voting machine data recount repunit (Wisconsin official NOT NEEDED?) ---------------------------------------------
# This file from here: http://elections.wi.gov/elections-voting/voting-equipment/voting-equipment-use
# Voting machine equipment for 13 September 2016
vot.equip = read.xlsx(
  "voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
...
setdiff(vot.equip.join$muni_county,
        repunit.2016.2012.r$muni_county)
...
# Loading in spatial data for recount repunit data -------------------------------------------------
require(rgdal); require(plotKML); require(sp); require(rgeos)
...
elections.100.years = plyr::join(elections.100.years, historical.turnout.join, by = "year")
...
{ # 2008 county data frame
  ...
  election.2008.final.group = dplyr::group_by(election.2008, COUNTY)
...
{ # 2004 county data frame
  ...
  election.2004.final.group = dplyr::group_by(election.2004, COUNTY.1)
...
{ # 2000 county data frame
  ...
  election.2000.final.group = dplyr::group_by(election.2000, COUNTY)
...
counties.2000.2016 = plyr::join(counties.2000.2016, county.winners, by = "county")
...
counties.2000.2016 = plyr::join(counties.2000.2016, counties.2000.2016.quant.join, by = "county",match="first")

# Join county vote machine to other county dfs -----------------------------------------


counties.2000.2016 = plyr::join(counties.2000.2016,machine.join,by="county")
county.summary.2016.final = plyr::join(county.summary.2016.final,machine.join,by="county")
county.summary.df = plyr::join(county.summary.df,machine.join,by="county")
...
write.csv(repunit.2016.2008.o, "repunit.2016.2008.o.csv")

write.csv(repunit.2016.2012.r, "repunit.2016.2012.r.csv")
write.csv(repunit.2016.2008.r, "repunit.2016.2008.r.csv")
...
repunit.2016.2008.o = read.csv("Prepped files/repunit.2016.2008.o.csv", stringsAsFactors = F)
repunit.2016.2012.r = read.csv("Prepped files/repunit.2016.2012.r.csv", stringsAsFactors = F)
repunit.2016.2008.r = read.csv("Prepped files/repunit.2016.2008.r.csv", stringsAsFactors = F)
...
county.2016.vs.2012 = plyr::join(county.2016.vs.2012, demographics, by = "county")
counties.2000.2016 = plyr::join(counties.2000.2016, demographics, by = "county")
...
ward.2016.2012.o.dem = plyr::join(ward.2016.2012.o, demographics.caps, by = "county", match = "first")

# Also joining the data from the Election snapshot

ward.2016.2012.o.dem = plyr::join(ward.2016.2012.o.dem, elec.snapshot.dem.wi, by = "county", match = "first")
...
county.prop.machines.grp = dplyr::group_by(ward.prop.machines, county)
...
county.summary.2016.final = plyr::join(county.summary.2016.final, county.prop.machines)

county.summary.2016.final = plyr::join(county.summary.2016.final,elec.snapshot.dem.wi, by = "county")
...
# Demographics from David Griffen and co
ward.2016.2012.r.dem = plyr::join(ward.2016.2012.r, demographics.caps, by = "county", match = "first")

# Also joining the data from the Election snapshot
ward.2016.2012.r.dem = plyr::join(ward.2016.2012.r.dem, elec.snapshot.dem.wi, by = "county", match = "first")
...
ward.aprdata.2016 = read.csv(
  "2016_presidential_and_general_election_el_190_2017_18402.csv",
  stringsAsFactors = F,
  header = T,
  strip.white = T
)
...
# Ward 83, City of Madison, Dane has 709 late ballots in registered voter pop of 1698
  # str(ward.aprdata.2016)

 # Preparing basic data
  ward.aprdata.2016 = clean_names(ward.aprdata.2016)
  ward.aprdata.2016$reporting_unit = toupper(ward.aprdata.2016$reporting_unit)
  ward.aprdata.2016$municipality = toupper(ward.aprdata.2016$municipality)
  ward.aprdata.2016$county = toupper(ward.aprdata.2016$county)

  ward.aprdata.2016$county = gsub(" COUNTY","", ward.aprdata.2016$county)

  ward.aprdata.2016$muni_county = paste(ward.aprdata.2016$municipality, ward.aprdata.2016$county)

  ward.aprdata.2016$registrants = as.integer(ward.aprdata.2016$registrants)
  ward.aprdata.2016$election_day_registrants = as.integer(ward.aprdata.2016$election_day_registrants)
  ward.aprdata.2016$total_ballots = as.integer(ward.aprdata.2016$total_ballots)
  ward.aprdata.2016$total_voters = as.integer(ward.aprdata.2016$total_voters)
  ward.aprdata.2016$paper_ballots = as.integer(ward.aprdata.2016$paper_ballots)
  ward.aprdata.2016$optical_scan_ballots = as.integer(ward.aprdata.2016$optical_scan_ballots)
  ward.aprdata.2016$dre = as.integer(ward.aprdata.2016$dre)
  ward.aprdata.2016$auto_mark = as.integer(ward.aprdata.2016$auto_mark)
  ward.aprdata.2016$absentee_issued = as.integer(ward.aprdata.2016$absentee_issued)
  ward.aprdata.2016$absentee_in_person = as.integer(ward.aprdata.2016$absentee_in_person)
  ward.aprdata.2016$received_by_election_day = as.integer(ward.aprdata.2016$received_by_election_day)
  ward.aprdata.2016$counted_2 = as.integer(ward.aprdata.2016$counted_2)
  
  ## Need to change some municipality - county names to match my other data frame
  ward.aprdata.2016$county[grep("CITY OF ASHLAND BAYFIELD",ward.aprdata.2016$muni_county)] = "ASHLAND"
  ward.aprdata.2016$county[grep("CITY OF COLUMBUS DODGE",ward.aprdata.2016$muni_county)] = "COLUMBIA"
  ward.aprdata.2016$county[grep("CITY OF HARTFORD DODGE",ward.aprdata.2016$muni_county)] = "WASHINGTON"
  ward.aprdata.2016$county[grep("CITY OF KAUKAUNA CALUMET",ward.aprdata.2016$muni_county)] = "OUTAGAMIE"
  ward.aprdata.2016$county[grep("CITY OF MILWAUKEE WASHINGTON",ward.aprdata.2016$muni_county)] = "MILWAUKEE"
  ward.aprdata.2016$county[grep("CITY OF MILWAUKEE WAUKESHA",ward.aprdata.2016$muni_county)] = "MILWAUKEE"

  # Wisconsin Dells Juneau doesn't exist in my db. Maybe a 0 population. Has a pop of 0 also in feb 17 data
  ward.aprdata.2016$county[grep("CITY OF WISCONSIN DELLS JUNEAU",ward.aprdata.2016$muni_county)] = "JUNEAU"

  ward.aprdata.2016$county[grep("VILLAGE OF GENOA CITY KENOSHA",ward.aprdata.2016$muni_county)] = "WALWORTH"
  ward.aprdata.2016$county[grep("VILLAGE OF HARRISON OUTAGAMIE",ward.aprdata.2016$muni_county)] = "CALUMET"
  ward.aprdata.2016$county[grep("VILLAGE OF HOWARD OUTAGAMIE",ward.aprdata.2016$muni_county)] = "BROWN"
  ward.aprdata.2016$county[grep("VILLAGE OF KEWASKUM FOND DU LAC",ward.aprdata.2016$muni_county)] = "WASHINGTON"
  ward.aprdata.2016$county[grep("VILLAGE OF ONTARIO MONROE",ward.aprdata.2016$muni_county)] = "VERNON"
  ward.aprdata.2016$county[grep("VILLAGE OF PULASKI OCONTO",ward.aprdata.2016$muni_county)] = "BROWN"
  ward.aprdata.2016$county[grep("VILLAGE OF ROCKLAND MONROE",ward.aprdata.2016$muni_county)] = "LA CROSSE"

  ward.aprdata.2016$muni_county = paste(ward.aprdata.2016$municipality, ward.aprdata.2016$county)
  
  ward.aprdata.2016$reporting_unit = paste(ward.aprdata.2016$municipality,ward.aprdata.2016$reporting_unit, ward.aprdata.2016$county)
  ward.aprdata.2016$reporting_unit_orig = ward.aprdata.2016$reporting_unit
  
  
  ward.aprdata.2016$reporting_unit = gsub("WARDS","WARD",ward.aprdata.2016$reporting_unit_orig)
  # ward.12.mun$reporting_unit = gsub("WARDS","WARD",ward.12.mun$reporting_unit_orig)
  ward.aprdata.2016$reporting_unit = gsub("  "," ",ward.aprdata.2016$reporting_unit)
  # ward.12.mun$reporting_unit = gsub("  "," ",ward.12.mun$reporting_unit)
  ward.aprdata.2016$reporting_unit = gsub("&","-",ward.aprdata.2016$reporting_unit)
  # ward.12.mun$reporting_unit = gsub("&","-",ward.12.mun$reporting_unit)
  
  ward.aprdata.2016$reporting_unit = gsub("AND","-",ward.aprdata.2016$reporting_unit)
  # ward.12.mun$reporting_unit = gsub("AND","-",ward.12.mun$reporting_unit)
  ...
  ward.aprdata.2016$reporting_unit = gsub("WD","",ward.aprdata.2016$reporting_unit)
  # ward.12.mun$reporting_unit = gsub("WD","",ward.12.mun$reporting_unit)
  
  ward.aprdata.2016$reporting_unit = gsub("COMBINED","",ward.aprdata.2016$reporting_unit)
  # ward.12.mun$reporting_unit = gsub("COMBINED","",ward.12.mun$reporting_unit)
  ...
  ward.aprdata.group = dplyr::group_by(ward.aprdata.2016,muni_county, county, municipality)
  aprdata.municounty = dplyr::summarise(ward.aprdata.group,
...
# Ugh, I need to change the name of these muni_counties to make sure I have a full match
setdiff(unique(aprdata.municounty$muni_county), unique(ward.2016.2012.o.dem$muni_county))
setdiff(unique(ward.2016.2012.o.dem$muni_county), unique(aprdata.municounty$muni_county))

# differences = setdiff(unique(aprdata.municounty$muni_county),unique(ward.2016.2012.o.dem$muni_county))
# aprdata.diffs = subset(aprdata.municounty, muni_county %in% differences)

differences = setdiff(unique(ward.2016.2012.o.dem$muni_county),unique(aprdata.municounty$muni_county))
aprdata.diffs = subset(ward.2016.2012.o.dem, muni_county %in% differences)
...
aprdata.repunit.join = with(ward.aprdata.2016, data.frame(reporting_unit))
aprdata.repunit.join$total_ballots = ifelse(ward.aprdata.2016$total_ballots == 0,
                                    ward.aprdata.2016$total_voters,
                                    ward.aprdata.2016$total_ballots)
aprdata.repunit.join$tch_prop = with(ward.aprdata.2016, dre / total_ballots)
aprdata.repunit.join$os_prop = with(ward.aprdata.2016, optical_scan_ballots / total_ballots)
aprdata.repunit.join$paper_prop = with(ward.aprdata.2016, paper_ballots / total_ballots)
aprdata.repunit.join$absentee_person_prop = with(ward.aprdata.2016, absentee_in_person / absentee_issued)
aprdata.repunit.join$inspector_prop = with(ward.aprdata.2016, total_election_inspectors / total_ballots)
# aprdata.repunit.join$tch_prop = ifelse(aprdata.repunit.join$tch_prop>1, 1, aprdata.repunit.join$tch_prop)

aprdata.repunit.join$turnout_reg_2016 = with(ward.aprdata.2016, total_ballots / registrants)
aprdata.repunit.join$sameday_reg_prop = with(ward.aprdata.2016, election_day_registrants / registrants)
aprdata.repunit.join$counted_absent_prop = with(ward.aprdata.2016,
                                        counted_2 / total_ballots)
aprdata.repunit.join$rejected_absent_prop = with(ward.aprdata.2016,
                                         rejected2 / total_ballots)
aprdata.repunit.join$registrants = with(ward.aprdata.2016, registrants)
aprdata.repunit.join$late_registrants = with(ward.aprdata.2016, late_registrants / total_ballots)

aprdata.repunit.join$ballots_counted_at = ward.aprdata.2016$ballots_counted_at
aprdata.repunit.join$absentee_undeliverable = with(ward.aprdata.2016, absentee_undeliverable / absentee_issued)


# Made up variables for municipality level
aprdata.join = with(aprdata.municounty, data.frame(muni_county))
aprdata.join$total_ballots = ifelse(aprdata.municounty$total_ballots == 0,
                                   aprdata.municounty$total_voters,
                                   aprdata.municounty$total_ballots)
aprdata.join$tch_prop = with(aprdata.municounty, dre / total_ballots)
# aprdata.join$tch_prop = ifelse(aprdata.join$tch_prop>1, 1, aprdata.join$tch_prop)
aprdata.join$os_prop = with(aprdata.municounty, optical_scan_ballots / total_ballots)
aprdata.join$paper_prop = with(aprdata.municounty, paper_ballots / total_ballots)
aprdata.join$absentee_person_prop = with(aprdata.municounty, absentee_in_person / absentee_issued)
aprdata.join$inspector_prop = with(aprdata.municounty, total_election_inspectors / total_ballots)

aprdata.join$turnout_reg_2016 = with(aprdata.municounty, total_ballots / registrants)
aprdata.join$sameday_reg_prop = with(aprdata.municounty, election_day_registrants / registrants)
aprdata.join$counted_absent_prop = with(aprdata.municounty,
                                              counted_absent / total_ballots)
aprdata.join$rejected_absent_prop = with(aprdata.municounty,
                                              rejected_absent / total_ballots)
aprdata.join$registrants = with(aprdata.municounty, registrants)
aprdata.join$late_registrants = with(aprdata.municounty, late_registrants / total_ballots)

ward.2016.2012.o.dem = plyr::join(ward.2016.2012.o.dem, aprdata.join, by = "muni_county")
ward.2016.2012.r.dem = plyr::join(ward.2016.2012.r.dem, aprdata.join, by = "muni_county")

write.csv(aprdata.municounty,"apr.2017.municounty.data.csv")

repunit.2016.2012.o.dem = plyr::join(repunit.2016.2012.o.dem, aprdata.repunit.join, by = "reporting_unit")
repunit.2016.2012.r.dem = plyr::join(repunit.2016.2012.r.dem, aprdata.repunit.join, by = "reporting_unit")

write.csv(ward.aprdata.2016,"apr.2017.repunit.data.csv")
...
ward.2016.2012.o.dem = plyr::join(ward.2016.2012.o.dem, edl2012.join, by = "muni_county")
ward.2016.2012.r.dem = plyr::join(ward.2016.2012.r.dem, edl2012.join, by = "muni_county")

write.csv(edl2012.municounty,"edl.2012.municounty.data.csv")

repunit.2016.2012.o.dem = plyr::join(repunit.2016.2012.o.dem, repunit.2012.edl.join, by = "reporting_unit")
repunit.2016.2012.r.dem = plyr::join(repunit.2016.2012.r.dem, repunit.2012.edl.join, by = "reporting_unit")
...
# ward.2016.2008.r.dem = plyr::join(ward.2016.2008.r.dem, edl2008.join, by = "muni_county")
...
repunit.2016.2008.r.dem = plyr::join(repunit.2016.2008.r.dem, repunit.2008.edl.join, by = "reporting_unit")
...
model.o.df = plyr::join(model.o.df,tig_dems_join, by = "MCD_FIPS")
...
{ # Adding in vote change from Spring primary
  ...
  model.o.df = plyr::join(model.o.df,repprim_join,by="muni_county")
  ...
  model.o.df = plyr::join(model.o.df,demprim_join,by="muni_county")
...
{ # Adding in vote change from 2012 to primaries
  ...
  # model.o.df = plyr::join(model.o.df,edl_join,by="muni_county")
...
model.o.df = plyr::join(model.o.df, off_vote_machines, by = "muni_county")
...
model.o.df = plyr::join(model.o.df, density.join, by = "muni_county")
...
{ ### Creating a carto-presentable map
 ...
 wisc.join@data = plyr::join(wisc.join@data, model.o.df, by = "muni_county", match = "first")
 colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
...
# The correlation with the other variables
  ...
  model.r.df = plyr::join(model.r.df,tig_dems_join, by = "MCD_FIPS")
} # Creating df
  { # Adding in vote change from Spring primary
    model.reprim.df = read.csv("model_reprim.csv")
    repprim_join = model.reprim.df[,c(3:22)]
    ...
    model.r.df = plyr::join(model.r.df,repprim_join,by="muni_county")
    ...
    model.r.df = plyr::join(model.r.df,demprim_join,by="muni_county")
  ...
  { # Adding in vote change from 2012 to primaries
    ...
    # model.r.df = plyr::join(model.r.df,edl_join,by="muni_county")
  ...
  model.r.df = plyr::join(model.r.df, off_vote_machines, by = "muni_county")
  ...
  { # Getting municipality densities
    ...
    model.r.df = plyr::join(model.r.df, density.join, by = "muni_county")
  ...
  } # Municipality densities
  {
    ...
    wisc.join@data = plyr::join(wisc.join@data, model.r.df, by = "muni_county", match = "first")
    colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
    # wisc.join@data$county_machine_vendor_dealer = as.character( wisc.join@data$county_machine_vendor_dealer)
    wisc.join@data$demwinner = as.character( wisc.join@data$demwinner)
    wisc.join@data$demwinner2012 = as.character( wisc.join@data$demwinner2012)
...
setwd("C:/Users/s_cas/Dropbox/Perso/2016 voting election county results/Wisconsin/All prep together/All processed files/")
    ...
    repunit.2016.2012.o.dem = plyr::join(repunit.2016.2012.o.dem, ward.2016.2012.o.dem.mcdjoin, by = "muni_county")
    
    repunit.2016.2012.o.dem = plyr::join(repunit.2016.2012.o.dem, repunit.2016.2008.o.dem, by = "reporting_unit")
...
# Models repunit data 2012 2016 recount --------------------------------------------------
  ...
  { # Model data frame preparation
    ...
    repunit.2016.2012.r.dem$log.pop.sq.mile.2010 = log(repunit.2016.2012.r.dem$pop_sq_mile_2010)
    
    ward.2016.2012.r.dem.mcdjoin = with(ward.2016.2012.r.dem, data.frame(muni_county, MCD_FIPS))
    
    repunit.2016.2012.r.dem = plyr::join(repunit.2016.2012.r.dem, ward.2016.2012.r.dem.mcdjoin, by = "muni_county")
    
    repunit.2016.2012.r.dem = plyr::join(repunit.2016.2012.r.dem, repunit.2016.2008.r.dem, by = "reporting_unit")
    
    # repunit.2016.2012.r.dem$total
    
    model.r.repunit.df = with(repunit.2016.2012.r.dem, data.frame(MCD_FIPS,reporting_unit,reporting_unit_orig,county, muni_county,
                                                                  democrat_votes.perc, republican_votes.perc, other_votes.perc,
                                                                  democrat_votes.rec, republican_votes.rec, other_votes.rec,
                                                                  dem.change.perc,rep.change.perc,oth.change.perc,
                                                                  tch_prop, os_prop, paper_prop, counted_absent_prop,rejected_absent_prop,
...
reporting_units_compared = with(model.o.repunit.df, data.frame(reporting_unit_orig, muni_county, county, 
                                                               ...
                                                               total_ballots, registrants, turnout_reg_2016,
...
voting.bycandcounty.2016.joined = plyr::join(voting.bycandcounty.2016,county.summary.join,by="county")
...
counties.2000.2016.2000ppl = plyr::join(counties.2000.2016.2000ppl,county.turnout.lookup, by="county")
...
# Ordering by turnout percentage, setup summarised data frames
{
  ...
  county.2016.demrep.wins = dplyr::group_by(county.2016.vs.2012, dem.2016)
  ...
  county.2016.machines.mostlydem = dplyr::group_by(county.2016.vs.2012, all.machines,mostly.dem)
  county.2016.machines.mostlydem.group = dplyr::summarise(county.2016.machines.mostlydem,
                                                turnout.perc.2016 = mean(turnout.perc.allage.est),
                                                turnout.perc.2012 = mean(turnout.2012.perc)
  )

  county.2016.machines.mostlydemwins = dplyr::group_by(county.2016.vs.2012, all.machines,dem.wins)
...
# Ordering by turnout percentage, setup summarised data frames
{
  ...
  county.2016.machines.mostlydem = dplyr::group_by(county.2016.vs.2012, county_paper_or_paperplusmachine,mostly.dem)
  county.2016.machines.mostlydem.group = dplyr::summarise(county.2016.machines.mostlydem,
                                                          turnout.perc.2016 = mean(turnout.perc.allage.est),
                                                          turnout.perc.2012 = mean(turnout.2012.perc)
  )

  county.2016.machines.mostlydemwins = dplyr::group_by(county.2016.vs.2012, county_paper_or_paperplusmachine,dem.wins)
  county.2016.machines.mostlydemwins.group = dplyr::summarise(county.2016.machines.mostlydemwins,
                                                              turnout.perc.2016 = mean(turnout.perc.allage.est),
                                                              turnout.perc.2012 = mean(turnout.2012.perc)
  )
