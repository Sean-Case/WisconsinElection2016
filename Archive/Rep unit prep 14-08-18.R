...
{ # Matching voting statistics with the election statistics
  ...
  gabdata.join = with(gabdata.repunit, data.frame(reporting_unit,muni_county))
...
} # Matching voting statistics with election results
{
  ...
  # Also joining the data from the Election snapshot
  gov.2014.dem = plyr::join(gov.2014.dem, elec.snapshot.dem, by = "county", match = "first")
...
{ # Making model df
 ...
 model.2014.gov.repunit.df2 = with(model.2014.gov.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop,counted_absent_prop,rejected_absent_prop,
                                 sameday_reg_prop, total_ballots,registrants,late_registrants,
                                 county_paper_or_paperplusmachine,
                                 pct_white, ratio_nocollege_college, log.pop.sq.mile.2010,
                                 pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
...
names(mysheets$Sheet6)

head(mysheets$Sheet6)
...
for (i in 6) {
  ...
  names(sheet)[1:6] <- c("county", "reporting_unit", "total_votes_cast", "dem", "rep", "other")
  names(sheet)[grep("SCATTERING",names(mysheets[i]))] <- "scattering"
  
  names(sheet)[!(which(names(mysheets) %in% c("county", "reporting_unit", "total_votes_cast", "dem", "rep", "other", "scattering")))] <- "other"
...
{ # Preparing basic data
  ...
  cong.2014.l$cand.group[grep("mary_burke",cong.2014.l$cand)] = "democrat"
  cong.2014.l$cand.group[grep("walker",cong.2014.l$cand)] = "republican"
  cong.2014.l$cand.group[grep("fehr",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("robert_burke",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("mary_jo",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("steve_r",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("jumoka",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("brett_d_",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("jessica",cong.2014.l$cand)] = "other"
  cong.2014.l$cand.group[grep("susan_p",cong.2014.l$cand)] = "other"
...
} # Matching voting statistics with election results
{
  ...
  # Demographics from David Griffen and co
  cong.2014.dem = plyr::join(cong.2014.l, demographics.caps, by = "county", match = "first")
  
  # Also joining the data from the Election snapshot
  cong.2014.dem = plyr::join(cong.2014.dem, elec.snapshot.dem, by = "county", match = "first")
} # Pasting demographics onto the dataframe
{
  setwd("C:/Users/s_cas/Dropbox/Perso/2016 voting election county results/Wisconsin/All prep together/All processed files")
  off_vote_machines = read.xlsx2("voting_equipment_by_municipality_09_2016_xlsx_78114.xlsx1207162619.xlsx",
...
{ # Making model df
  ...
  model.2014.cong.repunit.df2 = with(model.2014.cong.repunit.df, data.frame(reporting_unit, reporting_unit_orig, county, muni_county, votes_perc_republican, votes_perc_democrat, votes_perc_other,turnout_perc, tch_prop,counted_absent_prop,rejected_absent_prop,
                                                                          sameday_reg_prop, total_ballots,registrants,late_registrants,
                                                                          county_paper_or_paperplusmachine,
                                                                          pct_white, ratio_nocollege_college, log.pop.sq.mile.2010,
                                                                          pop_sq_mile_2010,tch_model_off_full, os_model_off, bmd_model_off,
...
{ # Getting municipality densities
  ...
  # density.join = with(wisc.spat.o@data, data.frame(muni_county,muni.dense))
  # 
  # model.2014.cong.repunit.df = plyr::join(model.2014.cong.repunit.df, density.join, by = "muni_county")
...
# write.csv(model.2014.cong.df,"model_2014_gov.csv")
write.csv(model.2014.cong.repunit.df,"Prepped files/2014/model_2014_gov_repunit.csv")
