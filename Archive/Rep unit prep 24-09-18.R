...
# Presidential 2008 data - GOT TO HERE. MATCHING GAB ------------------------------------------------
...
{ # GAB  / EDL stats
  ...
  # 
  # ward.gabdata.group = dplyr::group_by(gabdata.2008.pres,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2008.pres.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(pres.2008.l$muni_county))
  # setdiff(unique(pres.2008.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # pres.2008.l$muni_county[grep("VILLAGE OF MAINE MARATHON",pres.2008.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # pres.2008.l$municipality[grep("TOWN OF MAINE MARATHON",pres.2008.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(pres.2008.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(pres.2008.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
...
} # Matching voting statistics with election results - GOT TO HERE
{
  # Adding demographic data to the ward data frame
  demographics.caps = demographics
  demographics.caps$county = toupper(demographics.caps$county)
