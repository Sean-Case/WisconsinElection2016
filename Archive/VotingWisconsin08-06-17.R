...
{   # 2008 ward data
  ...
  { # Loading 2008 ward vote data
    ...
    # Joining back county and municipality names
    ward.08.l = join(ward.08.l, ward.08.look, by = "reporting_unit", match = "first")
    
    tot.votes.look.group = group_by(ward.08.l,reporting_unit)
    tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                      votes.tot = sum(as.integer(votes.rec),na.rm=T)
    )
    
    ward.08.l = join(ward.08.l,tot.votes.look,by="reporting_unit")
    ...
    ward.08.mun.group = group_by(ward.08.l,reporting_unit,municipality,muni_county,county,cand.group)
  ...
  {  # Now joining the 2008 data back into the 2016 data
    ...
    # 
    # 
    # vote.2008.diffs.join = join(vote.2008.diffs.join,ward.08.mun.join,by = "reporting_unit")
    # 
    ...
    ward.08.mun.votes.group = group_by(ward.08.mun,cand.group,reporting_unit,reporting_unit_orig,municipality, county)
    ...
    ward.2016.o.mun.g = join(ward.2016.o.mun.votes.g.cast, ward.08.mun.votes.g.cast, by = "reporting_unit")
...
# Rejoining the county data to the ward 2016 2012 data
{
  ...
  repunit.2016.2008.o = join(repunit.2016.2008.o, ward.2016.look, by = "muni_county", match = "first")
}
...
{ # Preparing basic data
  ...
  ward.2016.r.l = gather(ward.2016.r, reporting_unit);
  colnames(ward.2016.r.l) = c("reporting_unit", "cand", "votes.rec")
  
  unique(ward.2016.r.l$cand)
  unique(ward.2016.r.l$cand.group)
  ...
  # Joining back county and municipality names
  ward.2016.r.l = join(ward.2016.r.l, ward.2016.look, by = "reporting_unit", match = "first")
  
  ward.2016.r.l$votes.rec = as.numeric(as.integer(ward.2016.r.l$votes.rec))
  # table(is.na(ward.2016.r.l$votes.rec))
  
  tot.votes.look.group = group_by(ward.2016.r.l,reporting_unit)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes.tot = sum(votes.rec,na.rm=T)
  )
  ward.2016.r.l = join(ward.2016.r.l,tot.votes.look,by="reporting_unit",match="first")
  ward.2016.r.l$votes.perc = (ward.2016.r.l$votes.rec / ward.2016.r.l$votes.tot) * 100
  
  ward.2016.r.l$muni_county = paste(ward.2016.r.l$municipality,
                                    ward.2016.r.l$county)
  
  ward.2016.r.mun.group = group_by(ward.2016.r.l,reporting_unit, municipality,muni_county,county,cand.group)
  ward.2016.r.mun = dplyr::summarise(ward.2016.r.mun.group,
                                     votes.rec2 = sum(votes.rec),
                                     votes.tot2 = mean(votes.tot)
  )
...
{   # 2012 ward data
  ...
  {  # Now joining the 2012 data back into the 2016 data
    ...
    # back.to.votes.df.join = with(vote.2012.diffs.join, data.frame(reporting_unit,reporting_unit_2012_agreed))
    # 
    # ward.2016.r.mun = join(ward.2016.r.mun, back.to.votes.df.join, by = "reporting_unit")
    # 
    ...
    ward.2016.r.mun.g = join(ward.2016.r.mun.votes.g.cast, ward.12.mun.votes.g.cast, by = "reporting_unit")
...
{   # 2008 ward data
  ...
  {  # Now joining the 2008 data back into the 2016 data
    ...
    ward.2016.r.mun.g = join(ward.2016.r.mun.votes.g.cast, ward.08.mun.votes.g.cast, by = "reporting_unit")
...
# Rejoining the county data to the ward 2016 2012 data
{
  repunit.2016.2012.r = join(repunit.2016.2012.r, ward.2016.look, by = "muni_county", match = "first")
  
  repunit.2016.2008.r = join(repunit.2016.2008.r, ward.2016.look, by = "muni_county", match = "first")
}
...
repunit.2016.2012.r = join(repunit.2016.2012.r,vot.equip.join, by = "muni_county")
...
ward.2016.2012.test = join(repunit.2016.2012.r, county.machines.join, by = "muni_county", match = "first")
...
# Joining data together
wisc.spat.j@data = join(wisc.spat.d.df@data, repunit.2016.2012.r, by = "MCD_FIPS", match = "first")
...
# Demographics from David Griffen and co
repunit.2016.2008.o.dem = join(repunit.2016.2008.o, demographics.caps, by = "county")

# Also joining the data from the Election snapshot
repunit.2016.2008.o.dem = join(repunit.2016.2008.o.dem, elec.snapshot.dem.wi, by = "county")
#

# RECOUNT
# Demographics from David Griffen and co
repunit.2016.2012.r.dem = join(repunit.2016.2012.r, demographics.caps, by = "county")

# Also joining the data from the Election snapshot
repunit.2016.2012.r.dem = join(repunit.2016.2012.r.dem, elec.snapshot.dem.wi, by = "county")

# Demographics from David Griffen and co
repunit.2016.2008.r.dem = join(repunit.2016.2008.r, demographics.caps, by = "county")

# Also joining the data from the Election snapshot
repunit.2016.2008.r.dem = join(repunit.2016.2008.r.dem, elec.snapshot.dem.wi, by = "county")
...
# Ward 83, City of Madison, Dane has 709 late ballots in registered voter pop of 1698
 ...
 # Preparing basic data
  ...
  ward.aprdata.group = group_by(ward.aprdata.2016,muni_county, county, municipality)
...
ward.2016.2012.o.dem = join(ward.2016.2012.o.dem, aprdata.join, by = "muni_county")
ward.2016.2012.r.dem = join(ward.2016.2012.r.dem, aprdata.join, by = "muni_county")

write.csv(aprdata.municounty,"apr.2017.municounty.data.csv")

repunit.2016.2012.o.dem = join(repunit.2016.2012.o.dem, aprdata.repunit.join, by = "reporting_unit")
repunit.2016.2012.r.dem = join(repunit.2016.2012.r.dem, aprdata.repunit.join, by = "reporting_unit")
...
repunit.2016.2012.r.dem = join(repunit.2016.2012.r.dem, repunit.2012.edl.join, by = "reporting_unit")
...
# Grouping the data into municipalities
ward.edl2008.group = group_by(ward.gabdata.2008,muni_county, county, municipality)
...
# ward.2016.2008.o.dem = join(ward.2016.2008.o.dem, edl2008.join, by = "muni_county")
# ward.2016.2008.r.dem = join(ward.2016.2008.r.dem, edl2008.join, by = "muni_county")

write.csv(edl2008.municounty,"edl.2008.municounty.data.csv")

repunit.2016.2008.o.dem = join(repunit.2016.2008.o.dem, repunit.2008.edl.join, by = "reporting_unit")
repunit.2016.2008.r.dem = join(repunit.2016.2008.r.dem, repunit.2008.edl.join, by = "reporting_unit")
...
setwd("C:/Users/s_cas/Dropbox/Perso/2016 voting election county results/Wisconsin/All prep together/All processed files/")
    ...
    repunit.2016.2012.o.dem = join(repunit.2016.2012.o.dem, repunit.2016.2008.o.dem, by = "reporting_unit")
...
# Models repunit data 2012 2016 recount --------------------------------------------------
  ...
  { # Model data frame preparation
    ...
    repunit.2016.2012.r.dem = join(repunit.2016.2012.r.dem, ward.2016.2012.r.dem.mcdjoin, by = "muni_county")
    
    repunit.2016.2012.r.dem = join(repunit.2016.2012.r.dem, repunit.2016.2008.r.dem, by = "reporting_unit")
    ...
    model.r.repunit.df = join(model.r.repunit.df,tig_dems_join, by = "MCD_FIPS")
  ...
  { # Adding in vote change from Spring primary
    ...
    model.r.repunit.df = join(model.r.repunit.df,repprim_join,by="reporting_unit")
    ...
    model.r.repunit.df = join(model.r.repunit.df,demprim_join,by="reporting_unit")
  ...
  { # Adding in vote change from 2012 to primaries
    ...
    # model.r.repunit.df = join(model.r.repunit.df,edl_join,by="muni_county")
  ...
  { # Adding in voting machines
    ...
    model.r.repunit.df = join(model.r.repunit.df, off_vote_machines, by = "muni_county")
  ...
  { # Getting municipality densities
    ...
    # 
    # pop.join = with(model.r.df, data.frame(muni_county,pop))
    # wisc.spat.r@data = join(wisc.spat.r@data, pop.join, by = "muni_county")
    # 
    ...
    model.r.repunit.df = join(model.r.repunit.df, density.join, by = "muni_county",match="first")
  ...
  { ### Creating a carto-presentable map
    ...
    # wisc.join = wisc.spat.r.cut
    # 
    # wisc.join@data = join(wisc.join@data, model.r.repunit.df, by = "muni_county", match = "first")
    # colnames(wisc.join@data) = make.unique(colnames(wisc.join@data))
    # wisc.join@data$county_machine_vendor_dealer = as.character( wisc.join@data$county_machine_vendor_dealer)
    # wisc.join@data$demwinner = as.character( wisc.join@data$demwinner)
    # wisc.join@data$demwinner2012 = as.character( wisc.join@data$demwinner2012)
