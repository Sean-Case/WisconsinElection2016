...
{ # Preparing basic data
  ...
  # Any commas in numbers will wreck all calculations, need to remove them
  pres.2012 <- as.data.frame(lapply(pres.2012, function(y) (gsub(",", "", y))))
  
  # Get the municipality names from the reporting unit names    
  odd_indexes<-seq(1,length(pres.2012$reporting_unit)*2,2)
  
  require(stringr)
  pres.2012$reporting_unit[which(grepl("[0-9]", pres.2012$reporting_unit) == F)] <- paste(pres.2012$reporting_unit[which(grepl("[0-9]", pres.2012$reporting_unit) == F)], "1", sep = " ")
  ...
  pres.2012$reporting_unit <- gsub("\\bWDS\\b", "WARD", pres.2012$reporting_unit)
  ...
  # rm(pres.20122,pres.20123,pres.20124)
...
{ # GAB  / EDL stats
  ...
  # gabdata.2012.pres.g <- group_by(gabdata.2012.pres, hindi, county, municipality, reporting_unit)
  # gabdata.2012.pres.m <- dplyr::summarise_if(gabdata.2012.pres.g, is.numeric, sum, na.rm = T)
  ...
  # 
  # ward.gabdata.group = dplyr::group_by(gabdata.2012.pres,reporting_unit, reporting_unit_orig, muni_county, county, municipality)
  # gabdata.repunit = dplyr::summarise(ward.gabdata.group,
  #                                       registrants = sum(registrants),
  #                                       late_registrants = sum(late_registrants),
  #                                       election_day_registrants = sum(election_day_registrants),
  ...
  # ward.gabdata.group = NULL
  # 
  # write.csv(gabdata.repunit,"jun.2016.2012.pres.repunit.data.csv")
} # GAB voting statistics
{ # Matching voting statistics with the election statistics
  
  # # Maybe need to change the name of these muni_counties to make sure I have a full match
  # setdiff(unique(gabdata.repunit$muni_county), unique(pres.2012.l$muni_county))
  # setdiff(unique(pres.2012.l$muni_county), unique(gabdata.repunit$muni_county))
  # 
  # pres.2012.l$muni_county[grep("VILLAGE OF MAINE MARATHON",pres.2012.l$muni_county)] = "TOWN OF MAINE MARATHON"
  # pres.2012.l$municipality[grep("TOWN OF MAINE MARATHON",pres.2012.l$muni_county)] = "TOWN OF MAINE"
  # 
  # differences = setdiff(unique(pres.2012.l$muni_county),unique(gabdata.repunit$muni_county))
  # gabdata.diffs = subset(pres.2012.l, muni_county %in% differences)
  # 
  # Missing:
  # TOWN OF HAYWARD (all put in city of Hayward), Albion Trempeleau (no data),
  # Caledonia Trempeleau (no data), Hendren Clark (no data), Strum Trempeleau (no data)
...
# write.csv(model.2012.pres.df,"model_2012_pres.csv")
...
names(model_2012_pres_repunit)
names(model_2016_pres_repunit)

setdiff(names(model_2012_pres_repunit), names(model_2016_pres_repunit))
