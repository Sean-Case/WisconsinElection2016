...
pres.2012[,5:length(pres.2012)] = apply(pres.2012[,5:length(pres.2012)], 2, function(x) as.character(x))
pres.2012[,5:length(pres.2012)] = apply(pres.2012[,5:length(pres.2012)], 2, function(x) as.numeric(x))
...
# str(pres.2012)
pres.2012.g <- group_by(pres.2012, reporting_unit, county_name, municipality_name, original_or_recount)
pres.2012.m <- summarise_all(pres.2012.g, sum)



{ # Preparing basic data
  names(pres.2012)[1] <- "county"
  names(pres.2012)[5] <- "votes.tot"
...
{ # Making model df
  model.2016.pres.repunit.df = pres.2012.dem
  # model.2016.pres.df = subset(model.2016.pres.df, votes_tot > 19)
