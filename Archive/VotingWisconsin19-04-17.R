...
{ # Preparing basic data
  ...
  ward.2016.o.l$cand.group[grep("michael",ward.2016.o.l$cand)] = "other"
...
{ # Preparing basic data
  ...
  ward.2016.r.l$cand.group[grep("michael",ward.2016.r.l$cand)] = "other"
...
{ # Preparing basic data
  ...
  ward.prim.2016.l = gather(ward.prim.2016.v, muni_county)
  colnames(ward.prim.2016.l) = c("muni_county", "cand", "votes_rec")
  ...
  # Joining back county and municipality names
  ward.prim.2016.l = join(ward.prim.2016.l, ward.prim.2016.look, by = "muni_county", match = "first")

  ward.prim.2016.l$votes_rec = as.numeric(as.integer(ward.prim.2016.l$votes_rec))

  tot.votes.look.group = group_by(ward.prim.2016.l,muni_county)
  tot.votes.look = dplyr::summarise(tot.votes.look.group,
                                    votes_tot = sum(votes_rec)
  )

  ward.prim.2016.l = join(ward.prim.2016.l,tot.votes.look,by="muni_county",match="first")
  ...
  ward.prim.2016.mun.group = group_by(ward.prim.2016.l,municipality,muni_county,county,cand.group)
  ward.prim.2016.mun = dplyr::summarise(ward.prim.2016.mun.group,
                                     votes_rec2 = sum(votes_rec),
                                     votes_tot2 = mean(votes_tot)
  )
  colnames(ward.prim.2016.mun)[5] = "votes_rec"
  colnames(ward.prim.2016.mun)[6] = "votes_tot"

  ward.prim.2016.mun$votes_perc = (ward.prim.2016.mun$votes_rec / ward.prim.2016.mun$votes_tot) * 100

  ward.prim.2016.melt = melt(ward.prim.2016.mun)
  ward.prim.2016.cast = dcast(ward.prim.2016.melt, muni_county + municipality + county ~ variable + cand.group, sum)
  ward.prim.2016.mun = ward.prim.2016.cast

  ward.prim.2016.mun$trump_prim_won = with(ward.prim.2016.mun,
                                           ifelse(votes_perc_trump > votes_perc_kasich &
                                                    votes_perc_trump > votes_perc_cruz &
                                                    votes_perc_trump > votes_perc_other,1,0))

  ward.prim.2016.mun$votes_tot_cruz = NULL
  ward.prim.2016.mun$votes_tot_trump = NULL
  ward.prim.2016.mun$votes_tot_kasich = NULL

  colnames(ward.prim.2016.mun)[grep("votes_tot_other",colnames(ward.prim.2016.mun))] = "votes_tot"

  rm(ward.prim.2016.cast,ward.prim.2016.melt,
  ward.prim.2016.mun.group ,ward.prim.2016.l ,
...
{ # Spring primary voting statistics June 2016
  ...
  ward.jundata.group = group_by(ward.jundata.2016,muni_county, county, municipality)
  ...
  write.csv(jundata.municounty,"jun.2016.municounty.data.csv")
} # Spring primary voting statistics June 2016
{ # Matching voting statistics with the election statistics
  ...
  setdiff(unique(jundata.municounty$muni_county), unique(ward.prim.2016.mun$muni_county))
  setdiff(unique(ward.prim.2016.mun$muni_county), unique(jundata.municounty$muni_county))

  ward.prim.2016.mun$muni_county[grep("VILLAGE OF MAINE MARATHON",ward.prim.2016.mun$muni_county)] = "TOWN OF MAINE MARATHON"
  ward.prim.2016.mun$municipality[grep("TOWN OF MAINE MARATHON",ward.prim.2016.mun$muni_county)] = "TOWN OF MAINE"

  differences = setdiff(unique(ward.prim.2016.mun$muni_county),unique(jundata.municounty$muni_county))
  jundata.diffs = subset(ward.prim.2016.mun, muni_county %in% differences)
  ...
  jundata.join = with(jundata.municounty, data.frame(muni_county))
  ...
  ward.prim.2016.mun2 = join(ward.prim.2016.mun, jundata.join, by = "muni_county")
  ward.prim.2016.mun = ward.prim.2016.mun2

  ward.prim.2016.mun[is.na(ward.prim.2016.mun)] = NA
  rm(ward.prim.2016.mun2)
...
} # Matching voting statistics with the election statistics
{
    ...
    # Demographics from David Griffen and co
    ward.prim.2016.dem = join(ward.prim.2016.mun, demographics.caps, by = "county", match = "first")
...
{ # Making model df
  model.prim.df = ward.prim.2016.dem
  model.prim.df = subset(model.prim.df, votes_tot > 19)
   ...
   model.prim.df$log.pop.sq.mile.2010 = log(model.prim.df$pop_sq_mile_2010)
    ...
    ##
   model.prim.df2 = with(model.prim.df, data.frame(county, muni_county,
      votes_perc_trump, votes_perc_kasich, votes_perc_cruz,trump_prim_won,turnout_perc,
    ...
    rownames(model.prim.df2) = model.prim.df2$muni_county

    model.prim.df = model.prim.df2
  } # Making model df
{ # Getting municipality densities
    ...
    wisc.spat.o@data$muni.dense = as.numeric(wisc.spat.o@data$voters.tot2016) / wisc.spat.o@data$muni.area
    ...
    model.prim.df = join(model.prim.df, density.join, by = "muni_county")
...
{ ### Creating a carto-presentable map
    ...
    wisc.join@data = join(wisc.join@data, model.prim.df, by = "muni_county", match = "first")
...
{ # Preparing basic data
  ...
  ward.prim.2016.mun$clinton_prim_won = with(ward.prim.2016.mun,
                                           ifelse(votes_perc_clinton > votes_perc_sanders &
                                                    votes_perc_clinton > votes_perc_other,1,0))
  ward.prim.2016.mun$votes_tot_clinton = NULL
  ward.prim.2016.mun$votes_tot_sanders = NULL
